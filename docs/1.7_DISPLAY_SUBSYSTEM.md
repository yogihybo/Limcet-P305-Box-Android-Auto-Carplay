...
- **Kernel:** the `screen=N` boot argument sets `g_screen_id`, which indexes a
  ...
  vendor reference source (see [`vendor_source/README.md`](../vendor_source/README.md))
...
1. **Stage 1 — `mtd4` `arkdata.ini` (per-physical-unit, authoritative).**
   ...
   `firmware_source/mtd4_arkdata/arkdata.ini`
...
(`ark_display_lcd.c` and `uboot/ark_lcd.c`, see [`vendor_source/README.md`](../vendor_source/README.md)) is the concrete reference for
...
**Source:** ASTRI reference BSP (see [`vendor_source/README.md`](../vendor_source/README.md)) — ASTRI's (Hong Kong Applied
Science and Technology Research Institute) reference ARK1680 kernel/U-Boot/userspace source.
...
the real kernel display driver in `ark_display_lcd.c`:

...
| [`../combined_cleanroom_boot_factory_app_64k.bin`](../combined_cleanroom_boot_factory_app_64k.bin) (SHA-256 `62d37aaaa79daca94d4f55a4555fb18e434f46300b7c9323139aaf50ff05899b`) | This project's own clean-room bootloader (`hardware/MCU/bootloader/`) + reconstructed factory app (#4 above) | Working alternative combo — the clean-room bootloader explicitly sets `SCB->VTOR` itself before jumping (see its `jump_to_application()`), so it doesn't depend on the app's `Reset_Handler` running correctly. **Re-flashed and independently hardware-verified on the spare board (2026-09-14, post vector-fix regeneration)**: handshake `*0x20004000 == 0x20141003` pass, `SCB->VTOR == 0x08003000` pass, `CFSR`/`HFSR` both `0` sustained across repeated samples, `MSP = 0x20001710` matching the factory-bootloader combo's own confirmed value, CAN1 active (`CAN1->MSR = 0x00000C00`, `CAN1->BTR = 0x00140008` — 500kbps, the clean-room bootloader's own timing config), `PC` genuinely advancing through app code with no stuck/trapped state observed. |

...
These are **not the same kernel** — 30 build numbers and ~22 months apart. The "dump" tree is the raw MTD5 pull straight from a live physical unit; the "reconstructed" tree appears to carry a later build (from a firmware update package or a different source — see `docs/13.1_SOURCES.md`). Both target the same SoC/driver family (RN6752, Goodix-TS, ARK1680 identifiers all present in both), so the findings below likely transfer, but **any GPIO/register-offset claim here is only confirmed against build #353**, not #383. Worth re-running the same Ghidra pass against the `reconstructed` zImage to confirm no board-file changes crept in between builds.
...
**GPIO 95 = `PBANK_2`, pin 31.** Label `"apple_encpy_ic_rst"` = reset line for an Apple MFi/CarPlay authentication IC — consistent with this being a Toyota OEM head unit (`ProductId=Limcet-P306`, `HomeIconLabel=TOYOTA`, per `docs/13.1_SOURCES.md`) needing wired CarPlay support.
...
**Found the factory/dispatcher:** `MCUAdapter::getAdapterInstance(MCUAdapter::McuType)` (exact demangled symbol name — no guessing needed) is a 30-case `switch` on the numeric `McuType` value, `case 1` through `case 0x1e`, each `operator_new`-ing and constructing the matching adapter class. This is the mechanism `docs/13.1_SOURCES.md`'s `McuType` field (16 for Holden, **6 for Limcet P306**) actually drives.

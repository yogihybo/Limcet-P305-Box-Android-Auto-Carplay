...
`docs/1.5_AUDIO_SUBSYSTEM_INVESTIGATION.md`), and `soc_probe_aux_devices()`
...
worse and was reverted** — see `docs/1.4_WIRELESS_AND_INIT.md` §7 for the
...
(`docs/1.4_WIRELESS_AND_INIT.md` §6, `docs/1.8_ARK1680_TS_REVERSE_ENGINEERING.md`,
`tools/mcu-handshake/README.md`, `docs/1.6_BD37033.md`) — if something looks
...
**Conclusion**: "factory" camera mode is almost certainly a genuine hardware video bypass -- most plausibly the companion MCU (which already independently reads the reverse-gear trigger directly, confirmed via schematic + binary RE in `1.1_HARDWARE_AND_SOC_REFERENCE.md` §7) driving a physical relay/video-mux at the panel, entirely outside the SoC/kernel/MsnCoreApp stack. A deliberate, common automotive-head-unit safety feature (factory camera keeps working even if the aftermarket unit crashes or is powered off) -- not a bug, nothing to fix. This **narrows the still-open black-flash issue specifically to "aftermarket" camera mode**, which does route through MsnCoreApp/the kernel's own ITU656/RN6752 video-layer path -- reinforcing the video-layer-config-gap hypothesis above. A `dmesg` capture from an aftermarket-mode test is still the needed next step.
...
- **CAN bus**: no kernel driver in either stock or our build (`CONFIG_CAN` unset both sides; stock `vmlinux.elf`'s `can_*` symbols are unrelated VFS/scheduler helpers, not SocketCAN). Matches `docs/1.2_CANBUS.md`'s existing conclusion despite the schematic's `CAN_TRANSCEIVER` block.
...
- **Suspend/power management**: `CONFIG_SUSPEND` not set, no ACC/power-key GPIO node in the DTS -- and stock's `vmlinux.elf` has no `pm_suspend`/suspend-ops symbols either. Confirmed intentional: ACC/ignition power handling is done entirely by the companion MCU cutting the supply rail, not kernel suspend/resume (consistent with `1.1_HARDWARE_AND_SOC_REFERENCE.md`'s MCU/ACC findings elsewhere). Worth stating explicitly in `1.9_KERNEL_REFERENCE.md` so this stops looking like an open gap in future audits.
...
- Bluetooth HS-UART (`/dev/ttyHS1`, `ark1668-pinctrl.dtsi`'s `pinctrl_uart5`) has no RTS/CTS pins wired, even though `ark_hsuart.c` implements `CRTSCTS` handling. BT is already extensively confirmed working end-to-end (audio, HFP, data) per `docs/1.4_WIRELESS_AND_INIT.md`, so this reads as an intentional 2-wire link design, not a bug -- flagged only as a latent risk if BT throughput/reliability issues ever surface.
...
**Follow-up, same day: permanent-lockout bug found and fixed before this ever shipped.** User asked whether `bootcount` could also be cleared from stock's own NAND-resident rootfs (via an rcS command), to catch pure U-Boot-level hangs too (not just a bad `bootusb` image) -- specifically noting this wouldn't cover runtime crashes, only boot-time hangs. Checked first whether that's even feasible: `nandboot` boots the **genuinely untouched, physically-flashed vendor NAND content** -- confirmed no script anywhere in this project ever writes to NAND (`firmware_source/mtd6_rootfs` is only ever used to build SD/USB images for `bootmmc`/`bootusb`, never flashed back). Doing so would mean flashing our own content onto the real device's NAND for the first time -- a new, meaningfully higher-risk category of action this project has deliberately avoided, specifically because NAND's untouched state is what makes it a reliable fallback.

Surfaced a real bug in the process, independent of the NAND question: `bootcount` was **only** ever cleared by our own `rcS` (`fw_setenv bootcount 0`), which never runs when `nandboot` boots stock's rootfs. Once `bootcount` exceeded `bootlimit` (3), the device would be **permanently** stuck skipping `bootusb` forever -- even after whatever caused the original failures got fixed -- since a successful `nandboot` never reset the counter.

**Fixed** (`linux-arkmicro` commit `04c46b7ee`): `do_bootnand()` now clears `bootcount` itself, right before running the `nandboot` env script -- U-Boot successfully reaching that point (not hung/crashed earlier) is itself evidence U-Boot/hardware is fundamentally healthy, independent of whether `bootusb`'s specific kernel image was good or bad, which is the actual thing `bootcount` is meant to track. Stays entirely within U-Boot/this project's own repo, no NAND write needed. Builds clean. Not yet hardware-tested.

## 80. Ported stock's instant-boot reverse-camera preview + MCU notify to our U-Boot (2026-07-31)

User asked how stock's real U-Boot handles the reverse camera and whether we'd ported all of it. A subagent disassembled stock's genuine 2012.10 `uboot.bin` and found we hadn't: our port only reads the reverse-gear GPIO and hands the value to the kernel via `ATAG_BACKCAR` (§46-ish era work). Stock does substantially more -- an **instant, pre-kernel camera preview**: an ITU656 hardware bypass wired straight into the LCDC video layer, live within ~1-2s of power-on if already in reverse, entirely independent of Linux. Distinct from both the kernel-side `track_paint_init()` parking-guide overlay and the fully-separate MCU/relay "factory" camera bypass (§76).

**Key discovery while planning the port**: most of the hard register-level work already existed. `ark1668_debug_cmds.c`'s `do_itu656()` (added in an earlier session, "ported from the stock production u-boot binary" via Ghidra decompilation) implements nearly the exact same sequence the subagent found in stock's automatic boot-time call chain -- same function addresses down to the identical `LCD_BASE + 0x320 |= 0x800` enable-bit write. This was "wire up and verify an existing but never-yet-hardware-tested port," not reverse-engineer-from-scratch. **Important standing caveat, carried forward**: `docs/5.1_UBOOT_REVERSE_ENGINEERING.md` §7 already flags `itu656` as one of the riskiest untested commands ("real hardware register writes with inferred, not datasheet-confirmed, semantics") -- **never tried on real hardware**. One block (`FUN_0006e9f0`, branches on unresolved runtime pointers, "likely current screen/resolution state") was deliberately left out in that earlier port; not revisited here, only worth chasing if hardware testing shows a wrong picture.

**Implemented** (`linux-arkmicro` commit `d0507710d`):
- Extracted `do_itu656()`'s register-write body into `ark_itu656_camera_bypass_enable()` (`ark1668_debug_cmds.c`) so both the existing manual `itu656` command and the new automatic path share one implementation.
- New `ark_carback_camera_check()` (`ark1668_display_cfg.c`): reads a `carback_camera_mode` env gate and the reverse-gear GPIO (reusing `ARK_BACKCAR_GPIO` from the existing `do_backcarcheck()`), calls the camera bypass when in reverse. Wired as the very first thing in `CONFIG_BOOTCOMMAND` (before uEnv.txt import, `bootcheck`, or medium selection) via a new `carbackcamcheck` command -- matches stock's own early timing and the feature's whole point (fastest possible camera-on).
- New `ark_mcu_notify_backcar()`: stock's U-Boot→MCU UART backcar-onoff notification. **This one has a real prior-session finding worth flagging explicitly**: the comment above `do_backcarcheck()` already documented this exact function (`FUN_0006ede4`) as "deliberately NOT ported... no caller of that function could be found anywhere in stock's binary after an exhaustive search (direct branch, literal pool, movw/movt, thumb interworking), so it may be dead code." **Ported anyway per explicit instruction**, despite that reachability doubt not being resolved. Disassembled it fresh to get concrete specifics: UART2 (`0xE8000000`, matching our own `CONFIG_SYS_SERIAL2`), a 7-byte frame (`0d 24 03 00 01 ff 02`) sent unconditionally plus a 6-byte frame (`0d 24 02 02 04 fb`) sent additionally only when the input parameter is nonzero, both extracted directly from stock's rodata (high confidence -- literal data). Also replicated stock's own UART2 one-time init sequence (pinmux at `SYS_BASE+0x1e0`, baud/control registers) since this port isn't necessarily already clocked/configured if it's not the active console. **Unconfirmed**: which parameter value means "entering" vs "leaving" reverse -- no direct caller was found to check the real calling convention against, so this is implemented as nonzero=camera-on (send both frames) as a best-effort inference, flagged as the first thing to flip if hardware testing shows the MCU reacting backwards.
- Explicitly excluded: `FUN_0006ee8c`'s "arkupdate" ASCII notify (literal string content indicates an unrelated firmware/config-update mechanism, not backcar/camera).

Full disassembly trail and reasoning: `/home/osboxes/.claude/plans/mossy-crafting-oasis.md` (the approved implementation plan).

Builds and injects ARK header clean. **NOT YET HARDWARE-TESTED.** Given the standing "never tested" caveat on the underlying `itu656` register sequence itself, the necessary first step before trusting any of this automatically is exactly what §UBOOT_REVERSE_ENGINEERING §7 already recommended: manually run `itu656` at the U-Boot prompt with the vehicle in reverse and confirm a correct picture appears, *before* relying on the new automatic `carbackcamcheck` path.

## 81. First real hardware test of today's U-Boot changes -- `bootnand` hangs (fixed), autoboot-vs-manual discrepancy found, `/dev/mtd3` missing under Linux

User's first hardware test of §79/§80's changes (`docs/logs/new uboot new kernel baseline v23_260731.txt`) surfaced three things.

**1. `bootnand` hangs on real hardware -- fixed.** User reported it directly. Per explicit instruction, reversed the earlier "never arm the watchdog before nandboot" decision specifically for `do_bootnand()` (`linux-arkmicro` commit `6f550e353`) -- `ark_wdt_arm(30000)` now runs right before `run_command("run nandboot", 0)`, same pattern as `boot_from_block_dev()`'s 20s arm, just more generous since this path does its own NAND I/O before ever reaching `bootz`. The original exclusion reasoning (stock's kernel doesn't feed our watchdog, so arming it there risks firing mid-boot on an otherwise-working path) is still noted in the code as an accepted, not-fully-eliminated residual risk -- but a confirmed hang with zero recovery is strictly worse than a theoretical false-trigger risk, so the trade-off was accepted. `bootstock`/`bootstockusb`/`boothybrid` remain untouched (no hang reported there). Not yet hardware-tested with this specific fix.

**2. Autoboot (after the countdown) appears to produce zero console output and never boots; typing `bootmmc` manually at the prompt afterward works fine.** User confirmed this predates today's `carbackcamcheck`/`bootcheck` additions -- not caused by them originating this symptom class, though a second, self-inflicted cause was found in the same code (see below). Directly visible in `v23`'s log: `Hit space to stop autoboot:  0` (line 67) is immediately followed by `=> bootmmc` (line 68, an interactively-typed command, given the very next line is `do_bootmmc()`'s own printf) -- nothing from `carbackcamcheck`, the uEnv.txt fatload attempt, or `bootcheck` ever printed in between, even though all three are supposed to run automatically, in that order, before anything else.

Two non-exclusive candidate causes:
- **Stale saved `bootcmd` -- RULED OUT, correction below.** Originally theorized U-Boot's saved environment shadows the compiled-in `CONFIG_BOOTCOMMAND` once anything's ever been `saveenv`'d. **User corrected this same day**: the SD card is a freshly built image each test, so any error has to trace back to something in this repo's own files, not accumulated device-side state -- which is exactly right, and led directly to finding something far more fundamental (see §82): `configs/ark1668_limcet_p305_defconfig` had `CONFIG_ENV_IS_NOWHERE=y`, whose backend (`env/nowhere.c`) has no save/load implementation at all. **`saveenv`/`env_save()` was never persisting anything, anywhere** -- there was no stale value for `bootcmd` to get stuck at, because nothing was ever actually being saved in the first place. See §82 for the real fix.
- **Found and fixed, same day**: `ark_carback_camera_check()` (the new first command in `CONFIG_BOOTCOMMAND`, §80) calls `ark_mcu_notify_backcar()` -> `ark_uart2_putc()`, which had an **unbounded busy-wait** on the UART2 TX-ready bit, no timeout. This runs automatically on every boot, gated only by a GPIO5 reverse-gear read -- on a bench/test rig a floating or unconnected GPIO5 pin can easily read as asserted (active-low), triggering this never-hardware-tested UART2 init/write path even when not genuinely in reverse. If any of the untested register assumptions were wrong and TXFF never cleared, this would hang the *entire* automatic boot chain silently and permanently with zero console output -- matching the observed symptom exactly (nothing prints, nothing boots, but the system isn't truly locked up since a manually-typed command afterward still works). **Fixed** (`linux-arkmicro` commit `5de2ed1d9`): bounded with the same timeout-loop pattern already used elsewhere in this file (`jpeghw`) -- times out and prints a warning instead of hanging. Not yet hardware-tested. This remains the leading explanation for the autoboot symptom specifically, now that the stale-env theory is ruled out.

**3. `/dev/mtd3` doesn't exist under the booted (bootmmc) kernel -- symptom of a much bigger, now-fixed problem, see §82.** The very end of `v23`'s log shows, from inside the booted shell: `Cannot access MTD device /dev/mtd3: No such file or directory` / `Error: environment not initialized` -- §79's new `(sleep 15 && fw_setenv bootcount 0) &` line in `rcS` failing every single boot. Chasing why `/dev/mtd3` doesn't exist turned out to be the wrong question entirely -- see §82.

## 82. Root cause of §81 items 2+3 found: U-Boot's environment was never actually persisted anywhere (`CONFIG_ENV_IS_NOWHERE`)

User corrected the §81 "stale saved `bootcmd`" theory directly: the SD card is a freshly built image every test, so any real error has to trace back to this repo's own files, not accumulated device-side state. That framing led straight to the actual root cause: `linux-arkmicro/u-boot/configs/ark1668_limcet_p305_defconfig` had `CONFIG_ENV_IS_NOWHERE=y`. Checked `env/nowhere.c` directly -- it implements only an `init` callback (marks the environment permanently invalid, always falls back to compiled defaults) with **no save or load function at all**. `saveenv`/`env_save()` had never been persisting anything, anywhere, on this board's build -- not to NAND, not to MMC, nowhere.

This is the real explanation for both remaining §81 items: `bootcount` only ever lived in RAM for the current boot session -- the moment an actual reset happens (the *only* scenario `bootcheck`/`bootlimit` exists to survive), the counter was wiped back to the compiled default (0), so it could never accumulate across repeated failures and never reach `bootlimit`. And `/etc/fw_env.config` pointing at `/dev/mtd3` was never going to work regardless of the missing-device-node issue -- there was no NAND-backed environment to read in the first place.

**Fixed** (`linux-arkmicro` commits `3faddaa57`, main repo `c85b095`/`595e0e3`): switched to `CONFIG_ENV_IS_IN_MMC`, storing the environment in a raw sector range on the SD card itself (`CONFIG_ENV_OFFSET=0x40000`, `CONFIG_SYS_MMC_ENV_DEV=0`) -- inside the ~1MiB gap before partition 1 that nothing else in this project's boot process uses (U-Boot's own binary is a regular FAT file inside partition 1, read via the SoC's Stepldr, not a raw-sector-resident bootloader; confirmed via `build_bootable_sdcard.sh`'s partition layout, `mklabel msdos` + partition 1 starting at 1MiB). Matches how the project already treats the SD card as the primary rewritable deployment medium, vs. NAND which stays deliberately untouched.

Two companion fixes to make this real end-to-end, not just on the U-Boot side:
- `build_bootable_sdcard.sh` now zeroes the pre-partition-1 region (`dd if=/dev/zero ... bs=1M count=1`) before partitioning -- without this, a physical SD card reused across test cycles (not the `--image`-file path, which already starts from a zeroed file) could carry a stale environment forward from an earlier build across a "fresh" rebuild, silently reintroducing the exact "state hidden outside the repo" problem this whole fix eliminates.
- `/etc/fw_env.config` (`firmware_source/mtd6_rootfs/etc/`) updated to point `fw_printenv`/`fw_setenv` at `/dev/mmcblk0`, offset `0x40000` (matching the new U-Boot config exactly), replacing the stale `/dev/mtd3` NAND entry -- needed for `rcS`'s bootcount-clear line to actually function.

**Real build-pipeline gotcha hit and fixed during this same session**: `build_uboot.sh` only re-applies the defconfig if `.config` doesn't already exist. The first rebuild attempt after editing the defconfig silently kept using a stale cached `.config` from earlier in the session -- `CONFIG_ENV_IS_NOWHERE=1` still showed up in the merged `u-boot.cfg` despite the source change. Caught by explicitly checking the compiled config output rather than assuming the edit took effect; resolved with `./build_uboot.sh --defconfig`. Worth remembering for any future defconfig-level change in this repo.

Builds and injects ARK header clean, `CONFIG_ENV_IS_IN_MMC` confirmed present in both `.config` and the merged `u-boot.cfg`. **Not yet hardware-tested.** Real regression test once hardware access is available: reproduce a failing `bootusb` a few times in a row (matches §79's original test plan) and confirm `bootcount` now genuinely climbs *across* real resets and the fallback to `nandboot` triggers after `bootlimit` (3) -- that's the actual thing this fix restores.

## 83. §82's `CONFIG_ENV_IS_IN_MMC` fix REVERTED same day -- hangs the board completely, before "DRAM:" even prints

First hardware test of §82's fix: the board locked up completely, with **nothing printed after the U-Boot version banner line** -- not even `DRAM:`/`NAND:`/`MMC:`, which print successfully in every other boot log this project has, including ones captured the same day. User initially suspected the new MCU-notify/camera-preview code (§80, `carbackcamcheck`), which runs from `CONFIG_BOOTCOMMAND` -- checked and ruled out architecturally: `CONFIG_BOOTCOMMAND` only runs after the version banner, DRAM/NAND/MMC probing, and the bootlogo/autoboot countdown all print, which is *after* the point where this hang happens. Not that code.

**Root cause isolated, mechanism not fully understood.** Found `.config.old` (a real, ~week-old backup U-Boot's own build process had already saved from before this session's `--defconfig` runs) and diffed it against the freshly-regenerated `.config`: the **only** functional difference was `CONFIG_ENV_IS_NOWHERE=y` -> `CONFIG_ENV_IS_IN_MMC=y`. This cleanly ruled out defconfig drift (the project's known recurring bug class, [[project_kernel_defconfig_drift]]) as a confounding factor -- nothing else changed. Checked `env/mmc.c`'s actual code paths for anything that could plausibly hang this early (`blk_get_device_by_str()`, used by an OF_CONTROL DT-property lookup path) but found that specific path is gated behind a `u-boot,mmc-env-partition` DT property this board's DTS doesn't set, so it should be skipped entirely -- the exact mechanism causing the hang wasn't pinned down before deciding to revert rather than keep experimenting against a fully locked, un-debuggable board.
...
**Claimed safe at the time -- WRONG, corrected in §86.** With the environment reverted to `CONFIG_ENV_IS_NOWHERE` (§83), checked whether `do_bootcheck()`/`do_bootnand()`'s unconditional `env_save()` calls could themselves cause any new problem. Traced `env/env.c`'s `env_save()` and concluded it loops over registered backends, skips any without a `.save` callback, and cleanly terminates -- no I/O, no print, no hang. **This was an incomplete trace that missed a real infinite loop; see §86, found the same day once `bootnand` was actually observed hanging immediately after this exact call.**
...
**HARDWARE-CONFIRMED same day**: user flashed this build and reports U-Boot now boots normally and `bootmmc` works. Confirms the §83 revert (`CONFIG_ENV_IS_IN_MMC` -> back to `CONFIG_ENV_IS_NOWHERE`) and this section's `carbackcamcheck` removal together restored a working automatic boot -- the board is no longer locking up. This is the first successful hardware boot since §81's initial test round. Still open/not yet re-confirmed on this exact build: the watchdog-arm-before-jump behavior in `boot_from_block_dev()`/`do_bootnand()` (§79/§81) and manual verification of `itu656`/`carbackcamcheck` (§80/84) -- none of these were specifically exercised by this basic "does it boot" check.

## 85. `bootnand` still hangs (watchdog never fires) -- root-caused to `WDT_PSR` requiring a power-of-two value, confirmed via live A/B hardware testing

`bootmmc`/`bootusb` confirmed working (§84), but `bootnand` still hangs -- no reset, waited a full 30s past the arm point with nothing happening. Separately confirmed plain `reset` at the U-Boot prompt *does* work (board reboots immediately). Both use the exact same `ark_wdt_arm()` primitive, so the difference had to be in the parameters: `reset` uses 100ms (fits the 16-bit countdown register with prescaler=1, no division needed); `do_bootnand()`'s new 30s arm needs a much larger prescaler (~290, computed via plain `div_round_up()`).

**Isolated via direct live register pokes at the U-Boot prompt** (`md`/`mw` on `0xe4b00000`-range, the `WDT_CR`/`WDT_PSR`/`WDT_LDR` registers), bypassing all C code:
- `WTPSR=2` (power of two): board reset at the expected ~200ms.
- `WTPSR=0x100`/256 (power of two): board reset at the expected ~27s.
- `WTPSR=290` (not a power of two -- `290 = 0b100100010`, three bits set): never fired, even after 30+ seconds.

**Root cause, empirically confirmed across three real hardware tests**: `WDT_PSR` requires a power-of-two value -- most likely a one-hot/shift-selector field internally (each bit position N meaning "divide by 2^N"), not a literal linear divisor. No datasheet exists to confirm the exact mechanism, but the A/B result is unambiguous. This bug was invisible until this session: `reset_cpu()`'s original 100ms/2000ms uses never needed a prescaler at all, so this path was never exercised before today's longer boot-supervision timeouts needed it.

**Fixed** (`linux-arkmicro` commit `a02f3fbb6`): `ark_wdt_arm()` now rounds the computed prescaler up to the next power of two instead of using the raw `div_round_up()` result. **Same bug found and fixed in the kernel's own `ark_wdt.c` driver** (commit `8d5c6aed8`) -- identical math pattern, meaning the kernel watchdog's 15-second default timeout (wired up earlier today, §78) would very likely have silently never fired either; not yet independently hardware-tested for the kernel side specifically.

## 86. The REAL reason `bootnand` hangs: `env_save()` infinite loop, not (only) the prescaler bug

User reported the actual furthest point reached is right after `do_bootnand()`'s own printf -- `nandboot`'s *own* first `echo` never appears at all. This is a narrower, earlier hang than anything downstream in the `nandboot` env script (`switchecc`, NAND reads, etc.) -- it's inside `do_bootnand()` itself, between the printf and `run_command("run nandboot", 0)`.

Traced `env/env.c`'s `env_save()` far more carefully this time (correcting §85's now-superseded "confirmed safe no-op" claim from earlier the same day). `env_get_location()`'s `ENVOP_SAVE` case unconditionally returns `gd->env_load_location` **regardless of `prio`** -- unlike `ENVOP_LOAD`/`ENVOP_INIT`, which bound `prio` against `ARRAY_SIZE(env_locations)` and correctly return `ENVL_UNKNOWN` once exhausted. Since `CONFIG_ENV_IS_NOWHERE`'s `ENVL_NOWHERE` is a real, valid, always-found location (just one whose driver has no `.save`), `env_driver_lookup(ENVOP_SAVE, N)` returns the exact same driver for every `N`. `env_save()`'s own loop (`for (prio = 0; (drv = env_driver_lookup(ENVOP_SAVE, prio)); prio++)`) re-resolves that same driver every iteration, sees `drv->save == NULL`, does `continue`, and repeats -- **a genuine infinite loop**, no way to ever reach `ENVL_UNKNOWN` and terminate.

This fully explains the observed hang: `do_bootnand()`'s order is `printf()` -> `env_set_ulong()` -> `env_save()` -> `ark_wdt_arm()` -> `run_command("run nandboot")`. `env_save()` hangs forever, so **`ark_wdt_arm()` is never even reached** -- the watchdog was never armed at all, which is *also* why §85's prescaler bug alone couldn't explain "waited 30s, nothing happened" (there was nothing armed to fire, independent of whether the prescaler was correct). `do_bootcheck()` (called from the automatic `CONFIG_BOOTCOMMAND` chain) has the identical bug -- plausibly a contributing/alternate explanation for earlier "autoboot never proceeds past the countdown" symptoms (§81), independent of the `carbackcamcheck`/UART busy-wait issue found and fixed the same day.

**Fixed** (`linux-arkmicro` commit `15ee42826`): added the same `prio` bounds check to `ENVOP_SAVE` that `LOAD`/`INIT` already have in `env/env.c` -- a framework-level fix, not board-specific, since this landmine could be hit by any future caller of `env_save()`/`saveenv` under a backend with no `.save` implementation, not just this project's own board code.

Both this fix and §85's prescaler fix are now in the same build. Builds clean. **Not yet hardware-tested** -- next test should confirm `do_bootnand()` no longer hangs at all (getting at least as far as `nandboot`'s own first `echo`), and separately confirm whether the watchdog now actually fires and recovers if `nandboot` still hangs somewhere further downstream (the `switchecc`/NAND-read theory raised in §84 remains untested).

## 87. HARDWARE-CONFIRMED: `bootmmc`, `bootusb`, and `bootnand` all boot successfully -- §85/86's fixes resolved it

User confirmed on real hardware: `bootnand` now fires automatically after the countdown and loads successfully (no hang, no manual power-cycle needed); `bootmmc` and `bootusb` both load successfully too. This closes out the entire chain of issues opened by this session's boot-supervision work (§79 through §86) -- the `env_save()` infinite loop (§86) was the actual blocker for `bootnand`/`bootcheck`, and the `WDT_PSR` power-of-two prescaler fix (§85) means the watchdog-arm safety net is now confirmed sound in principle (though not yet separately stress-tested against a real induced hang, since `bootnand` isn't hanging at all anymore -- nothing to catch).

**Current known-good state, all hardware-confirmed working together:**
- `env/env.c`: `env_save()` bounds-check fix (`15ee42826`)
- `linux-arkmicro`/`u-boot`: `ark_wdt_arm()` power-of-two prescaler fix (`a02f3fbb6`), same fix in the kernel's `ark_wdt.c` (`8d5c6aed8`, kernel side still not independently stress-tested)
- `CONFIG_BOOTCOMMAND`: `carbackcamcheck` removed from the automatic path (manual-only, unverified register sequence, see §84)
- `CONFIG_ENV_IS_NOWHERE` (env persistence still reverted/open, §83) -- `bootcount`/`bootlimit` logic runs without hanging now, but still can't persist across a real reset; `/etc/fw_env.config`'s entry and the `rcS` bootcount-clear line are both intentionally quieted/commented pending a proper re-attempt at persistent storage.

**Still open, not yet done**: a real induced-failure test of the watchdog-arm safety net specifically (e.g. temporarily corrupt `zImage` again, per §79's original test plan) to confirm it actually recovers a genuine hang now that both blocking bugs are fixed -- today's testing confirmed the *normal* boot paths all work, not that the safety net catches an *abnormal* one. §83's `CONFIG_ENV_IS_IN_MMC` hang remains a separate, still-unexplained issue -- it locked up *before* `DRAM:` even printed, well before board code ever gets a chance to call `env_save()` explicitly, so it's not the same bug as §86's fix; any re-attempt still needs its own investigation, ideally with live serial/JTAG debug access.

## 88. Manual `itu656`/`carbackcamcheck` verification (§84's required Phase 0) -- `carbackcamcheck` clean, `itu656` confirmed genuinely broken, not just untested

User ran both manual commands at the U-Boot prompt, per §84's requirement to verify them by hand before ever re-adding `carbackcamcheck` to the automatic boot path.

- **`carbackcamcheck`**: works cleanly -- correctly prints the GPIO 5 (reverse-gear) status, no hang, no unexpected side effects. The GPIO-read/gating logic (reused from the already-established `do_backcarcheck()`) and the bounded UART timeout fix (§81) both hold up on real hardware.
- **`itu656`**: does something real to the hardware -- prints its NTSC-configured message -- but the result is the screen flickering randomly, not a clean camera picture. **This confirms a genuine bug, not just "never tested"** -- the register sequence is doing *something*, just not the right thing.

This validates the "lowest confidence" warning already written into `ark1668_debug_cmds.c`'s file header for this exact command. Leading suspect, already flagged in that same file's comments: stock's `FUN_0006e9f0` block (a screen/resolution-dependent step between the ITU656 enable calls) was deliberately left unported during the original RE work, with an explicit note that "if the picture looks wrong, that block is the first place to look" -- flickering is a plausible symptom of missing timing coordination between the ITU656 bypass and the already-running RGB panel display.

**Net result**: `carbackcamcheck` is safe but currently pointless without `itu656` actually producing a correct picture. Both remain manual-only (not in `CONFIG_BOOTCOMMAND`) -- correctly so, this confirms that decision (§84) was the right call. Further work on the camera preview feature needs either resolving `FUN_0006e9f0`'s omitted logic (more disassembly of stock's real binary) or accepting this as a known-broken, deprioritized feature for now -- not blocking anything else in the boot chain.

## 89. `FUN_0006e9f0`'s omitted logic decoded and ported -- root cause of §88's flickering

Disassembled stock's real `FUN_0006e9f0` in full (the outer function that calls both blocks already in `ark_itu656_camera_bypass_enable()`) to recover the previously-omitted logic between them. Found it configures `VIDEO_LAYER2` -- a layer the ported code never touched at all, meaning the ITU656 capture data had nowhere correctly configured to land. That's the concrete explanation for §88's flickering.

**Decoded and cross-validated, not guessed**: the register offsets (`LCD_BASE+0x32c/0x330/0x354/0x4`) were checked against this project's OWN independently-written kernel LCDC driver (`drivers/video/fbdev/arkmicro/ark1668_lcdc_funcs.c`, the already hardware-confirmed Android Auto video scaler-bypass fix) -- real cross-validation, since that code was written completely separately and uses the exact same registers/bit positions for `VIDEO_LAYER2`:
- `rLCD_VIDEO2_WIN_SIZE` (0x32c): stock's literal `0x1202d0` decodes as `720x288` via the exact packing convention `ark1668_lcdc_set_video_win_size()` uses -- the camera's native source window, board-independent, reused verbatim.
- `rLCD_VIDEO2_SIZE` (0x330): the active panel's own width/height (stock reads this from a runtime struct at boot; hardcoded here to this board's confirmed `800x480`, matching how the existing `NTSC_*` constants are also fixed board-specific values).
- `rLCD_VIDEO2_SCALE_CTL` (0x354): stock's literal `0x80` = bit 7 only, "YUV format" per that same kernel driver's own documented bit meaning for this exact register -- board-independent, reused verbatim.
- `rLCD_CONTROL` (0x4) read-modify-write: sets bit 6 (**independently confirmed as `VIDEO_LAYER2`'s enable bit** by `ark1668_lcdc_set_video_en()` in that same kernel file, `offset=6` for `VIDEO_LAYER2`), clears bits 7 and 9 (meaning not independently confirmed, replicated verbatim rather than reinterpreted).

Also found and corrected a stale attribution in the existing code comment while doing this: what was labeled "stock `FUN_0006e9d8` / `FUN_0006e82c(1)`" as one combined block was actually two separate stock function calls with the newly-found `VIDEO2` config genuinely sitting *between* them in stock's real execution order (`FUN_0006e9d8` first, then the `VIDEO2` block, then `FUN_0006e82c(1)`) -- re-disassembled `FUN_0006e82c` directly to confirm it really is exactly the three `ITU656_BASE` lines already in the ported code, just mis-attributed in an earlier session's comment. Re-ordered the code to match stock's real sequence.

Stock's own runtime branch here (between this `rLCD_CONTROL` path and an alternate `rLCD_TV_CONTROL` path for TV/composite output) is hardcoded to the `rLCD_CONTROL` path -- this board's screen type is confirmed RGB (`ScreenType:0` in every boot log), not TV/composite.

**Fixed** (`linux-arkmicro` commit `e64d76d48`). Builds clean. **Not yet hardware-tested** -- next step is re-running `itu656` manually at the prompt and confirming a real picture appears instead of flickering.

## 90. U-Boot does not distinguish factory vs. aftermarket camera mode -- confirmed by both static analysis and user's real-world observation

User asked directly whether U-Boot reads any setting to tell factory vs. aftermarket reverse-camera mode apart. Checked stock's real U-Boot binary for any "factory"/"aftermarket" strings or logic -- none found. The only camera-related setting present anywhere is `carback_camera_mode`, which (based on how it gates the ITU656 bypass) is a simple on/off switch for whether U-Boot's own camera preview runs at all, not a mode selector.

This is consistent with the existing "factory" camera finding above (this section, "factory camera mode confirmed working via a hardware-level bypass") -- the factory/aftermarket decision is made entirely outside U-Boot, at the MCU/relay level. U-Boot's `carbackcamcheck`/`itu656` attempt to run identically regardless of which mode the vehicle is actually configured for, gated only by GPIO5 (reverse gear) and that one on/off env var.

**User confirmed real-world evidence, same conversation**: on real stock hardware, the aftermarket overlay is sometimes visible momentarily on screen before the factory camera takes over -- direct confirmation that the MCU's relay/mux does NOT fully electrically isolate the SoC's own video output when the vehicle is set to factory mode. There's a real transition window where the SoC's video briefly shows before the relay switches over. This resolves the "isolation vs conflict" open question raised when this was first investigated -- it's a genuine race/transition, not a clean handoff, and matches exactly what would be expected now that U-Boot's own camera preview code (§80/85/89) has no factory-mode awareness and will always attempt to activate on reverse gear regardless of the vehicle's actual camera-type setting.

## 91. `reversingtrack` (parking-guide overlay data) never loaded on `bootmmc`/`bootusb` -- fixed

Follow-up question while discussing the camera preview work: does U-Boot load the reverse-camera guideline overlay data from NAND? Checked directly -- `nand read 0xfd00000 reversingtrack` (loading the `RSTK`-magic'd overlay data the kernel's `track_paint_init()` checks for) exists **only** inside the `nandboot` env script. `boot_from_block_dev()` -- the shared function behind both `bootmmc` and `bootusb`, the two paths this whole session has focused on getting working -- never loaded it at all.

Practical effect on those paths: the fixed physical address (`0xfd00000`) was never written by anything in the boot chain, so it'd contain leftover DRAM content on a cold boot. The kernel's magic check would almost certainly fail, degrading gracefully (a `"reservingtrack check failed!"` alert, not a crash) but silently leaving the guideline overlay permanently unavailable on `bootmmc`/`bootusb` -- only `nandboot` ever had it working.

**Fixed** (`linux-arkmicro` commit `de8a63329`, main repo `8141d69`): rather than keep the raw NAND read, moved the source to the SD card's FAT partition instead -- `firmware_source/mtd10_reversingtrack/reversingtrack` (already the canonical source for the existing `redirect_mtd_data`/`/nanddata/` Linux-side symlink mechanism) is now also copied onto `p1` as `reversingtrack.raw` by `build_bootable_sdcard.sh`, and `boot_from_block_dev()` loads it via `fatload mmc 0:1 0xfd00000 reversingtrack.raw` right after the kernel/DTB loads succeed -- same "always `mmc 0:1` regardless of `iface`" convention `display_bootlogo_file()` already uses for its own assets, so even a `bootusb` test boot (kernel/DTB from the USB stick) still pulls this from the SD card. Not fatal if missing -- the kernel's own graceful degradation means a warning is enough here, doesn't block boot.

Both build clean. **Not yet hardware-tested** -- next step is confirming the guideline overlay actually appears on a `bootmmc`/`bootusb` boot with the reverse camera engaged (previously only ever confirmed working via `nandboot`).

## 92. Reviewed `docs/logs/new uboot new kernel baseline v24_260801.txt` -- reconfirms §87's boot-supervision fix, itu656/reversingtrack fixes not yet in this build, one new unrelated kernel probe-order bug found

User provided a fresh 1682-line hardware capture. Build timestamps (U-Boot `Jul 31 2026 - 21:34:18`, kernel `#156 Fri Jul 31 21:25:11`) place this build right after §86's `env_save()` fix (`15ee42826`, committed 21:32:02) but **before** both §89's itu656 VIDEO2 fix (`e64d76d48`, 22:50:18) and §91's reversingtrack SD-card fix (`de8a63329`, 23:11:16) -- so this log is really a second confirmation of §87, not a test of §89/§91. Neither `itu656`/`carbackcamcheck` nor a `reversingtrack.raw` fatload appear anywhere in the log (expected -- not built yet).

**Reconfirms §87, two boot cycles captured:**
- `bootmmc` boots clean both times, full driver probe, no hangs.
- `bootcheck` correctly prints `bootcount=1 bootlimit=3` ahead of `bootusb`.
- `bootusb` correctly fails over to `nandboot` when no USB storage device is present (`0 Storage Device(s) found` -- no stick was plugged in for this test), and `nandboot` boots the **real stock Toyota firmware** from NAND end-to-end (`MSNCoreApp version: V3.21.09.0219`, familiar stock boot strings) -- confirms NAND is still untouched/original and the full fallback chain works, not just our own build's paths.

**New finding, unrelated to this session's U-Boot work -- a kernel-side driver probe-ordering bug**, visible for the first time only because this capture includes a full uncropped `dmesg`:
```
[    0.760986] get mirror type=0
[    0.760999] carback_first_enter g_carback null error
```
`rn6752_probe()` (ITU656 sensor driver) calls the exported `carback_first_enter()` (`drivers/soc/arkmicro/ark-carback.c:84`) as part of its own init, but `ark-carback`'s own platform driver hasn't probed yet at that point in boot order, so its file-static `g_carback` is still `NULL` -- the function just prints the error and returns. Practical effect: the "check the reverse-gear detect GPIO and call `dvr_enter_carback()` if already in reverse at boot" logic silently never runs, on every single boot. Not caused by anything in this session -- just newly visible. Flagged as an open item for a future session, not investigated further yet (a fix would need either deferred-probe ordering between the two drivers, or moving the call out of `rn6752_probe()` into `ark-carback`'s own probe).

**Still pending, unchanged**: a build made *after* 23:11 on 2026-07-31 needs to be flashed and tested to actually exercise §89's itu656 fix and §91's reversingtrack fix -- this log doesn't cover either.

## 93. Real regression found+fixed: stock UI red/blue color swap after manually testing itu656, caused by leftover VIDEO_LAYER2 state

User reported the stock UI's colors (red/blue swapped) after this session's U-Boot changes. Traced it down with the user's confirmation of the repro sequence: they ran `itu656` (or `carbackcamcheck` while in reverse) manually at the prompt to verify §89's VIDEO2 fix, then continued straight on to `nandboot`/the stock UI in the *same* U-Boot session, without a power cycle in between.

Root cause: `ark_itu656_camera_bypass_enable()` (§89) turns `VIDEO_LAYER2` on (`rLCD_CONTROL` bit 6) as a YUV-format layer, but nothing anywhere in the port ever turned it back off. Ruled out it being caused by an *automatic* code path first -- `ark_itu656_camera_bypass_enable()` is not called by `nandboot`'s `backcarcheck;` (that's the older, separate `do_backcarcheck()`, which only touches the ITU656 capture-INPUT block at 0xe0800000, a different register range) or by anything else in `CONFIG_BOOTCOMMAND` -- `ark_carback_camera_check()`/`carbackcamcheck` remains manual-only, per §84. Also confirmed no bit overlap with the RGB-order field (`rLCD_CONTROL` bits 18-20) that would explain it via a clean boot alone. This left "stale leftover hardware state from a prior manual command in the same session" as the only mechanism -- confirmed by the user.

**Fixed** (`linux-arkmicro` commit `76882bfe7`): added `ark_itu656_camera_bypass_disable()` (clears `rLCD_CONTROL` bit 6 only -- deliberately leaves bits 7/9 alone, their meaning still unconfirmed per §89), called from `do_backcarcheck()`'s existing not-in-reverse branch (`rITU656IN_IMR = 0`) alongside it. `backcarcheck` runs unconditionally on every boot path (`nandboot` and friends), so this is the one place guaranteed to run before Linux starts regardless of what was tested manually beforehand. Builds clean. **Not yet hardware-tested** -- next step is repeating the exact repro (manual `itu656`, then `nandboot` same session) and confirming the stock UI's colors are correct now.

**Follow-up question, same conversation**: does this fix still apply if the rootfs loads while the camera is genuinely active (real reverse gear)? No, by design -- the disable call only sits in `do_backcarcheck()`'s not-in-reverse branch. If `in_reverse` is true, `VIDEO_LAYER2` is deliberately left alone, since in that case it's the real live camera preview, not stale state -- disabling it there would kill the feature this whole port exists for. Separately flagged as still-open, untouched by this fix: whether the kernel's own camera/carback driver cleanly takes ownership of `VIDEO_LAYER2` from U-Boot's raw bypass during a genuine in-reverse handoff, or produces its own transition glitch -- same category as the already-noted unconfirmed bits 7/9 meaning, not investigated this session.

## 94. Real regression found+fixed: occasional U-Boot hang after the boot countdown, stuck mid-fatload with no recovery short of an SD reseat

User reported an intermittent hang: countdown finishes, a `fatload` "bytes read" line prints, then nothing -- boot command effectively never runs. Only recovered by removing the SD card, letting the board boot to stock (`nandboot`, no card present), then reinserting and retrying -- not a plain reset/power-cycle.

Investigated `boot_from_block_dev()` (the shared function behind `bootmmc`/`bootusb`) directly rather than guessing blind: it runs a sequence of 3 `fatload`s (kernel zImage, DTB, and -- new this session, §91 -- `reversingtrack.raw`, which prints nothing at all on success) plus a DTB patch, and only arms the hardware watchdog (`ark_wdt_arm()`) at the very end, immediately before the final `bootz` jump. That whole fatload sequence -- including the brand-new reversingtrack read -- had **zero watchdog protection**, so any hang anywhere in it (marginal SD card timing, controller getting wedged) just sits forever, exactly matching the "needs a manual SD reseat" recovery ritual reported. Same class of bug, same fix already applied once this session to `do_bootnand()` (commit `6f550e353`, "arm watchdog before do_bootnand() too").

Notably, this couldn't be pinned to the reversingtrack read specifically (user couldn't confirm which "bytes read" line was last), but the fix addresses the whole class regardless of which exact read stalls.

**Fixed** (`linux-arkmicro` commit `39563e170`): moved `ark_wdt_arm(KERNEL_HANDOFF_WDT_MS)` from right before `bootz` to the very top of `boot_from_block_dev()`, before any of the fatloads run. Still a large safe margin (20s) against a false trigger on a legitimately-successful boot -- every log shows the whole fatload+bootz+kernel-probe sequence finishing in well under 5s combined. Builds clean. **Not yet hardware-tested** -- next step is reproducing the original hang (if possible) and confirming the board now auto-resets back to U-Boot instead of sitting stuck.

**Important caveat surfaced by a follow-up question, same conversation, before this fix was even flashed**: user reported that (on the *old*, unfixed build) even a full ignition-off/wait-30s/ignition-on power cycle doesn't clear the hang on the next boot attempt -- only physically removing the SD card, booting to stock via `nandboot` with no card present, then reinserting and retrying actually recovers it. Checked for a software-controlled SD card power rail (`vmmc`/regulator/power-GPIO tied to `mmc0`) anywhere in U-Boot or the DTS -- **none exists**; the card's VDD appears hardwired always-on. This means:
- Neither the watchdog-triggered SoC reset from this fix, nor the physical reset button, can power-cycle the *SD card itself* -- only removing it (breaks the electrical connection outright) or a board power-off long enough to fully discharge whatever's holding that rail up can do that.
- If the true root cause is the SD card's own onboard controller getting logically wedged (a known real-world failure mode on marginal/aging cards under repeated reads, not necessarily a board/software bug), this fix converts the symptom from "silent permanent hang" into a "visible 20s reset-and-retry loop" -- a real improvement and directly observable -- but may not fully eliminate needing a physical reseat if the card itself is genuinely stuck each retry.
- No second SD card was available to test whether this is card-specific. **Open, unresolved**: whether this is a genuine U-Boot/board timing bug or a marginal-SD-card hardware issue exposed by more total reads per boot (this session added a 3rd mandatory read, reversingtrack.raw, on top of the existing kernel+DTB reads). Swapping to a different physical SD card, if one becomes available, is the most direct way to tell these apart.

## 95. §93's fix (VIDEO_LAYER2 disable) did NOT resolve the color swap -- real root cause found: a stale, previously-reverted arkdata.ini value

After flashing the §93/§94 build and confirming `backcarcheck` ran its not-in-reverse branch (`gpio5=1, in_reverse=0`, so `ark_itu656_camera_bypass_disable()` executed) on a genuine cold power-on boot (`ARK1680 SoC Booter` from scratch -- no possible leftover cross-session state), user confirmed the stock UI's colors were still swapped. This ruled out §93's entire theory -- `VIDEO_LAYER2` was never the cause.

Went looking for what's actually different between U-Boot's own display config and what the stock UI (MsnCoreApp, running on the real 3.4 kernel booted via `nandboot`) uses, since both should agree on `RgbMode` for colors to come out right. Found two genuinely independent `arkdata.ini` sources:
- U-Boot's own copy (`sd_bootable/arkdata.ini`, loaded via `fatload mmc 0:1 ... arkdata.ini` at every boot) -- `RgbMode=5` (RGB), consistently applied to the LCDC hardware registers on every boot path including `disconfig 0` (part of the `nandboot` script, right before jumping to the stock kernel).
- The rootfs's own copy, read by MsnCoreApp itself at Linux userspace boot (`firmware_source/mtd6_rootfs/msnprofile/arkdata.ini` and `.../arkdata/arkdata_0.ini`, matching this board's `screen_id=0`) -- found at **`RgbMode=0`** (BGR).

Traced the history: commit `db55426` (2026-07-23 21:30) correctly set both rootfs files to `RgbMode=5`, matching U-Boot. Commit `264e95e`, only 2 minutes later in the same session, reverted them back to `0` while claiming to move the fix into `firmware_overlay/msnprofile/arkdata.ini` instead. Checked where `firmware_overlay/` actually gets applied -- **only `build_bootable_sdcard.sh`** references it anywhere in the repo. Nothing that builds the NAND rootfs (`build_update.sh`'s "Build rootfs image" item, `build_rootfs.sh`) ever applies it. So the revert-to-0 half of that swap landed, but the "put the fix somewhere else instead" half never actually took effect for the NAND path -- a genuine, silent regression sitting in the tree since 2026-07-23, three days before this session even started.

Net effect: U-Boot configures the LCDC hardware for RGB(5) on every boot path, but the stock UI's own arkdata.ini has told it BGR(0) since 2026-07-23 -- a real mismatch between what the hardware is wired/configured for and what MsnCoreApp assumes, which is exactly what shows up as an inverted red/blue swap.

**Fixed** (main repo commit `e3261f2`): restored `RgbMode=5` in `firmware_source/mtd6_rootfs/msnprofile/arkdata.ini` and `.../arkdata/arkdata_0.ini` only -- the other ~92 vehicle-model-variant `.ini` files in that same directory are untouched stock originals for different hardware configs, not used by this board, and were left alone (confirmed via `grep`: all 94 files with a `RgbMode` field were already `=0`, so this isn't an isolated one-off value, it's every file's original stock default -- only the two this board actually reads had ever been deliberately changed and then un-changed).

**Not yet hardware-tested** -- this fix lives in the rootfs *source* tree, so it needs an actual rootfs rebuild+reflash (`build_update.sh`'s rootfs build item, default OFF) to reach the live NAND, not just a U-Boot/SD-card rebuild. §93's `VIDEO_LAYER2` disable fix stays in as a real, separate, still-valid fix for its own leftover-state bug -- just not the cause of this particular symptom.

**Follow-up pushback from the user, same conversation: "why were the colors correct before we applied our recent changes?"** Checked the actual dates rather than assume -- the `arkdata.ini` mismatch has existed continuously since **2026-07-24 12:06** (`sd_bootable/arkdata.ini`'s last RgbMode edit, commit `7f47294`), a full week before this session started, and neither `arkdata.ini` copy was touched by anything in this session before this fix. What *did* change this session: `nandboot` only became the default automatic fallback path on 2026-07-29 (checklist context, see [[project_bootstock_intermittent_nand_state]]) -- before that it wasn't a routinely-exercised path. User confirmed directly: had not specifically checked/confirmed the stock UI's colors before this session. So this was a pre-existing, unnoticed bug that only became visible once `nandboot`/the stock UI started getting exercised regularly as part of this session's boot-supervision work -- not a regression caused by anything changed this session.

## 96. §95's fix was itself wrong -- real fix found and applied: revert U-Boot's own arkdata.ini RgbMode 5->0, not the rootfs copy

User pushed back again, explicitly declining to reflash NAND (real risk of bricking the device) and hypothesizing directly: "it must be something we're doing in u-boot such as loading arkdata.ini." That pushback was correct and led to finding the real mechanism, superseding §95 entirely.

**§95's fix didn't address anything real.** Grepped the entire stock rootfs dump (every binary and init script in `firmware_dumps/Prado firmware dump/mtd6_rootfs`) for any reference to `arkdata.ini` or `RgbMode` by name -- found nothing. Stock's own userspace never reads that file at all, so correcting `firmware_source/mtd6_rootfs/msnprofile/arkdata.ini` (commit `e3261f2`) was a fix for a file nothing consumes.

**The real mechanism**: `ark_display_init()` -- called on every boot including `nandboot`'s `disconfig 0`, which is what configures the layer the real stock kernel's display driver actually renders through (OSD2) -- sets two separate things from two separate sources:
- `rLCD_OSD2_CTL = 0x002320ff`, a **hardcoded, fixed value**, completely independent of `arkdata.ini`. Checked its provenance in `docs/historical/HANDOFF_nand_ecc_uboot_vs_kernel.md` section 5 -- captured via a **live byte-for-byte register comparison against genuine working stock hardware**, about as solid as evidence gets in this project. Not the bug.
- `rLCD_CONTROL`'s global `rgb_mode` field, driven by `arkdata.ini`'s `RgbMode` value (currently `5`, applied via `arkdata_apply_lcd_timing()`/`ark_display_initialize_port()`). This one was **never** live-verified against stock -- traced its git history and found it was toggled `0`->`5`->`0`->`5` across four commits in about 16 hours on 2026-07-23/24 (`14d2ce6`, `01c643a`, `db55426` [rootfs only], `264e95e` [rootfs only], `7f47294`) during what looks like an unresolved trial-and-error session, landing on `5` with no confirmation it was ever correct.

Checked the one truly authoritative source available: the **real, completely untouched factory NAND dump's own `arkdata.ini`** (`firmware_dumps/Prado firmware dump/mtd4_arkdata/extracted/arkdata.ini`) -- straight from a genuine unit, never edited by this project. It has **`RgbMode=0`**.

**Fixed** (main repo commit `aabd934`): reverted `RgbMode` from `5` back to `0` in the two files U-Boot's own boot path actually reads/could read -- `sd_bootable/arkdata.ini` (the one `build_bootable_sdcard.sh` actually copies to p1, confirmed via `grep`) and `firmware_source/mtd4_arkdata/arkdata.ini` (kept consistent for the NAND "Display Config" partition path, `build_update.sh`'s `arkdata` item). Also updated a stale comment in `build_bootable_sdcard.sh` that incorrectly asserted both copies "already have RgbMode=5 applied."

**Testable via an SD-card-only rebuild -- no NAND flash involved**, matching the user's explicit constraint. User is rebuilding/flashing the SD image on their own side. §95's rootfs-side change (commit `e3261f2`) is now known to be a no-op, not reverted (harmless, just unused) but should not be trusted as "the fix" going forward -- see [[project_stock_ui_rgbmode_mismatch]] for the corrected writeup.

**HARDWARE-CONFIRMED WORKING (2026-08-01)**: user tested and confirmed the stock UI's colors are now correct. Closes out the whole §93->§96 investigation -- the real cause was always U-Boot's own `arkdata.ini` `RgbMode` value, not `VIDEO_LAYER2` state (§93, wrong) or the rootfs's copy (§95, wrong -- nothing reads it).

## 97. Third instance of the watchdog-armed-too-late bug: `board_late_init()` itself had zero protection, matching a real in-vehicle symptom

User reported ongoing random hangs, in-vehicle (no serial console available to pin down the exact location): stuck on the splash screen, never reaching `bootusb` or `nandboot` at all. This is a third occurrence of the exact bug class already fixed twice this session (`do_bootnand()` §87/checklist history, `boot_from_block_dev()` §94) -- watchdog armed too late to protect the thing that actually hangs.

Traced the earliest point `ark_show_bootlogo()` is called from: `board_late_init()` (`ark1668.c`), a standard U-Boot hook that runs once, before the autoboot countdown and `CONFIG_BOOTCOMMAND` even execute. **Nothing before this session's two existing arm points protected any of**: `ark_show_bootlogo()`'s own 2 fatloads (arkdata.ini, bootlogo.raw), the autoboot countdown, `CONFIG_BOOTCOMMAND`'s own `uEnv.txt` fatload, or `bootcheck` -- all genuinely earlier than either `do_bootnand()` or `boot_from_block_dev()`, and exactly matching "stuck after the splash shows, never reaches USB or NAND."

**Fixed** (`linux-arkmicro` commit `544999c75`): arm the watchdog (`BOARD_LATE_INIT_WDT_MS`, 20s) as the very first line of `board_late_init()`, before `ark_show_bootlogo()` runs -- the earliest point in the whole automatic-boot sequence it's possible to arm from. 20s comfortably covers this whole window (every boot log shows it finishing in a couple of seconds); the already-fixed `bootusb`/`nandboot` paths re-arm with their own fresh timeouts before this one could expire on a legitimately slow but successful boot. Builds clean. **Not yet hardware-tested** -- and given no serial console is available in-vehicle, "fixed" here will likely only be confirmable indirectly (does the random-hang complaint stop recurring over time), not from a single clean log.

With this fix, all three fatload/fatload-adjacent windows in the entire automatic boot sequence (`board_late_init()` → `bootusb`/`nandboot` → their own kernel/DTB/reversingtrack fatloads) now have watchdog coverage, no known gaps left in the automatic path.

## 98. Real bug found+fixed: random/garbage colors flashing on screen between bootlogo image swaps

User reported a second, distinct display symptom: random colors briefly appearing on screen between each bootlogo image change (not the stock-UI-specific §96 swap, this is visible during U-Boot's own splash-cycling before any kernel even loads).

Root cause: `ark_display_init()` unconditionally enables `OSD2` on every boot, pointing at `0x0be00000` -- and at that early point in boot, nothing has ever written to that address, so it's raw, uninitialized DRAM. `display_bootlogo_file()` already correctly disables `OSD1` while overwriting its own buffer during each swap (fixed once before, commit `0b336e63f`, to stop tearing) -- but that means whatever sits *underneath* OSD1 is what's actually visible during that gap, and OSD2 (enabled, garbage) is sitting right there. This fork's own boot-stage-status feature (cycling through `bootlogo.raw` → `bootlogo_usb.raw`/`bootlogo_sd.raw`/`bootlogo_nand.raw` as the boot progresses) is what exercises this repeatedly -- real stock likely only ever shows a single splash image and never triggers this particular race, consistent with `OSD2_CTL=0x002320ff` itself being confirmed correct via a live stock comparison (§96) while this specific interaction was never a stock behavior to compare against in the first place.

**Fixed** (`linux-arkmicro` commit `3b8a08b73`): zero OSD2's framebuffer (`memset((void *)0x0be00000, 0, 800*480*4)`) once, right where `ark_display_init()` first configures and enables it -- before any bootlogo swap can ever expose it. Doesn't change stock's eventual behavior at all: the real stock kernel driver overwrites this same buffer with real content once it starts using the layer, this fix only controls what's visible in the meantime, during this fork's own extra U-Boot-level splash swaps. Builds clean.

**HARDWARE-CONFIRMED WORKING (2026-08-01)**: user tested this build and confirmed the random/garbage colors between bootlogo swaps are gone.

## 99. `nandboot` now reads the real NAND kernel partition first, not an SD-staged copy

User asked directly: can `bootnand` point at the actual kernel on NAND instead of the `zImage_stock` copy staged on the SD card? Checked why the SD copy was ever tried first -- it wasn't a deliberate reliability choice, it was leftover from an earlier separate `bootstockkernel` command (`fatload mmc 0:1 ... zImage_stock`) that got merged straight into `nandboot` on 2026-07-23 (commit `0e8528f48`) and happened to keep its own SD-first order. No known issue with the real NAND `kernel` partition itself -- `switchecc 2`, already run just above this in the same script, exists specifically because NAND ECC reads were fixed and validated earlier in this project. Philosophically backwards too: `bootnand` exists as the last-resort path meant to work independent of the SD card, so preferring an SD file there undercut its own purpose.

**Fixed** (`linux-arkmicro` commit `d5630e6c3`): swapped the order in the `nandboot` env script -- `nand read ${kerneladdr} kernel` now runs first, falling back to the SD `zImage_stock` fatload only if that NAND read fails (user's explicit choice: keep the SD fallback for redundancy, not NAND-only). Builds clean. **Not yet hardware-tested.**

## 100a. Real regression from §97's fix: the early watchdog arm fired even while manually parked at the U-Boot prompt

User attached a serial console specifically to catch the hang §97 was meant to fix, but reported "couldn't find the problem" -- and separately noticed the watchdog was still armed and resetting the board even at the "Hit space to stop autoboot" prompt, after manually stopping autoboot to investigate. These are almost certainly the same issue: §97's fix armed the watchdog unconditionally at the very top of `board_late_init()`, with no awareness of whether the user then stops autoboot to look around -- the 20s timer just kept counting down regardless and reset the board mid-investigation, actively preventing the debugging session it was meant to make possible.

**Fixed** (`linux-arkmicro` commit `a1aa160a6`): removed the unconditional arm from `board_late_init()`. Added a new `wdtarm [timeout_ms]` shell command (`ark1668_boot_cmds.c`, wraps `ark_wdt_arm()`, defaults to 20000ms) and made it the very first thing `CONFIG_BOOTCOMMAND` runs, before the `uEnv.txt` fatload. Since `CONFIG_BOOTCOMMAND` only executes if autoboot isn't interrupted, this means the watchdog now arms only when autoboot genuinely proceeds unattended -- pressing space to stop autoboot means that command string never runs at all, so nothing to disarm, no reset while sitting at the prompt.

**Trade-off, accepted deliberately**: this reopens a narrower gap than §97 closed -- `ark_show_bootlogo()`'s own 2 fatloads (arkdata.ini, bootlogo.raw), which run inside `board_late_init()` *before* `CONFIG_BOOTCOMMAND` ever executes, are unprotected again. That gap was always speculative coverage (never confirmed as an actual hang location), whereas the prompt-reset behavior was a confirmed, actively-obstructive regression -- fixing the confirmed problem took priority. If a hang is ever pinned down specifically inside `ark_show_bootlogo()`'s fatloads, this will need a different approach (e.g. a way to detect/skip disarm on manual abort, rather than just never arming that early).

Builds clean. **Not yet hardware-tested.**

## 100b. `itu656` re-tested (bench, no camera connected): black screen, not flickering -- consistent with the VIDEO2 fix working

User ran `itu656` again on the log-confirmed build and reported a black screen, not the flickering seen before the §89 fix. Confirmed no physical camera is connected to the ITU656 input for this bench test -- black is the expected, correct result with no video signal present, not a bug in the register sequence.

Worth noting as a positive signal: the *same* no-camera bench condition previously showed flickering (§88, before the VIDEO_LAYER2 fix), not black -- an unterminated/floating video input reading as noise. Now resolving to a stable, clean black instead is consistent with the fix having actually worked (a properly configured, stable hardware state), even though it doesn't prove a real picture displays correctly. **Still open**: a genuine end-to-end confirmation needs an actual camera signal connected to the ITU656 input -- not yet done.

**Follow-up question, same conversation: does stock's real U-Boot draw the parking-guideline overlay itself, before the kernel boots?** User believed it did, based on how fast the overlay appears to show up on real stock hardware. Checked directly against stock's actual U-Boot binary disassembly (`/tmp/stock_uboot_disasm.txt`) rather than inferring -- found `RSTK`/`reversingtrack`/`reversingtrack_size` strings present, but traced their real code reference (`0x31634`) to a large partition-name table (the exact same `{name, offset, size, description}` shape this project's own `build_update.sh` already replicates), alongside a `"*****Now update reversingtrack ......"` string -- this is stock's own **firmware-update tool** listing `reversingtrack` as a flashable NAND partition name, not a routine that reads and draws the overlay data. **No overlay-drawing code exists anywhere in stock's real U-Boot binary.** The guideline overlay is confirmed purely a Linux kernel-side feature (`track_paint_init()`) in stock too, same architecture already ported here -- what looks like an instant overlay on real stock hardware is almost certainly just stock's kernel initializing that driver quickly after boot, not anything U-Boot itself does. Settles this question; no code change needed.

## 100. Autoboot countdown reduced 9s -> 3s

User requested a shorter default countdown. `CONFIG_BOOTDELAY=9` -> `3` in `ark1668_limcet_p305_defconfig` (`linux-arkmicro` commit `293badaf9`). Rebuilt with `--defconfig` to force it into the live `.config` (per this project's established defconfig-caching gotcha -- a plain `./build_uboot.sh` without that flag silently keeps whatever `.config` already has), confirmed `CONFIG_BOOTDELAY=3` present in the built config before flashing. Builds clean. **Not yet hardware-tested.**

## 100d. Real code-level cause found for the "boots when a terminal is connected" failure mode: musb USB driver aborts on Ctrl-C-like noise

Building on §100's serial-cable observation: user did an A/B test that ruled out pure floating-RX electrical noise as the whole explanation. With the serial-to-USB adapter connected to the board but not actively read by an open terminal, **stock's own real U-Boot, run on the identical physical hardware at the same time, always boots fine -- only this fork's compiled U-Boot fails** (bootusb, with apparent downstream effects on nandboot in the same cycle). Since the electrical environment was identical for both, the differentiator has to be code, not wiring.

Found it: `drivers/usb/musb-new/musb_uboot.c`'s `musb_submit_urb()` wait loop -- the actual path used for essentially every USB transaction during `bootusb`'s enumeration on this board (musb-hdrc, not the unrelated `r8a66597-hcd.c` driver, which is the only other `ctrlc()` caller among USB host drivers) -- calls `ctrlc()` on every iteration:
```c
do {
    if (ctrlc())
        return -EIO;
...
```
`ctrlc()` (`common/console.c`) polls console RX for a literal `0x03` byte. A console RX line that isn't being actively driven by an open terminal can pick up noise that occasionally decodes as a real byte -- and if it lands on `0x03` during this high-frequency loop (runs continuously throughout the whole USB enumeration sequence), the operation aborts right there with `-EIO`. Stock's much older (~2012.10) U-Boot's own USB driver very likely predates this check or doesn't share it -- a genuine code-generation difference, not something explainable by register config drift the way most of this project's other findings have been.

**Fixed** (`linux-arkmicro` commit `d6f11afab`): new `noctrlc [1]` command wraps U-Boot's own existing `disable_ctrlc()`/`ctrlc_disabled` (`common/console.c`, already used elsewhere in U-Boot for exactly this "don't let stray input abort something critical" need) -- deliberately not patching the shared `musb_uboot.c` driver directly, since that would also remove legitimate manual Ctrl-C abort capability at the interactive prompt, not just during automatic boot. Wired into `CONFIG_BOOTCOMMAND` the same way as `wdtarm` (§100a): `noctrlc;` right after `wdtarm;` at the very start, `noctrlc 1;` (re-enable) at the very end as a safety net in case boot fails and control falls through to the interactive prompt anyway. Never touched at all if autoboot is stopped manually -- that command string doesn't run in that case. Builds clean. **Not yet hardware-tested.**

## 100e. §100d's musb ctrlc() fix was based on the wrong driver -- corrected, and the hang re-localized to a much narrower window

Follow-up investigation the same day. Checked which musb driver is actually compiled into this board's `.config`: `CONFIG_USB_MUSB_HCD=y` (the legacy `drivers/usb/musb/musb_hcd.c`), with `CONFIG_USB_MUSB_HOST`/`CONFIG_USB_MUSB_GADGET` (which gate `drivers/usb/musb-new/musb_uboot.c`, where §100d's `ctrlc()` check was found) both explicitly unset. **§100d's `noctrlc` fix targets code that isn't even built into this board's U-Boot.** Confirmed no `ctrlc()`/`tstc()`/`getc()` call exists anywhere in the actually-compiled `musb_hcd.c`, nor in `dw_mmc.c`, generic `mmc.c`, or `fat.c` -- checked all four, zero hits. There is no code path anywhere in the automatic boot sequence's I/O stack that behaves differently based on console RX state. The `noctrlc` command/wiring is harmless and left in place (real, reusable capability), but should not be treated as having fixed anything confirmed.

Also confirmed stock's real binary contains the exact same distinctive USB debug strings (`"usb_lowlevel_init %d us timeout"` etc.) as this project's `musb_hcd.c` -- same driver lineage, ruling out "different U-Boot generation's USB driver" as an explanation too.

**Real re-localization, from the user's own observation**: during the hang, *neither* `bootusb`'s nor `nandboot`'s boot-stage splash (`bootlogo_usb.raw`/`bootlogo_nand.raw`) ever appears -- meaning execution never reaches `bootlogofile bootlogo_usb.raw` inside `if bootcheck; then`. That narrows the hang to somewhere in the few lines before it: `wdtarm` (trivial, can't hang) → `noctrlc` (trivial) → the `uEnv.txt` fatload (real SD I/O) → `bootcheck` (env-var arithmetic + an `env_save()` call already fixed for its one known infinite-loop bug, §86). The `uEnv.txt` fatload is the one genuine I/O operation left in that window, and the first SD transaction of the whole automatic sequence.

**Working theory, not yet confirmed**: more likely electrical (marginal SD signal integrity that an actively-connected/read serial adapter happens to stabilize via extra grounding) than a specific software bug -- this fork does noticeably more total SD I/O per boot than stock likely does (`uEnv.txt`, `arkdata.ini`, multiple `bootlogo_*.raw` swaps, kernel, DTB, `reversingtrack`), so more reads means more exposure to the same underlying marginal condition, not necessarily a different one.

**Diagnostic step taken** (`linux-arkmicro` commit `1fe8e03d5`): removed the `uEnv.txt` fatload from `CONFIG_BOOTCOMMAND` entirely. Confirmed safe to drop -- `sd_bootable/uEnv.txt`'s content (`bootcmd=`/`bootargs=` override) is functionally inert regardless of whether it loads: nothing downstream depends on it, and `CONFIG_ENV_IS_NOWHERE` means an `env import` here wouldn't persist to affect even a future boot's `bootcmd` default. Removing it costs nothing except one fewer SD transaction per boot -- directly tests whether that specific operation was the hang point, or at least reduces total I/O exposure if the cause is cumulative/electrical. Still manually reachable at the prompt if ever needed. Builds clean. **Not yet hardware-tested** -- next step is repeating the same "adapter connected but not actively read" condition that originally exposed this and seeing whether the loop still happens.

## 100f. Bounded a second, genuinely-unbounded blocking read found while chasing §100e -- confirmed not the cause, but fixed anyway

While asking about the console-related hang, user asked to "remove the second serial port too so it matches stock." Investigated two distinct candidates before asking which was meant: (1) `arch/arm/mach-arkmicro/armv7/mcu_serial.c`, a vendor-provided UART2 driver gated by `CONFIG_MCU_SERIAL_PORT=2`, confirmed uncalled anywhere in this board's own code (`mcu_serial_init()`'s only caller in the whole tree is a different board, `arkn141-sqwf.c`); (2) this project's own UART2/MCU-notify code for the reverse-camera preview (`ark_uart2_init_once()`, manual-only). User chose a more surgical option: don't remove either, just fix the specific blocking function.

`mcu_serial_getc()` had a genuinely unbounded loop -- `while(1) { ... if (ch == -EAGAIN) { WATCHDOG_RESET(); continue; } ... }`, no real timeout, only a generic watchdog-feed macro standing between it and hanging forever if nothing ever arrives on that UART.

**Fixed** (`linux-arkmicro` commit `856ad67eb`): bounded with a 1-second `get_timer()`-based timeout, same pattern as this project's other unbounded-blocking-read fixes (`ark_uart2_putc()`, the original `bootnand` `env_save()` hang).

**Verified this has zero effect on the actual shipped binary, not just "probably unreachable"**: `nm u-boot | grep mcu_serial` returns nothing at all -- the entire `mcu_serial.c` object file is linker-eliminated since nothing reachable in this board's boot path calls into it. Binary size was byte-identical before/after the fix, confirming this. Definitively rules this out as a cause of the ongoing hang investigation (§100e) -- kept as a real source-level fix (defensive, in case anything is ever wired up to call it later) rather than reverted, since it costs nothing and closes a genuine latent bug.

## 100g. Systematic audit for other unbounded blocking loops -- found and fixed a real, high-exposure one: console output itself

User asked directly whether there were other blocking functions in the code, given the pattern of finding these (§100f, plus the original bootnand `env_save()` hang and `ark_uart2_putc()`) had been productive. Audited every `while(1)`/`for(;;)` and every bare register-polling `while` loop across the board-specific and mach-specific source (`board/arkmicro/ark1668_limcet_p305/*.c`, `arch/arm/mach-arkmicro/armv7/*.c`), using `nm u-boot | grep <symbol>` to verify reachability rather than just reasoning about callers -- the same technique that caught §100f's fix being dead code.

**Confirmed safe, no action needed**: `ark1668_arkdata_ini.c`/`ark1668_lcd_console.c`'s string-parsing loops (bounded by buffer/string length); `timer.c`'s `__udelay()` (the actual hardware-timer-bounded delay primitive); `ark1668_debug_cmds.c`'s `gpiotest` Ctrl-C-to-stop loops (intentional manual diagnostic commands, already an improvement over stock's own version which had no escape at all); `ark1668_boot_cmds.c`'s NAND/BCH FSM-idle wait (already bounded from an earlier session); `reset.c`'s `while(1)` in `reset_cpu()` (correct by design -- the watchdog it just armed will fire shortly).

**Confirmed dead code, real bugs but zero effect on the shipped binary** (verified via `nm`, not just "probably unreachable"): `display_updatelogo()` and the two frame-interrupt waits it was the only caller of (`ark_disp_wait_lcd_frame_int()`/`ark_disp_wait_tvenc_frame_int()`, `ark1668_lcd.c`) -- `display_updatelogo()` is `static` and genuinely never called (the one reference found was a comment, not a call), confirmed absent from the linked binary. `early_printf.c` -- entirely unused; this board's `CONFIG_DEBUG_UART_PL011=y` actually resolves to the generic `serial_pl01x.c`'s own debug-uart functions instead, confirmed via `nm` showing `printch`/`debug_uart_init` present but nothing from `early_printf.c`.

**Real, high-exposure bug found and fixed** (`linux-arkmicro` commit `57433b814`): `drivers/serial/serial_pl01x.c`'s `pl01x_serial_putc()` -- confirmed via `nm` to be the actual linked console putc (`CONFIG_DM_SERIAL` is unset, so the legacy non-DM path is what's active, not the newer DM variant later in the same file) -- had a completely unbounded wait on TX-FIFO-full, with **no timeout at all**. This is the function behind every single `printf()` call throughout the entire automatic boot sequence -- by far the highest-exposure unbounded loop found in this whole audit, unlike the two dead-code ones above. `pl01x_serial_setbrg()` (console baud-rate init, runs once) had the same gap, fixed for consistency. Both bounded with a `get_timer()`-based 100ms timeout, same pattern as this project's other fixes for this bug class.

Binary size grew after this fix (`nm` confirms `pl01x_serial_putc` present, unlike the two dead-code fixes where size was byte-identical before/after) -- genuine, reachable change to the shipped binary, not just source hygiene. Builds clean.

## 100h. CRITICAL: §100g's fix immediately crashed real hardware -- get_timer() called too early in boot, corrected to a plain iteration counter

User flashed and tested §100g's build. Result: a data abort / reset loop, immediately, every single boot -- crashing right after `DRAM: 180 MiB` prints, before `NAND:`/`MMC:` even show. Console output itself was garbled (`pc : ]crr09 e0R` instead of clean hex), consistent with the CPU state being corrupted at the moment of the fault.

Root cause: `get_timer()`, used for §100g's timeout bound, depends on the timer subsystem already being initialized. Console/serial init runs very early in `board_init_f` -- evidently before that dependency is reliably satisfied on this board. This exactly matches the crash location (right where early console init runs, before anything else has had a chance to print).

**Fixed** (`linux-arkmicro` commit `3ec5e300f`): replaced `get_timer()` with a plain bounded iteration counter (`int timeout = 100000; while (... && --timeout > 0);`) in `pl01x_serial_putc()`/`pl01x_serial_setbrg()` (the real fix from §100g) and, for consistency, in `mcu_serial_putc()`/`mcu_serial_getc()` (§100f's already-dead-code fixes, which used the same pattern). This matches how every other early-boot bounded loop in this codebase already works -- e.g. the NAND/BCH FSM-idle wait uses a plain `timeout--` counter, never `get_timer()` -- no dependency on any subsystem being ready. Also bounded `setbrg()`'s second wait (`FR_BUSY`), which was unbounded in the original upstream driver already, not something introduced this session, while already in that function.

**Lesson for this project going forward**: `get_timer()` is not safe to use in code that might run before the timer subsystem is confirmed initialized -- prefer a plain iteration counter for anything in or near early console/board init, reserve `get_timer()` for code confirmed to run later (post-relocation boot commands, U-Boot shell commands, etc.) where this project's existing `get_timer()`-based fixes (`ark_wdt_arm()` callers, `boot_from_block_dev()`, etc.) are known-safe. Builds clean.

**HARDWARE-CONFIRMED: the crash is fixed** -- user reflashed and confirmed a clean boot. Additionally, user reports it booted cleanly with **no serial monitor attached at all** -- exactly the condition that originally exposed the intermittent hang this whole investigation (§100d onward) started from. Not yet enough runs to call the underlying intermittent-hang investigation itself closed (per §100c's established pattern for this project -- an intermittent bug needs an extended clean stretch, not one good boot), but this is the first real positive signal that `pl01x_serial_putc()`'s unbounded TX-FIFO wait (§100g) may have been the actual root cause all along, ahead of the earlier `uEnv.txt` fatload removal (§100e) or the two dead-code fixes (§100f). User is continuing to test/monitor over time.

## 100i. Real re-localization from a live repro: hang confirmed to be upstream of CONFIG_BOOTCOMMAND, in ark_show_bootlogo()'s own fatloads or the countdown -- root cause found and safely closed

Continued monitoring turned up a genuine hang. User connected a terminal *mid-hang* (screen stuck at the bootlogo splash, no console visibility until connecting), pressed Ctrl-C, and it successfully printed `<INTERRUPTED>` and dropped to the prompt.

This is decisive evidence, not just a guess: `CONFIG_BOOTCOMMAND`'s very first two commands are `wdtarm;` then `noctrlc;` (§100a/§100d) -- `noctrlc` globally disables Ctrl-C detection for the rest of that command string. If `noctrlc` had already run, Ctrl-C could not have interrupted anything. Since it clearly did, `CONFIG_BOOTCOMMAND` had not started executing yet -- the hang is upstream of it, in `board_late_init()`'s own `ark_show_bootlogo()` (2 fatloads: `arkdata.ini`, `bootlogo.raw`) or the autoboot countdown itself. This is exactly the narrower gap §100a's fix deliberately reopened when it moved watchdog-arming out of `board_late_init()` and into `CONFIG_BOOTCOMMAND` -- flagged explicitly in that section's own comment as "never confirmed as an actual hang location," and now it has been.

**Root-caused why §100a's removal was necessary in the first place, rather than just re-adding the arm blindly**: `common/autoboot.c`'s `__abortboot()` calls `ark_watchdog_stop()` (confirmed to control the *same* hardware watchdog `ark_wdt_arm()` does -- both use the `WDT_BASE=0xe4b00000` register block) but **only** for the narrow case of space being pressed before the countdown loop even starts. Any abort during the actual countdown loop -- any key, or Ctrl-C specifically, which is exactly what the user's repro used -- sets `abort=1` but never calls `ark_watchdog_stop()`. A pre-existing gap in the vendor's own code, not something introduced this session, but the reason simply re-arming early would have reintroduced §100a's exact regression.

**Fixed** (`linux-arkmicro` commit `b1f831d37`), two parts:
1. `common/autoboot.c`: moved `ark_watchdog_stop()` from right after the immediate-keypress check (narrow) to unconditionally cover any abort, right before `__abortboot()` returns.
2. `ark1668.c`: restored `ark_wdt_arm(BOARD_LATE_INIT_WDT_MS)` at the top of `board_late_init()`, now safe -- any successful autoboot interrupt (space or Ctrl-C) disarms the watchdog automatically before the user ever reaches the interactive prompt, closing the loop §100a's original comment left open.

`CONFIG_AUTOBOOT_KEYED` confirmed unset, so only the `__abortboot()` variant touched here is actually active -- no need to also patch the password-based variant. Builds clean. **Not yet hardware-tested** -- this closes the last known gap in the automatic-boot watchdog coverage (bootlogo fatloads + countdown + `CONFIG_BOOTCOMMAND`'s own sequence, all covered now) while, for the first time, also making manual investigation via Ctrl-C during that same window verifiably safe.

**First test result, same build (`Aug 02 2026 - 13:56:49`)**: one full clean cycle -- manual `reset` command (warm reset, not a power cycle) worked cleanly, then a second boot went `bootusb` (correctly failed, no stick) -> `nandboot` -> real NAND kernel read (§99) -> stock kernel booting, no hangs, no crashes, `RgbMode=0` confirmed throughout. Good sign, but this was a warm reset via the `reset` command, not the cold power-cycle condition some of the original hang reports were specifically tied to -- user is planning a genuine cold boot test next. Not calling this closed until that's in, consistent with how every other data point in this intermittent-hang investigation has been treated.

**Cold boot testing, same build**: user reports "holding up on cold boot so far" -- the more rigorous test that was specifically pending is now underway and clean so far. Still not calling this closed -- the original bug was random/intermittent by nature, and this project's own established pattern (see the earlier bootstock intermittent hang, [[project_bootstock_intermittent_nand_state]]) is to wait for an extended clean stretch across many cold boots before treating an intermittent hang as actually resolved, not a handful of good runs. Watching.

## 100j. Hang recurred on a cold boot ("back to its old habits") -- user's own key observation: only ever happens on cold boots, never warm resets

User hit the hang again, connected a terminal mid-hang, pressed Ctrl-C -- same `<INTERRUPTED>` result as §100i, confirming the hang location is unchanged (still upstream of `CONFIG_BOOTCOMMAND`). Asked whether the watchdog (re-armed in §100i, 20s window) had actually fired before Ctrl-C was pressed -- user wasn't certain of the exact timing on the first check, then reported "I think it was greater than 20secs" on a retest, suggesting the watchdog may genuinely not be firing in this window, not just "didn't wait long enough." Checked `ark_wdt_arm()`'s math for this exact 20000ms value -- already hardware-confirmed correct elsewhere in this build (`boot_from_block_dev()`'s `KERNEL_HANDOFF_WDT_MS` uses the identical value), so unlikely to be a computation bug specific to this call site; `ark_get_apb_clock()` also unlikely to hit the same "too early" class of bug `get_timer()` did (§100h), since `board_late_init()` runs late enough in `init_sequence_r[]` that clocks/serial/MMC are already up and printing by that point. Root cause of *why the watchdog isn't firing* remains unconfirmed -- not enough certainty in the timing observation to chase further yet.

**User's own key new data point, unprompted**: "it only happens on cold boots so must be something unique to that." This is a strong, well-reasoned lead independent of the watchdog-firing question. A warm reset (`reset` command, or a watchdog-triggered reset) re-runs the *entire* boot ROM/Stepldr/U-Boot init chain identically to a cold boot at the code level -- the one thing genuinely different is physical power state. This board has no software-controlled SD card power rail (confirmed earlier, §94's investigation) -- so on a warm reset the card's VDD never actually drops, it stays continuously powered and already-initialized throughout, while on a true cold boot the card is *also* powering up from zero alongside everything else and needs real time to physically stabilize before reliably responding to commands. Checked upstream `mmc_go_idle()` (`drivers/mmc/mmc.c`) -- it already has a short, fixed `udelay(1000)`/`udelay(2000)` around `CMD0`, but that's identical on every boot, no extra cushion specifically for a card still settling from a genuine cold power-up. This board's own `board_mmc_init()`/`ark_dwmci_init()`/`dwmci_reset()` (`ark1668.c`) likewise has no card-settle delay of its own, only a 100us controller-reset toggle delay.

**Fixed** (`linux-arkmicro` commit `d49b0a46c`): added `mdelay(200)` in `ark_dwmci_init()`, right after `dwmci_reset()` and before `add_dwmci()` lets the generic MMC framework start issuing commands to the card. A fixed cost on every boot (negligible against total boot time), giving a genuinely cold-powering-up card real settle time it didn't have before, while costing nothing meaningful on a warm reset where the card was never in that state to begin with. Builds clean.

**First test result**: user flashed and cold-booted the new build -- clean, no hang. User is continuing to monitor rather than treating this as confirmed -- consistent with how every other step in this intermittent-hang investigation has been calibrated (a clean run is a good sign, not proof; the original bug was random enough that it needs an extended stretch before calling it closed). A live stock-U-Boot timing comparison (via `bootstock`/`boothybrid` chainload) was considered as a way to validate the `mdelay(200)` choice against genuine stock behavior, but deferred for now -- static disassembly search wasn't practical for finding an unlabeled numeric delay constant (no stock U-Boot boot log with real timestamps exists in this project's log archive either), and the fix stands on its own reasoning regardless of what stock specifically does. Revisit only if the settle delay doesn't fully resolve the cold-boot hang on its own.

User reports being unable to reproduce the original hang that motivated §97/§100a's watchdog work. **Not calling this closed** -- the bug was explicitly random/intermittent from the start, so a run of clean boots is a good sign, not proof. Several changes landed since the original report that could plausibly explain it, individually or combined: `wdtarm` now covers `CONFIG_BOOTCOMMAND`'s `uEnv.txt`/`bootcheck` window (§100a), `nandboot` reads the real NAND kernel first instead of an SD-staged copy (§99), and the autoboot countdown itself is shorter (3s vs 9s, §100 -- less time in a window where anything could go wrong). No way to attribute it to one specific fix from behavior alone. Treat as a watch item: revisit if it recurs during continued regular use, lean toward "resolved" only after an extended clean stretch -- same pattern this project already used for the earlier bootstock intermittent hang (see [[project_bootstock_intermittent_nand_state]]).

## 100k. New theory for a genuinely different symptom: cold-boot-only, console-cable-dependent stall AT the "Loaded U-boot"/countdown screen, not a hang inside `CONFIG_BOOTCOMMAND`

Distinct from §100i/§100j (which were hangs *upstream* of the countdown, in the bootlogo fatloads): user reported a fully cold boot getting stuck sitting at the `__abortboot()` prompt itself, only when no console was attached, only on a genuinely cold boot -- theorized undriven console UART RX picking up electrical noise during power-up, misread as a byte by the PL011 receiver.

**Confirmed against the actual code**: `common/autoboot.c`'s countdown loop only ever checked `if (tstc())` -- "is a byte pending," not what the byte was -- so any spurious byte, real keypress or not, set `abort=1` and dropped straight to the prompt. Matches the symptom exactly (sitting at an interactive prompt, not crashed) and the console-cable dependency (a real USB-serial adapter actively drives RX to idle-high the instant it's attached, eliminating the floating-pin noise susceptibility; nothing else does on this board -- no `bias-pull-up` anywhere in the kernel DTS or U-Boot pinmux code for this pin). Checked the real factory-dumped log (`docs/logs/archived/uboot original dumped log_260715.txt`) before assuming stock avoids this differently -- stock has the identical `tstc()`-driven "Press space key to stop autoboot" prompt, so this isn't a design gap stock sidesteps, just one that happens to matter more here given the longer 3s window.

**Fixed, three parts** (`linux-arkmicro` commits `84a77c2ad`, `ae1881eae`, `1f325ea75`; this repo's `sd_bootable`/`firmware_source/mtd4_arkdata` copies staged to match):
1. `__abortboot()` now requires the actual byte to be space or Ctrl-C (`0x03`) before treating it as a real abort -- any other byte is consumed and ignored, countdown continues. Prompt text also corrected to "Press spacebar to stop autoboot" to match.
2. A new `arkdata.ini` key, `[PRADO_PROJECT] BootInterrupt` (reusing the existing `arkdata_ini_get_int()` reader already used for LCD timing/RgbMode), lets an SD card disable the whole abort check -- no prompt, no console polling at all. Default flipped to **off** (`BootInterrupt=0`, or the key/file simply missing) -- closing this exposure by default was judged more valuable than console-abortability by default; set `BootInterrupt=1` to restore the interactive countdown for active development.
3. A matching single checkpoint added to `do_bootusb()`, right after `usb start` succeeds -- the earliest point a USB boot path specifically has anything to check against, since `__abortboot()` itself runs before `CONFIG_BOOTCOMMAND`/USB init. Same space/Ctrl-C gating, same `BootInterrupt` flag, deliberately only this one checkpoint (not polling through the fatload sequence, which would reopen the same unattended-noise exposure §100d's `noctrlc` fix exists to close for the USB driver's own Ctrl-C check). Calls `cli_loop()` directly on a real abort rather than returning an error, since a plain failure return would just fall through to `nandboot` per `CONFIG_BOOTCOMMAND`'s own chain, not reach the interactive prompt.

**First test result**: user flashed the new build, cold-booted from SD, booted cleanly through to NAND (no stall at the countdown screen). User is continuing to monitor and plans another cold boot test -- not calling this closed yet, same calibration as every other step in this cold-boot investigation (§100i/§100j): the original symptom was noise-dependent/probabilistic, so one clean run is a good sign, not proof.

**Follow-up, same day: bootusb checkpoint's missing time window, then a separate stuck-at-`DRAM:` regression, cause not confirmed, checkpoint reverted.** User reported `BootInterrupt=1` correctly reached the main countdown for `bootmmc`, but never dropped to prompt for `bootusb`. Root cause: `bootusb_check_abort()`'s `tstc()` check ran exactly once, instantaneously, right after `usb start` returned -- no polling window, no printed prompt, nothing a human could realistically time a keypress against. Fixed with a real 1s polling window and a visible "press spacebar..." countdown, matching `__abortboot()`'s own style (`linux-arkmicro` `a6b6bd798`).

**On the very next rebuild/reflash, user reported getting stuck at the `DRAM:` line with no further progress** -- no `<INTERRUPTED>`, no visible countdown, nothing after `DRAM:`. Checked whether anything from this session's changes could plausibly cause a hang this early: no. Everything touched in this whole `BootInterrupt`/abort-checkpoint effort (`__abortboot()`, `arkdata_ini_get_int()`, `bootusb_check_abort()`) runs during/after `board_init_r`, well after `DRAM:` prints during early `board_init_f` -- none of that code can execute before that point in the boot sequence. Also re-confirmed the earlier `get_timer()`-in-early-console fix (§100h) is still correctly in place (`serial_pl01x.c`/`mcu_serial.c` still use plain iteration counters), ruling out a recurrence of that specific prior bug. **Root cause of this stall genuinely not identified** -- could be a new instance of the same class of cold-boot hardware marginality already chased elsewhere in this file, or something unrelated entirely; not enough evidence to say.

**Per explicit request, the bootusb-specific abort checkpoint was reverted entirely** (`linux-arkmicro` `8cf5a811d`/`30b0e20a4`, cleanly reverts both the original checkpoint and its window fix) -- USB boot no longer has any abort-check of its own, only the main `__abortboot()` countdown (mmc-path, separately hardware-confirmed working) remains. **This revert is a precaution, not a confirmed fix** -- the DRAM-line stall's actual cause was never pinned down, so it's not established that removing this checkpoint actually resolves it, only that it removes the most recently-added code as the simplest variable to eliminate. Worth retesting cold boot on the reverted build; if the stall recurs, the checkpoint removal can be ruled out and the search needs to go earlier in the boot sequence (Stepldr/board_init_f/DRAM init itself).

**Test result on the reverted build**: user reports booting successfully again, and the `BootInterrupt`/space-Ctrl-C countdown fix on the `mmc` path is confirmed working. Consistent with (but not proof of) the checkpoint removal being the actual fix for the DRAM-line stall -- the stall's root cause was never directly identified, only correlated with reverting the most recently-added code. Not calling the DRAM-line stall itself closed on this one result, same calibration as every other cold-boot symptom in this file; the `BootInterrupt` countdown mechanism itself can be treated as working now that it's been exercised successfully on real hardware.

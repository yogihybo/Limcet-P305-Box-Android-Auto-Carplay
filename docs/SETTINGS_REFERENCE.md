...
| Key | Observed values | Meaning |
|-----|-----------------|---------|
| `ScreenType` | `1`, `3` | Panel interface type. Per `arkdata.ini` legend: `RGB565=1, RGB888=2, LVDS=4, VGA=8, CVBS=16, YPBPR=32, ITU656=64, ITU601=128`. `1`=RGB565 panel. **Not yet disassembly-confirmed** (2026-08-03 attempt: string exists in both `MsnCoreApp` and the fully-stripped, PIC-addressed `libMcuCenter.so`, but no consumption site was reached within budget) — whether the consuming code treats this as a real bitmask (matching the legend) or something narrower is still unknown. |
...
| `MirroringLinkType` | `1`, `2` | Screen‑mirroring protocol variant. **Checked, not found**: the string exists in `libMsnMirrLink.so`'s rodata (mixed in among UI dialog names) and in `libSetting.so` (inside what looks like a generic settings-editor key-label table, not a per-key branch site) — but `libMsnMirrLink.so` is fully stripped and no instruction cross-reference to the string was found in either binary. Likely read generically for UI display rather than gating distinct behavior, but not confirmed either way — still *(inferred)*. |

...
Background doc for [README §7.0 Custom U-Boot Boot Chain](../README.md#70-custom-u-boot-boot-chain-ark1668_limcet_p305).
...
All real values below are from `docs/8.1_PARTITION_LAYOUT.md` and `env/uboot-env.txt` (the live device's
actual, captured environment) — not guesses.

| Setting | `ark1668_defconfig` default | Limcet P306's real value | Why it matters |
|---|---|---|---|
| `CONFIG_ENV_OFFSET` | `0x160000` | `0x120000` | Must point at the Limcet P306's actual `U-boot-Env` partition start (`docs/8.1_PARTITION_LAYOUT.md`), not the reference board's offset. Same fix `docs/historical/SD_BOOT_PLAN.md` already identified before this source was actually located. |
...
3. **SPL size budget** — this defconfig produces an SPL (`CONFIG_SPL=y`) that must fit in the 128K
   `S-Loader` partition budget (`docs/8.1_PARTITION_LAYOUT.md`) if it's ever flashed there. Not a concern

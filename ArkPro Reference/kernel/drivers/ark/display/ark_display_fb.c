 ...
 */
static int ark_disp_fb_setcolreg(unsigned int regno,unsigned int red, unsigned int green, 
 ...
 *   return negative errno on error, or zero on success.
 */
static int ark_disp_fb_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	if (context->gui_tvout) 
		ark_disp_set_next_buf_start_addr(3, addr);
...
{
	struct ark_disp_obj_info *disp = info->par;
	struct ark_disp_context *context = container_of(disp, struct ark_disp_context, disp_obj[disp->id]);
	/*printk("cmd:%s %d: enter\n", __FUNCTION__, __LINE__);*/
	switch (cmd) {
		...
		case ARKFB_WAITFORVSYNC:
			...
			return 0;//ark_disp_wait_frame_sync_interrupt(disp);
		case ARKFB_UPDATE_WINDOW:
			{
				...
				if(copy_from_user(&input_arg, (void*)arg, sizeof(input_arg)))
					return -EFAULT;
				disp->writeback = 0;
				disp->show_tv = input_arg.show_tv;
				return ark_fb_update_window(disp, &input_arg);
			}
		case ARKFB_UPDATE_VIDEO_WINDOW:
			{
				struct arkfb_update_window input_arg;	
				unsigned long flags;
				if(copy_from_user(&input_arg, (void*)arg, sizeof(input_arg)))
					return -EFAULT;
				disp->writeback = 1;
				while (!list_empty(&disp->buf_info.indep_disp_list) ) {
					struct list_head *list = disp->buf_info.indep_disp_list.next;
				...
				return ark_video_update_window(disp, &input_arg);
			}
		...
		case ARKFB_GET_COLOR_KEY:
			{
				struct arkfb_color_key input_arg;
				ark_fb_get_color_key(disp, &input_arg);
		...
		case ARKFB_GET_BLEND:
			{
				struct arkfb_blend input_arg;

				ark_fb_get_blend(disp, &input_arg);
				if(copy_to_user((void*)arg, &input_arg, sizeof(input_arg)))
					return -EFAULT;

				break;
			}
		case ARKFB_SET_PRIORITY:
			{
				struct ark_osd_priority input_arg;
				if(copy_from_user(&input_arg, (void*)arg, sizeof(input_arg)))
					return -EFAULT;
				
				break;
			}
		case ARKFB_ENABLE: /*open lcd*/
			{ 
				int id = 0;
				ark_disp_set_osd_data_addr(0, context->disp_obj[id].buf_info.buf_pool_base_phys);
			...
			break;
		case ARKFB_SET_OSD_ALPHA:
			{
				struct ark_osd_alpha input_arg;
				...
				ark_disp_set_osd_alpha(input_arg.id, input_arg.alpha);			
				disp->layer_info.alpha = input_arg.alpha;
				break;
			}
		case ARKFB_GET_OSD_CFG :
			{
				struct ark_disp_vde_cfg_arg input_arg;
				if(copy_from_user(&input_arg, (void*)arg, sizeof(input_arg)))
					return -EFAULT;
				if(input_arg.layer_id > 4)
					return -EFAULT;
		...
		case ARKFB_SET_WINDOW_ADDR:
			{
				...
				if(copy_from_user(&input_arg, (void*)arg, sizeof(input_arg)))
					return -EFAULT;
				//printk("ark_set_window 0x%08x, 0x%08x, 0x%08x\n", input_arg.phy_addr, input_arg.phy_cbaddr, input_arg.phy_craddr);			
				disp->writeback = 0;
				return ark_fb_set_window_addr(disp, &input_arg);//ark_fb_set_window_addr(disp, &input_arg);			
			}

		case ARKFB_SET_VIDEO_WINDOW_ADDR:
			{
				struct arkfb_window_addr input_arg;

				if(copy_from_user(&input_arg, (void*)arg, sizeof(input_arg)))
					return -EFAULT;
				//printk("ark_set_video 0x%08x, 0x%08x, 0x%08x\n", input_arg.phy_addr, input_arg.phy_cbaddr, input_arg.phy_craddr);			
				disp->writeback = 1;
				return ark_fb_set_video_window_addr(disp, &input_arg);			
			}
		case ARKFB_GET_WINDOW_ADDR:
			{
				unsigned long lcd_video_addr;//rLCD_VIDEO_ADDR1;

				lcd_video_addr = rLCD_VIDEO_ADDR1;
				if(copy_to_user((void*)arg, &lcd_video_addr, sizeof(lcd_video_addr)))
					return -EFAULT;
			}
			break;

		case ARKFB_SET_WINDOW_POINT:
			{
				int id = disp->id;
				struct arkfb_update_window input_arg;
				...
				if (ARK_DISP_LAYER_IS_OSD(id)) {	
					ark_disp_set_osd_win_point(id, input_arg.win_x, input_arg.win_y);
				} else {
					id = ARK_DISP_GET_VIDEO_ID_FROM_LAYER_ID(id);
					ark_disp_set_video_win_point(id, input_arg.win_x, input_arg.win_y);
				}
				disp->layer_info.crop_xpos = input_arg.win_x;
				disp->layer_info.crop_ypos = input_arg.win_y;
			}
			break;
		...
		struct ark_disp_context *context)
{
	...
	fbd->fb_info.screen_base = (char __iomem *)buf->cur_buf_addr_virt;
#endif     

...
**⚠️ This document's primary analysis covers a different zImage than `docs/1.9_KERNEL_REFERENCE.md`.** See [Build Discrepancy](#build-discrepancy-vs-kernelmd) below — **§6 now cross-checks the `KERNEL.md` build (#383) directly and confirms the findings hold on both.**
...
**§9 cross-checks §§2–5 against real ASTRI vendor source** (ASTRI ArkPro reference BSP, see [`vendor_source/README.md`](../vendor_source/README.md) and §9) — confirms the `config.c` board-init device roster and the `CLCD_TIMING`/`CLKDIV1` register field names, and surfaces a third (unused-on-Limcet-P306) candidate mechanism for backcar detection.

**§10 works out the full pin-mux table** from the real `linux-arkmicro` devicetree source, filling in and correcting `hardware/ark1668-limcet-prado.dts`'s `pinctrl0` node — and finds that an earlier draft's `can0`/`can1` pin-mux entry was wrong: that capability only exists on the newer ARK1668E generation, not this chip.

---

## Build Discrepancy vs `1.9_KERNEL_REFERENCE.md`

Two different kernel builds exist in this repo's trees:

| | `firmware_dumps/Prado firmware dump/mtd5_kernel/extracted/zImage` (this doc's primary subject) | `firmware_source/mtd5_kernel/zImage` (`1.9_KERNEL_REFERENCE.md`, also verified in §6) |
|---|---|---|
...
These are **not the same kernel** — 30 build numbers and ~22 months apart. The "dump" tree is the raw MTD5 pull straight from a live physical unit; the "reconstructed" tree appears to carry a later build (from a firmware update package or a different source — see `docs/14.1_SOURCES.md`). Both target the same SoC/driver family (RN6752, Goodix-TS, ARK1680 identifiers all present in both), so the findings below likely transfer, but **any GPIO/register-offset claim here is only confirmed against build #353**, not #383. Worth re-running the same Ghidra pass against the `reconstructed` zImage to confirm no board-file changes crept in between builds.
...
**GPIO 95 = `PBANK_2`, pin 31.** Label `"apple_encpy_ic_rst"` = reset line for an Apple MFi/CarPlay authentication IC — consistent with this being a Toyota OEM head unit (`ProductId=Limcet-P306`, `HomeIconLabel=TOYOTA`, per `docs/14.1_SOURCES.md`) needing wired CarPlay support.
...
- ❌ **`arkdata` NAND partition (MTD4)** — checked `docs/1.7_DISPLAY_SUBSYSTEM.md`; this partition is confirmed to carry only LCD panel timing (resolution, `CLKDIV1`, `VBP`/`HBP`/`VSW`/`HSW`, touch-key config), nothing GPIO-related. Ruled out.
- ✅ **Likely candidate: userspace, via `/proc/ark_gpio`** — `docs/1.9_KERNEL_REFERENCE.md` already documents this debug/control interface exposed by the `ark_gpio` driver. A userspace daemon (`MsnCoreApp` or similar, reading `MsnProductInfo.ini`'s `ResourceName`/`McuType`/`ProductId` fields — see `docs/14.1_SOURCES.md`) most plausibly writes the camera/backup-camera GPIO numbers into the kernel at boot through this interface, based on which product variant is running. This is consistent with the whole project's established pattern of board variants being selected by userspace config rather than separate kernel builds.
...
Ran the identical extraction + Ghidra pipeline against `firmware_source/mtd5_kernel/zImage` (MD5 `78782daea...`, the build `docs/1.9_KERNEL_REFERENCE.md` documents) to check whether everything above still holds 22 months and 30 build numbers later.
...
**Found the factory/dispatcher:** `MCUAdapter::getAdapterInstance(MCUAdapter::McuType)` (exact demangled symbol name — no guessing needed) is a 30-case `switch` on the numeric `McuType` value, `case 1` through `case 0x1e`, each `operator_new`-ing and constructing the matching adapter class. This is the mechanism `docs/14.1_SOURCES.md`'s `McuType` field (16 for Holden, **6 for Limcet P306**) actually drives.
...
1. `docs/1.9_KERNEL_REFERENCE.md` already recorded that the `arktool` MCU-UART binary protocol carries a **`backcar enable/disable`** command.
...
## 9. Cross-check against real ASTRI vendor source
...
`mach-ark1680` kernel slice for its backlight/display kernel modules; see [`vendor_source/README.md`](../vendor_source/README.md)
for provenance and scope notes). This is a **generic reference BSP, not the Limcet P306's actual OEM board file** — it has
...
- **`ark1680_add_device_*()` roster in ASTRI `config.c`** registers platform devices under the same names found in the Limcet P306 binary's `FUN_8059f188` (§5's table): `gpio-ark`, `ark1680-uart`, `ark1680-nand`, `ark_i2s_dev`, `ark-display`, `ark-prescaler`, `ark-jpeg`, `ark-deinterlace`, `ark-itu656`, `ark-pwm`, `ark-wdt`, `pwm-backlight`. Real source confirming what was previously just a decompiled device-name string list.
...
Located and referenced the actual `linux-arkmicro` kernel devicetree source (`RD_Software/linux-arkmicro`,
see [`vendor_source/README.md`](../vendor_source/README.md) for provenance —
this is the same public BSP `docs/7.1_UBOOT_BUILD_GUIDE.md` uses for the U-Boot side). Its
...
against real source on both sides (`linux-arkmicro` U-Boot `board/arkmicro/ark1668/ark1668.c`'s
...
| BD37033 audio codec | `arkmicro,drv_bd37033` | `sound/soc/arkmicro/BD37033.c` | **DORMANT / UNPOPULATED I2C** (I2S DAC path confirmed active) | Live testing with `tools/bd37033-test` across all candidate I2C buses (`/dev/i2c-0`, `/dev/i2c-1`, direct bit-bang GPIO 9/121 and GPIO 3/2) and GPIO 34 standby states yielded no I2C ACKs at `0x40`/`0x41`. Confirms this board variant relies purely on the on-SoC Sigma-Delta DAC (`ark-sddac`) with analog bypass, matching stock's `SoundType=0` configuration. |
...
| Resistive touch (actually used) | `arkmicro,ark1680-ts` | `drivers/input/touchscreen/ark1680_ts.c` | PROBES OK; physical touch input inconclusive | boot log: `input: ark1680-ts as .../input0`, `ARK1680 resistive touchscreen registered, irq=20` — driver probes cleanly; whether physical touches actually register as input events on *this* kernel build is not confirmed by this evidence alone (see `1.8_ARK1680_TS_REVERSE_ENGINEERING.md`'s mixed findings, largely gathered on stock firmware, not this kernel) |
...
| I2S1 + DAC/ADC codecs | (ark1668 sound bindings) | `sound/soc/arkmicro/ark_i2s.c`, `ark1668-sddac-codec.c`, `ark1668-sdadc-codec.c`, `ark_dac_codec.c`, `ark_adc_codec.c` | **CONFIRMED** | dedicated working session/log (`new kernel audio working log v1.log`) plus the dai-link-order fix documented in `1.5_AUDIO_SUBSYSTEM_INVESTIGATION.md` |
...
| 82-84 | PBANK_2 18-20 | i2s1 sync/sadata/bclk (**active** — I2S/audio confirmed working) | `ark1668-pinctrl.dtsi:174-191`, `1.5_AUDIO_SUBSYSTEM_INVESTIGATION.md` |
...
- `ark1680_add_device_hsuart`'s device is named **`"ark1680-hsuart"`** — this
  ...
  used by `libMcuCenter.so`/`McuType=6`, see `1.3_MCU_ADAPTERS.md`). Its pad
...
evidence-level convention (see `1.3_MCU_ADAPTERS.md`'s header):** attempts to
...
upgraded to confirmed: `docs/1.4_WIRELESS_AND_INIT.md` independently
...
- `docs/1.1_HARDWARE_AND_SOC_REFERENCE.md` — plain-text block diagram of every
  peripheral/pin/GPIO in this doc, for a quick at-a-glance view. Keep both
  files in sync when either changes.
- `docs/1.7_DISPLAY_SUBSYSTEM.md` — the LCD/i2c-gpio-0 conflict.
- `docs/1.5_AUDIO_SUBSYSTEM_INVESTIGATION.md` — BD37033 investigation that
  led to all of this.
...
|     confirmed working -- docs/1.4_WIRELESS_AND_INIT.md
...
- 1.5_AUDIO_SUBSYSTEM_INVESTIGATION.md
- 1.8_ARK1680_TS_REVERSE_ENGINEERING.md
- 1.3_MCU_ADAPTERS.md             -- McuType=6 (BoxP300), /dev/ttyHS0 protocol
...
serial link `MsnCoreApp` opens (see `1.3_MCU_ADAPTERS.md`). Notably, the
...
`MCUAdapter_BoxP300` (already documented, `1.3_MCU_ADAPTERS.md`) -- its
...
`1.8_ARK1680_TS_REVERSE_ENGINEERING.md`), then symlinks `/tmp/touch_export`
...
`1.5_AUDIO_SUBSYSTEM_INVESTIGATION.md`) -- and since every `Sound_*`
...
[`MSN_APP_ARCHITECTURE.md`](MSN_APP_ARCHITECTURE.md) band 03) touch any GPIO? `objdump -T` +

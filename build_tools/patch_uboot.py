#!/usr/bin/env python3
"""
build_tools/patch_uboot.py - Patch compiled-in environment in ARK1680 U-Boot binary
...
Examples:
  ...
  python build_tools/patch_uboot.py -i uboot.bin --dump-env

  # Find ARM MOV instructions that load CONFIG_ENV_OFFSET
  python build_tools/patch_uboot.py -i uboot.bin --find-nand-offset

  # Source-compiled binary with a real env buffer: only needs NAND offset disabled
  python build_tools/patch_uboot.py -i uboot_sdboot.bin -o uboot_final.bin --patch-nand-offset
  ...
  python build_tools/patch_uboot.py -i uboot_sdboot.bin -o uboot_final.bin --mode sdboot --patch-nand-offset

  # Custom root device
  python build_tools/patch_uboot.py -i uboot_sdboot.bin -o uboot_final.bin --mode sdboot --root /dev/mmcblk1p2 --patch-nand-offset
  ...
  # firmware_source/env/sdboot_script.txt and docs/UBOOT_SDBOOT_INVESTIGATION.md.
  python build_tools/patch_uboot.py -i uboot.bin -o uboot_selfcontained.bin --mode sdscript --replace-env --patch-nand-offset
  ...
  python build_tools/patch_uboot.py -i uboot.bin -o uboot_patched.bin --set bootdelay=9

  # Dry run — show what would change without writing
  python build_tools/patch_uboot.py -i uboot_sdboot.bin --mode sdboot --patch-nand-offset --dry-run
"""
...
PRESETS = {
    ...
    'sdscript': {
        'description': (
            ...
            'itself — see firmware_source/env/sdboot_script.txt — not in this env entry.'
        ),
        'env': {

...
|---|---|---|
| `etc/rc.d/rcS` | Touchscreen `insmod`→`modprobe` (fixed path), userdata mount rewritten to try ext4 p3 first (SD/USB boot) with NAND UBI/yaffs2 fallback | 4.19.192 kernel compat; SD/USB boot support — see `docs/AUDIO_SUBSYSTEM_INVESTIGATION.md`/`docs/UBOOT_REVERSE_ENGINEERING.md` |

...
bool configure_video_layer(VideoLayerHandle & h, uint32_t width, uint32_t height) {
    ...
    // 2026-08-18: an explicit ARKFB_SET_BLEND call used to sit here,
    // added to fix a real grey/whitish wash seen on real hardware --
    // but that was observed under the OLD, wrong display API
    // (arkapi_init_fb_video_display, before the pivot documented in
    // this file's own top comment), not this one. Checked sink's
    // ENTIRE binary (every string reference, not just VideoDecoder)
    // for anything blend-related: zero hits. Real stock's AA video
    // path never touches blend/alpha configuration at all -- removed
    // to match exactly, rather than carry a fix for a bug that may
    // not even exist under the correct API.

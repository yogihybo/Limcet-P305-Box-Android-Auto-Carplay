...
**2. Autoboot (after the countdown) appears to produce zero console output and never boots; typing `bootmmc` manually at the prompt afterward works fine.** User confirmed this predates today's `carbackcamcheck`/`bootcheck` additions -- not a regression from this session. Directly visible in `v23`'s log: `Hit space to stop autoboot:  0` (line 67) is immediately followed by `=> bootmmc` (line 68, an interactively-typed command, given the very next line is `do_bootmmc()`'s own printf) -- nothing from `carbackcamcheck`, the uEnv.txt fatload attempt, or `bootcheck` ever printed in between, even though all three are supposed to run automatically, in that order, before anything else. **Leading hypothesis, not yet confirmed**: U-Boot's saved environment (persisted on NAND) shadows the compiled-in `CONFIG_BOOTCOMMAND` default for the `bootcmd` variable once anything has ever been `saveenv`'d -- and this session's own `do_bootcheck()`/`do_bootnand()` additions call `env_save()` routinely. If `bootcmd` itself ever got persisted with a stale/empty/pre-`bootcheck` value at any earlier point (this session or before), every subsequent build's freshly-compiled `CONFIG_BOOTCOMMAND` would be silently shadowed by that stale saved copy, regardless of how correct the newly-built default is -- matching "silently does nothing, but manually typing a command still works" exactly. **Not yet confirmed** -- the single most useful next check (no code change, just a console command) is running `printenv bootcmd` at the U-Boot prompt and comparing it against the actual current `CONFIG_BOOTCOMMAND` string in `include/configs/ark1668_limcet_p305.h`; if they differ, `env default -a; saveenv` resets to the compiled-in defaults and should resolve it immediately.

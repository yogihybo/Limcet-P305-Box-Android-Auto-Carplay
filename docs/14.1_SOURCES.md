...
**ASTRI reference source (`ArkPro`)** — ASTRI's reference ARK1680 kernel/U-Boot/userspace source (public leak,
`cphatt/ArkPro` commit `e743744`), used to confirm SoC-identity and register-field findings in
`docs/1.1_HARDWARE_AND_SOC_REFERENCE.md` §9 and `docs/1.7_DISPLAY_SUBSYSTEM.md`; see [`vendor_source/README.md`](../vendor_source/README.md).  
**`linux-arkmicro` vendor BSP** — ArkMicro's own public U-Boot BSP (`RD_Software/linux-arkmicro`,
a later generation than the Limcet P306's actual stock U-Boot — see [`vendor_source/README.md`](../vendor_source/README.md)), the starting

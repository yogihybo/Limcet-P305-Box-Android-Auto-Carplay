# 4.1 U-Boot Reverse Engineering

Background doc for [README §4.0 Boot Sequence](../README.md#40-boot-sequence-stock-nand), [§5.0 U-Boot Prompt](../README.md#50-u-boot-prompt), and [§6.0 Booting Stock Kernel from SD Card or USB](../README.md#60-booting-stock-kernel-from-sd-card-or-usb-non-destructive).
...
U-Boot-prompt command (README §6.0, "Manual SD Card Boot" / "USB boot")
...
Confirmed by dumping `firmware_source/mtd1-mtd2_uboot/uboot.bin`
(`mtd1` from the live device): `strings -a uboot.bin | grep -E "(bootcmd|nandboot|setbootargs)"`, see §3) immediately
after its env block:
...
`Holden firmware update/uboot.bin` and `firmware_source/mtd1-mtd2_uboot/uboot.bin`,
**not** the actual live Limcet P306 NAND dump (`firmware_dumps/Prado firmware dump/mtd1-mtd2_uboot/extracted/uboot.bin`,
...
`firmware_source/`), and that operation wiped the real command
...
1. **Real BSP-compiled source** — `docs/historical/SD_BOOT_PLAN.md` (superseded, but
   ...
   **Update: located and pulled in.** The real repo is `RD_Software/linux-arkmicro`
   (public Gogs instance, see [`vendor_source/README.md`](../vendor_source/README.md)
   for the URL/commit); a pointer and reference notes live in `vendor_source/`.
   ...
   [`docs/7.1_UBOOT_BUILD_GUIDE.md`](7.1_UBOOT_BUILD_GUIDE.md).
...
stock, not built-in.** `docs/1.9_KERNEL_REFERENCE.md` used to state "no
...
- **SD/USB boot now documented as a manual U-Boot-prompt command**, typed by
  ...
  device. See README §6.0, "Manual SD Card Boot", and the existing manual
...
Generated with:
```bash
python patch_uboot.py -i "firmware_source/mtd1-mtd2_uboot/uboot.bin" \
...
(README §6.0 "USB boot"). Revisit only after real hardware confirms (a)
...
board, see [`vendor_source/README.md`](../vendor_source/README.md)).
...
1. **Offline, once**: `convert_bootlogo.py` (repo root) converts a JPEG
   (e.g. the dumped `firmware_source/mtd8_bootlogo/bootlogo`,
...
(see `docs/7.1_UBOOT_BUILD_GUIDE.md`).
...
All 750 functions in the stock binary were decompiled in one pass (raw decompile output and
function index removed from the repo as intermediate working data, not needed once the findings
below were written up) — the full function listing, address order, plus up to 4 string hints per
function (Ghidra auto-labels string data by content, which substitutes for the missing symbol
names).
...
card specifically (see `docs/7.1_UBOOT_BUILD_GUIDE.md`'s boot chain). There's no
...
1. **The ARK header vs. `go`'s entry point.** Normally `Stepldr` reads
   the ARK header's `EP` field (see `docs/7.1_UBOOT_BUILD_GUIDE.md` §"Stock Binary
   ...
   (originally discovered the hard way, see `docs/7.1_UBOOT_BUILD_GUIDE.md`

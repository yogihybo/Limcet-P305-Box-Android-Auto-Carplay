...
On power-on, U-Boot checks for a FAT32 SD card. If a file named `UpConfig` is present in the SD root, U-Boot runs `arkupdate`, which reads the `update` script and flashes each listed partition in sequence. After completion the unit reboots — remove the SD card so it doesn't re-flash. See [`docs/4.1_PARTITION_LAYOUT.md`](4.1_PARTITION_LAYOUT.md) for offsets and sizes.
...
Both stay flashable manually from the U-Boot prompt instead — see [§6.0 U-Boot Prompt](../README.md#60-u-boot-prompt).

...
CONFIG_ITEMS=(
    ...
    "disable_msncoreapp_autolaunch|Disable MsnCoreApp auto-launch at login|firmware_overlay/etc/profile already ships with 'MsnCoreApp -qws&' commented out (see docs/1.8_ARK1680_TS_REVERSE_ENGINEERING.md) so it doesn't auto-run on every shell login. Turning this OFF re-enables the auto-launch line instead. Run 'start_msn' manually when this is on.|ON"

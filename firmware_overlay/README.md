...
| `etc/asound.conf` | `dmix`'s slave gains an explicit `rate 48000` (was left unset/auto-negotiated in the base `firmware_source` copy) | 2026-08-21 — real phone-side logcat confirmed Android Auto's media audio channel is protocol-mandatory 48000 Hz (rejects 44100 outright, tears the whole session down); `custom_ui/docs/AUDIO_CPU_SPIKE_AND_SIDECAR_LIFECYCLE_HANDOFF.md` had already traced a 60%+ CPU spike during playback to ALSA's `plug` layer software-resampling 48000→44100 to reach this same slave, whose fix (pin the slave to 48000 so nothing needs resampling) was documented but never applied — this file is that fix, finally landed in the one place that actually reaches the device. See `custom_ui/src/androidauto/session.cpp`'s own `audioChannelMedia_` construction for the matching 48000 on the AA-protocol side. **Not yet hardware-tested for CPU usage with this exact combination.** |
...
| `etc/blueware-bw121.properties` | `HFP_PWR_REC=` → `HFP_PWR_REC=1` | 2026-08-19 — with it empty, `blueware` starts with `[RECONNECT:0]` (confirmed via a real captured boot log, `docs/logs/bluetooth log stock_260718.txt`), completely disabling its own daemon-level paging retries. See `custom_ui/docs/BLUETOOTH_RECONNECT_HANDOFF.md`. **Not yet hardware-tested.** |

...
void update_dashboard_status(DashboardWidgets * w) {
    ...
    if (w->aa_status_lbl && w->connect_lbl) {
        if (is_carplay) {
            ...
            // 2026-09-05: consolidated onto the shared status poller --
            // see androidauto_client.h's own AndroidAutoStatusSnapshot
            // comment. This screen no longer runs its own socket/thread
            // at all; hal::cached_android_auto_status() is a cheap,
            // non-blocking read of the one shared background poll.
...
// Pausing poll_timer here (and resuming on LV_EVENT_SCREEN_LOADED, fired
// when navigation::pop() brings this screen back) avoids wastefully
// recomputing this screen's own labels/telemetry every second while
// it's not visible. There's no background poller to pause anymore --
// hal::cached_android_auto_status() is backed by ONE shared poller
// (see androidauto_client.h's own comment) that keeps running
// regardless, cheaply, since only one instance of it exists system-wide
// now instead of one per screen.

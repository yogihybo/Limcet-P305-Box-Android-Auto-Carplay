# 7.1 Booting Stock Kernel from SD Card or USB — Reference

Background doc for [README §7.0 Booting Stock Kernel from SD Card or USB](../README.md#70-booting-stock-kernel-from-sd-card-or-usb-non-destructive).

**Status:** Reference — full command sequences, build-tool flags, and file layouts behind §7.0's
summary table. All three methods below are hardware-confirmed for kernel loading; see the table
in README §7.0 for the current per-method status.
...
**Confirmed working end-to-end on real hardware.** See [`docs/5.1_UBOOT_REVERSE_ENGINEERING.md`](5.1_UBOOT_REVERSE_ENGINEERING.md) §10 for the full RE writeup.
...
| 10 | reversingtrack | `mtd10_reversingtrack/reversingtrack` | Dumped (1.2 MB, RSTK format — see [`docs/4.1_PARTITION_LAYOUT.md`](4.1_PARTITION_LAYOUT.md)) |

...
**Current state, top to bottom:** WiFi (RTL8811CU, USB) and Bluetooth (RTL8761B / RTL8761BTV, HS-UART)
are both hardware-confirmed working, including wireless CarPlay/Android Auto end to end across both
the stock `blueware` stack (§4–§6) and native upstream Linux **BlueZ 5.66** (§8).
Getting there took several fixed bugs, all hardware-confirmed: a dedicated second USB
controller (`usb1`) for onboard WiFi that needed distinguishing from the external port
(`usb0`); a missing RTL8761B Bluetooth firmware blob; a gadget-UDC race between `usb0`/`usb1`
that was breaking wired CarPlay, fixed by locking `usb1` to host mode; and a WiFi-driver
regression after a vendor SDK update where the new channel-plan/regulatory logic fell back to
a `WORLDWIDE` plan that excludes 5GHz, fixed by passing `rtw_country_code=US`. **Still open:**
wired Android Auto (AOA mode) never completes and always falls back to wireless — disassembly
has narrowed this to `Accessory::isValid()`'s USB-interface-class-only filter possibly
rejecting a phone's default (pre-AOA) interface descriptor before it ever reaches the
accessory-mode state machine; confirming this needs a live USB descriptor capture from a
connected phone, not further static analysis (§7 below).
...
| **Bluetooth** | Realtek RTL8761BTV (Feasycom BW121) | HS-UART (`/dev/ttyHS1`, 3-Wire H5 @ 1.5Mbps) | **GPIO 91** (Reset / Enable pin) |
...
- `BlueToothAdapter_Blueware` opens `/dev/bw_serial` via a
  ...
  in `docs/3.1_UI_AND_APP_ANALYSIS.md:481`, just now confirmed from the app
  side too.
...
(`docs/1.6_BD37033.md` §2) — `strings`/`.rodata` on `blueware` show a
...
`firmware_source/mtd6_rootfs/rootfs/`:**
...
Bluetooth — see `docs/1.6_BD37033.md` and `docs/1.5_AUDIO_SUBSYSTEM_INVESTIGATION.md`,
not this document.
...
See `docs/1.11_VENDOR_BSP_RESEARCH.md` §5 for the full writeup; summary here
...
3. **`Accessory::isValid(libusb_device*, libusb_device_descriptor*)`**
   ...
   **No vendor/product ID check anywhere in this function** -- confirmed
   ...
   generic class-only filter `docs/1.11_VENDOR_BSP_RESEARCH.md` §5 already
...
vendor SDK -> 2025-03-27, see `docs/1.11_VENDOR_BSP_RESEARCH.md` §2), user
...
## 8. Upstream Linux BlueZ 5.66 Subsystem & RTL8761BTV (Hardware-Confirmed 2026-08-19)

Historically, this hardware platform used the proprietary Feasycom `blueware` daemon (`/usr/bin/blueware`) running over `/dev/ttyHS1` with AT-commands and non-standard userspace sockets (`/dev/bw_aap`, `/dev/bw_iap`).

On **2026-08-19**, full native upstream Linux **BlueZ 5.66** was successfully brought up and validated on live hardware over the 4.19.192 kernel:

### 8.1 Hardware & Silicon Verification
* **Silicon Identification**: The Feasycom BW121 module houses a **Realtek RTL8761BTV** dual-mode Bluetooth 5.0 baseband controller (LMP Subversion `0x8761`, ROM version 1, EVersion 1).
* **Reset Pin**: GPIO 91 (`/sys/class/gpio/gpio91`) provides hardware active-low reset.
* **Serial Bus**: Connected via `/dev/ttyHS1` (`ark1680_hsuart`) using 3-Wire UART H5 protocol with hardware RTS/CTS flow control.

### 8.2 Driver & Patcher (`rtk_hciattach`)
* Mainline Linux in-kernel `btrtl.c` support for RTL8761B was introduced in Linux 5.8 (commit `9d38f887b47b`).
* On Linux 4.19.x, the open-source **`rtk_hciattach`** tool (`radxa/rtkbt`) drives the initialization cleanly in user space with zero kernel driver patches:
  1. Pulses GPIO 91 to reset the baseband.
  2. Syncs at 115,200 baud over H5.
  3. Uploads `/lib/firmware/rtlbt/rtl8761b_fw` (43,980 bytes, version `0x6ca9434d`, 250 packets).
  4. Steps the UART speed to **1,500,000 baud (1.5 Mbps)**.
  5. Attaches the Linux kernel H5 line discipline, creating **`hci0`**.

### 8.3 Daemon Architecture (`bluetoothd 5.66`)
* `dbus-daemon` runs on the system bus (`unix:path=/var/run/dbus/system_bus_socket`).
* `bluetoothd 5.66` binds to `hci0` (management interface 1.14), registering standard Linux D-Bus interfaces (`org.bluez`):
  - **A2DP Sink / Source** (`profiles/audio/a2dp.c`)
  - **AVRCP Controller / Target** (`profiles/audio/avrcp.c`)
  - **Network PAN (PANU, GN, NAP)** (`profiles/network/server.c`)
  - **GATT Database & LE Advertising Manager** (`src/gatt-database.c`)
  - **SDP Server** (`src/sdpd-service.c`)

Full engineering details, kernel `.config` options, and execution logs:
- Engineering Handoff: [`docs/BLUEZ_AND_KERNEL_BLUETOOTH_HANDOFF.md`](BLUEZ_AND_KERNEL_BLUETOOTH_HANDOFF.md)
- UART Trace: [`docs/logs/bluetooth bluez UART log.txt`](logs/bluetooth%20bluez%20UART%20log.txt)

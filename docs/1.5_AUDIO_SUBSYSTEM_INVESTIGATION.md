...
**Working theory, not yet confirmed**: the OEM/factory mic is wired to a BD37033 input
channel rather than the SoC's direct `MICIN`. Under this device's current `SoundType=0`
config, BD37033 is never initialized/unmuted, so the OEM mic's real audio signal never
reaches the capture pipeline regardless of what triggers the "listening" UI -- consistent
with clean session open/close (Step 2) and zero real content, while the "OK Google"
acknowledgment (Step 1) is likely happening on the phone's own mic and says nothing about
the car hardware either way. If true, this isn't "two mics, one hardware-broken" so much
as "one driver config that can only actively route one of the two mic inputs at a time,"
currently set to serve the aftermarket path.

**Concrete next steps to actually confirm this, not yet done:**
1. Physically trace/continuity-check the OEM-mic harness pin against BD37033's analog
   input pins vs. the SoC's own `MICIN` pin -- the only way to confirm the wiring
   assumption directly, rather than infer it from software behavior.
2. If wiring confirms BD37033, try `SoundType=3` with the OEM mic connected and check
   whether it captures real audio (expected, per this hypothesis) while the aftermarket
   mic goes silent (already documented to happen).
3. `dmesg` capture across an OEM-mic voice-command attempt, grepped for `pcmC0D0c`
   (capture-direction XRUN marker) -- the same decisive test flagged and never run in
   the 2026-08-03 investigation, now worth doing specifically with the OEM mic selected.

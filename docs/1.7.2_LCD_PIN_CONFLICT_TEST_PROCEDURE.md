...
**Status: the theory this procedure tests is confirmed correct, and the bug is fixed and hardware-confirmed** — see `docs/1.7_DISPLAY_SUBSYSTEM.md`'s pin-conflict section for the resolution. This procedure is kept as the methodology that proved it, and as a regression-test recipe if the symptom ever reappears (e.g. after a kernel change touching pinmux or `i2c-gpio`).

**Context:** `LCDTest -qws`'s color-test-pattern grid showed dramatic,
value-dependent color corruption (red/cyan cells in what should be a
neutral grayscale row, wrong hues in the yellow/olive row) on this
project's build, while stock hardware showed the same test correctly
(only a subtle panel-characteristic tint). Framebuffer content and the
LCDC's own 242 named registers were both checked and ruled out (see
`docs/historical/DEVICE_TEST_CHECKLIST_2026-07-18.md`) — the real cause
turned out to be a **physical pin-sharing conflict**: LCD RGB888 data
...
as `i2c-gpio-1`'s SDA (BD37033 audio chip bus) — `i2c-gpio` permanently
claims these pads to GPIO function at boot and never releases them.
An earlier single static debugfs snapshot had concluded the pins stay
muxed to LCD function even after `i2c-gpio` claims them, i.e. that it
only breaks I2C, not the LCD — that conclusion was wrong; live testing
showed a tint that visibly **moves across the screen in sync with I2C
message activity**, which this procedure was built to test directly.

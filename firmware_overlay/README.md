...
| `etc/blueware-bw121.properties` | `HFP_PWR_REC=` → `HFP_PWR_REC=1` | 2026-08-19 — with it empty, `blueware` starts with `[RECONNECT:0]` (confirmed via a real captured boot log, `docs/logs/bluetooth log stock_260718.txt`), completely disabling its own daemon-level paging retries. **Not yet hardware-tested.** |

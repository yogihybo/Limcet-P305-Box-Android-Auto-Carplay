...
- `docs/historical/1.3_MCU_ADAPTERS.md` §"BoxP300 (McuType=6) — full command dispatch" —
...
- **The outbound direction (Linux → STM32) is an explicitly open item**:
  ...
  not yet enumerated" (`docs/historical/1.3_MCU_ADAPTERS.md`, verbatim). This matters
...
- Root SSH access works. BusyBox toolset present: `cat`, `dd`, `hexdump`,
  ...
  `docs/historical/1.3_MCU_ADAPTERS.md` as a real constraint on live capture technique.
- `docs/historical/1.3_MCU_ADAPTERS.md`'s "Capturing the codes live on the device" section
...
4. **Is there already an outbound Linux→MCU command for this?** — **DONE,
   ...
   `docs/historical/1.3_MCU_ADAPTERS.md`'s "Outbound (Linux → STM32)" table for the full
...
3. In parallel (doesn't need the vehicle): finish enumerating BoxP300's
   ...
   directly from `docs/historical/1.3_MCU_ADAPTERS.md`'s existing open item, same
...
- `docs/historical/1.3_MCU_ADAPTERS.md` — full BoxP300 command dispatch table, the "not yet

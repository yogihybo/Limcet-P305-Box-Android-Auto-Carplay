...
On power-on, U-Boot checks for a FAT32 SD card. If a file named `UpConfig` is present in the SD root, U-Boot runs `arkupdate`, which reads the `update` script and flashes each listed partition in sequence. After completion the unit reboots — remove the SD card so it doesn't re-flash. See [`docs/9.1_PARTITION_LAYOUT.md`](9.1_PARTITION_LAYOUT.md) for offsets and sizes.
...
Use `bash build_update.sh` (see [§11.0 Build & Flash Tool](../README.md#110-build--flash-tool) / [`docs/11.1_BUILD_FLASH_TOOL.md`](11.1_BUILD_FLASH_TOOL.md)) to select partitions and generate the SD package — it does the build and package generation for Steps 1–2 in one interactive session.

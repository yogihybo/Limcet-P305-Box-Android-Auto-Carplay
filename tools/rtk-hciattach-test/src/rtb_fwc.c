...
static struct patch_info patch_table[] = {
	...
	/* 2026-08-19: real hardware (prado-firmware-reconstruction, Feasycom
	 * BW121 module) -- HCI Read Local Version reports LMP Subversion
	 * 0x434d ("CM" in ASCII) and HCI Revision 0x6ca9, neither of which
	 * match the stock RTL8761BTV entry above (0x8761/0x000b). The H5
	 * sync/config handshake completes cleanly and "Read ROM version 01"
	 * looks like genuine structured Realtek protocol data, not noise --
	 * most likely Feasycom customized the reported LMP subversion on an
	 * otherwise-genuine RTL8761BTV die, matching this project's own
	 * separately-confirmed evidence: the real on-device firmware/config
	 * blobs (etc/rtl8761bt_fw, etc/rtl8761bt_config) start with the
	 * standard Realtek "RealtechMC" patch header and 0x8723ab55 config
	 * magic respectively -- genuine stock Realtek blob format, not a
	 * proprietary Feasycom repackaging. Matching purely on lmp_subver
	 * (no other flags) and reusing CHIP_8761B/the same rtl8761b_fw/
	 * rtl8761b_config filenames the stock entry expects, since these
	 * are confirmed to be the correct files for this exact module.
	 */
	{ 0, CHIP_8761B, 0x434d, 0x434d, 0, 0,
		"rtl8761b_fw", "rtl8761b_config", "RTL8761BTV (Feasycom BW121)"},

...
**Wording fixed same day** (`generate_boot_status_logos.py`): the
initial splash was tense-inconsistent ("Loading U-Boot" next to
"Loading USB"/"Loading NAND", but U-Boot has already finished loading
by the time it shows) -- corrected to "Loaded U-Boot". `bootlogo_sd.raw`
("Booting SD Card" -> "Loading SD") fixed to match the same pattern.
`bootlogo.raw` is now regenerated by this script too (previously
treated as fixed reference-only input) -- confirmed the USB/NAND
variants come out byte-identical since their text didn't change.

**Tearing fixed same day** (`linux-arkmicro` commit `0b336e63f`):
user reported a multicolour striped screen for about a second between
each splash swap. `display_bootlogo_file()` kept OSD1 enabled and
pointed at the same physical buffer the whole time it
`memset`+`fatload`'d new data into it -- the LCDC composites from that
buffer continuously at the panel refresh rate, so the screen showed
whatever partially-written state existed at each refresh during the
~1s load. The very first call was unaffected (OSD1 starts disabled
from `ark_display_init()`'s default state); only later swaps hit it.
Fixed by disabling OSD1 before the `memset`/`fatload` and only
re-enabling it once the new image is fully loaded.

**Separate, similar-looking striping during the actual NAND kernel
read -- accepted as a known cosmetic artifact for now, not chased
further.** User reported striping during `nand read ${kerneladdr}
kernel`/`bootz`, i.e. well after the "Loading NAND" splash swap
itself has already completed and OSD1 is back on and stable -- not
explained by the tearing fix above. Checked one candidate
(`ark_show_bootlogo()` unconditionally enables `OSD2_LAYER` even
though `ENABLE_LCDCONSOLE=0` means nothing ever writes real content
into its buffer) but the code comment there says this was a
*deliberate* match to stock U-Boot's own observed register state from
earlier parity work -- stock does the same thing and presumably
doesn't show this striping, so it's probably not the cause. Leading
untested guess: real memory-bus contention between the NAND
controller's reads and the LCDC's continuous framebuffer reads during
the actual flash read, not a software buffer-overwrite bug like the
splash-swap case -- would need bus priority/QoS register tuning to
fix, not chased further per explicit user decision to accept this for
now. If revisited: first check whether it's present on a plain
`bootnand` from before any of this session's bootlogo work at all, to
tell a pre-existing hardware artifact apart from a regression.

Not yet hardware-tested (the tearing/wording fixes).

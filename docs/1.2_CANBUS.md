...
See also [`1.2.1_REAR_DVD_CANBUS_INVESTIGATION.md`](1.2.1_REAR_DVD_CANBUS_INVESTIGATION.md) — an open
...
**Real, hardware-confirmed correction (2026-08-31)**: the stock factory menu actually has
**two separate monitor screens** — one for MCU (UART) frames, one for raw CAN frames.
On this specific unit: the **MCU-frame monitor has real data** — it matches a direct
external UART sniff of `/dev/ttyHS0` (`tools/mcu-handshake`), which independently captured
real, checksum-verified `CMD 0x02` key-event frames for real button presses (Vol+/Vol-/
Answer/Hangup, see `docs/MCU_COMMAND_REFERENCE.md`). The **CAN-frame monitor shows
nothing**, even though real SWC key events are clearly reaching the SoC over UART.

This is a real, still-open question, not necessarily a broken UI screen: it's unresolved
whether the CAN-frame monitor itself is non-functional (common for vendor debug screens to
be half-wired), or whether it's tapping a genuinely different point than wherever the MCU
actually gets its SWC data from — e.g. if the MCU's own CAN peripheral isn't what's
decoding these presses at all. Given the MCU-frame monitor (and the external UART sniff)
both show real key events regardless, that layer is confirmed working — but "no CAN
frames visible" should not be read as "wiring/CAN-ID problem" per the table above without
first checking whether the CAN-frame monitor screen itself is trustworthy on this unit.
...
**Status update (2026-08-31): RESOLVED on this unit, real evidence.** A direct UART
capture confirms SWC buttons genuinely work end-to-end — real `CMD 0x02` key-event frames
correlate with real steering-wheel button presses (Vol+/Vol-/Answer/Hangup, plus Home and
the rotary knob), checksum-verified. This table is kept for historical/diagnostic
reference (and is still useful if SWC regresses on a different unit/rebuild), but the
underlying problem it describes is not currently open.

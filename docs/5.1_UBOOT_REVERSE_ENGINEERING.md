# 5.1 U-Boot Reverse Engineering
...
1. **Real BSP-compiled source** — `docs/historical/SD_BOOT_PLAN.md` (superseded, but
   ...
   **Update: located and pulled in.** The real repo is `RD_Software/linux-arkmicro`
   ...
   [`docs/8.1_UBOOT_BUILD_GUIDE.md`](8.1_UBOOT_BUILD_GUIDE.md).
...
(see `docs/8.1_UBOOT_BUILD_GUIDE.md`).
...
card specifically (see `docs/8.1_UBOOT_BUILD_GUIDE.md`'s boot chain). There's no
...
1. **The ARK header vs. `go`'s entry point.** Normally `Stepldr` reads
   the ARK header's `EP` field (see `docs/8.1_UBOOT_BUILD_GUIDE.md` §"Stock Binary
   ...
   (originally discovered the hard way, see `docs/8.1_UBOOT_BUILD_GUIDE.md`

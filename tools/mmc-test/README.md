...
1. `mmc0` (SD card slot, already **CONFIRMED** working per
   `PIN_MASTER_LIST.md`) — card enumerated, type/name if available.
...
3. `mmc1` — **investigative, not pass/fail.** Its DTS comment ("SDIO WiFi
   ...
   USB RTL8811CU instead (see `docs/DRIVER_TEST_PLAN.md` section 8). This
   step just reports what, if anything, is actually enumerated there —
   "nothing" is a valid, useful answer, not a failure.

...
This utility (`mcu-handshake.py`) allows manual emulation of the SoC-to-MCU connection handshake and periodic status ping-pong. 
...
### Usage
Run the script on the device over SSH or the serial console:
...
python3 /usr/bin/mcu-handshake.py

# Run in verbose mode to print hex dumps of RX/TX packets
python3 /usr/bin/mcu-handshake.py --verbose
```

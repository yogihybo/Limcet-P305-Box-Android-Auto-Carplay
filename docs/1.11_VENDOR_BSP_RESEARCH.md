# 1.11 Vendor BSP Research (ark1668ed-bsp / cstech-ip17-rootfs)

Background doc for [README §1.0 Hardware](../README.md#10-hardware) — general vendor/BSP provenance research.
...
**not** part of this project's build (see `docs/13.1_SOURCES.md`), looking for anything
...
(`/home/osboxes/Downloads/linux-arkmicro`, see `docs/13.1_SOURCES.md`), not in this repo — see
...
- This document: `docs/1.11_VENDOR_BSP_RESEARCH.md` (this repo).
...
- Vendor source trees referenced: `/home/osboxes/Downloads/ark1668ed-bsp`,
  ...
  personal Downloads on this machine (see `docs/13.1_SOURCES.md` for how this project usually
...
**`nr` values above 44 are generation-specific, not stable across the ARK1680/ARK1668 split.**
The vendor BSP's `ARKFB_SET_OSD_ALPHA` (49), `ARKFB_GET/SET_OSD_CFG` (51/52),
`ARKFB_SET_PRIORITY` (47), and `ARKFB_SET_WINDOW_POINT` (53) do **not** correspond to the
same-numbered ioctls on this board — those exact `nr` values are already used here for
different, independently disassembly-confirmed ioctls (47=`ARKFB_SET_REG_VALUE`,
49=`ARKFB_GET_WINDOW_ADDR`, 51=`ARKFB_SET_SCREEN_INFO`, 52=`ARKFB_GET_PLATFORM_INFO`,
53=`ARKFB_GET_WINDOW_FORMAT`). The `nr` space was evidently reused for different purposes as
the window/layer model evolved between ARK1680 and ARK1668 — same convention, different
assignment. Treat any `nr` above 44 as generation-specific unless independently confirmed
...
same settings taxonomy as this project's own `docs/1.10_SETTINGS_REFERENCE.md`).

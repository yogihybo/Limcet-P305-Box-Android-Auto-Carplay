...
```
arm-linux-gnueabihf-gcc -static -O2 -Wall -o ark-ts-test ark-ts-test.c
arm-linux-gnueabihf-strip ark-ts-test
```
...
/ # ark-ts-test events /dev/input/event0
```

## Suggested debug flow

1. `ark-ts-test regs` — confirm the ADC block is clocked/enabled and
...
4. `ark-ts-test events /dev/input/eventN` — confirm userspace actually

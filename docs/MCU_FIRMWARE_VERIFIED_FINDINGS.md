...
**Conclusion, stated plainly**: this word pair reading as `0xFFFFFFFF` in the dump is real (not a display bug in this analysis) and does not reflect the device's true, working configuration (which must have had a valid SP/Reset there to have ever booted) — but the precise mechanism is not fully nailed down; it may be a one-off extraction failure for this specific address during that run rather than a systematic, generally-recurring gap. **Nothing else in this document's findings depends on this value** — `is_app_valid()`'s real behavior and the `0x08003000` app-base-address finding were both derived from disassembling the *bootloader's own code* (sections 5.2, 6), never from reading this address directly out of the dump, and this session's own live-hardware SWD reads of the currently-flashed clean-room build (sections 15-17) are a separate, independently-confirmed data source unaffected by this dump-specific gap. If the original device is ever accessible for re-extraction, these two specific words would be a good target to recapture; otherwise this stands as a known, honest limitation of the archived dump rather than a claim to build further conclusions on.

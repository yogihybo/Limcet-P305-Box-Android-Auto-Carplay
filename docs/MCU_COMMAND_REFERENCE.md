...
> ⚠️ **Scope note (added 2026-09-15)**: every MCU-side ✅ CONFIRMED row below was
> derived from `hardware/MCU/can_app.bin`, a generic `DCn32-VOLVO-V2.10-20240909`
> reference build -- **not** this vehicle's own real firmware (see
> `docs/MCU_FIRMWARE_VERIFIED_FINDINGS.md`'s scope clarification for the full
> detail). This project now holds a real, hardware-verified dump of the actual
> firmware running on this vehicle's MCU (`Limcet-V1.0-1302`), and a first
> re-check against it -- `docs/MCU_LIMCET_DISPATCH_RECHECK_2026-09-15.md` --
> found the real SoC->MCU dispatch table has only 5 entries (`0x81, 0x82, 0xA0,
> 0xFF, 0xE1`; no `0x84`/`0x85`/`0x87`/`0x88`), and found strong (but not yet
> hardware-confirmed) evidence that the surviving command *numbers* may not map
> to the same handlers this doc describes below -- e.g. settings-sync-shaped
> code was found reachable via wire byte `0xFF` in the real firmware, not
> `0xA0`. Read that doc before relying on this one's MCU-side rows for
> anything real-vehicle-relevant; the SoC-side (`libMcuCenter.so`) rows below
> are unaffected (that binary's provenance was never in question).
...
**RESOLVED (2026-09-15)**: *why* neither candidate MCU firmware's own documented/coded `CMD 0x88` handling ever produced a reply on this real hardware -- this project now holds a real, hardware-verified dump of the actual MCU firmware running on this vehicle (`Limcet-V1.0-1302`, see `docs/MCU_LIMCET_DISPATCH_RECHECK_2026-09-15.md`), and its real SoC->MCU dispatch table was found to have only 5 entries (`0x81, 0x82, 0xA0, 0xFF, 0xE1`) -- **no `0x88` entry at all, anywhere in the binary.** The real vehicle's MCU firmware was simply never built to answer that command; neither candidate reference firmware's `CMD 0x88` behavior was ever going to be reachable on this hardware. Not a link/timing issue, not a gap in either candidate's own logic -- the real firmware genuinely doesn't implement it.
...
| `0xE1` | Enter bootloader for update | **Real erase risk**: resident bootloader erases flash before waiting for the first byte, no recovery if nothing follows. `tools/mcu-probe --reboot-probe` gated behind `--confirm-erase-risk`. **CONFIRMED ON REAL HARDWARE (2026-09-12)**, not just the generic Volvo reference below: `docs/MCU_FIRMWARE_VERIFIED_FINDINGS.md` section 14 independently traced the real Toyota Prado dump (`hardware/MCU/live_dumps/live_app_1302.bin`) and found the identical, unauthenticated behavior — `CMD 0xE1`'s real handler (`0x0800665D`) writes the bootloader magic cookie unconditionally, gated by nothing but a simple additive frame checksum (no crypto, no session state). This is a genuine, real sideloading path on this exact vehicle, not an inference from `can_app.bin`. | `MCUAdapter_BoxP300::onSendUpdateReadyTimer()` — a real timer callback, implying this is only armed during an actual firmware-update flow rather than firing ambiently | ✅ confirmed (generic reference); ✅ **independently confirmed on real hardware**, see above |
...
|---|---|---|---|---|
| `0x00` | Camera Type | `0`=AfterMarket / `1`=Factory / `2`=AfterMarket 360 / `3`=Factory 360 | `GPIOB Pin 1`. **Hardware-confirmed: controls the OEM camera relay.** (Value `2` is a distinct `AT+UPGRADE` trigger, not a camera value). **Real hardware test, 2026-09-04**: a `custom_ui` home-dashboard button was wired to call `sync_video_relay(true)` (this id plus `id=0x11` and `CMD 0x84` together) directly, with no reverse-gear transition happening -- it did NOT switch the display to Factory LCD. Confirms the arm-then-trigger model: sending this command alone only updates the stored preference, it doesn't force an immediate relay switch on its own -- the MCU's own real gear-transition edge is still required to actually apply it. The button was reverted (`4893de3`) since it had no real effect. No on-demand, software-only way to force a Factory LCD switch is currently known. | ✅ sent by `sync_video_relay()` |

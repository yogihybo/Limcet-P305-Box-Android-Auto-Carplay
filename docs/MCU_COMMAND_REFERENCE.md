...
| `0xFF` | Never sent | **Broadened search (2026-09-03), still not found.** Checked every other `MCUAdapter_*` variant in `libMcuCenter.so` (same cross-class technique that found `0x84` alive in `BoxC280`) and a whole separate library, `libCanBus.so` -- real, previously-unopened, containing 4 more vehicle-brand CAN-dashboard adapter classes (`CanBus_XinRi`, `CanBus_Raise_Volkswagen`, `CanBus_XBS_Mazda`, `CanBus_LiHang_JMCE200N`), confirmed genuinely using the same `[0x2E][cmd][len][payload][checksum]` framing (`mov r3,#0x2e` found directly in `makeCanBusProtocol()`). None of the ~20 real `writeCanBusData()` call sites checked send `cmd=0xFF` via an immediate constant. **None of these classes is the Prado's own active adapter either** -- confirmed real, but a tangent, same as `BoxC280` was for `0x84`. | -- Neither side has a confirmed real sender, now checked considerably more broadly than before; open on both sides equally, not a `custom_ui` gap. |

...
bool init_camera(CameraHandle & out, const char * dvr_path, const char * carback_path) {
    ...
    if (out.dvr_fd < 0) {
        std::fprintf(stderr, "%s [HAL:REVCAM] WARN: %s unavailable (%s)\n", core::log_timestamp().c_str(),
...
void start_camera_stream(CameraHandle & h) {
    ...
    if (ioctl(h.dvr_fd, ARK_DVR_START) < 0) {
        std::fprintf(stderr, "%s [HAL:REVCAM] ERROR: ARK_DVR_START failed (%s)\n", core::log_timestamp().c_str(),
                     std::strerror(errno));
    }
}
...
void stop_camera_stream(CameraHandle & h) {
    ...
    if (ioctl(h.dvr_fd, ARK_DVR_STOP) < 0) {
        std::fprintf(stderr, "%s [HAL:REVCAM] ERROR: ARK_DVR_STOP failed (%s)\n", core::log_timestamp().c_str(),
                     std::strerror(errno));
    }
}

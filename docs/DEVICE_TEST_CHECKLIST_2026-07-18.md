...
- **Stale saved `bootcmd`**: U-Boot's saved environment (persisted on NAND) shadows the compiled-in `CONFIG_BOOTCOMMAND` default once anything has ever been `saveenv`'d -- and `do_bootcheck()`/`do_bootnand()` both call `env_save()` routinely now. If `bootcmd` itself ever got persisted with a stale/pre-`bootcheck` value at any earlier point, every subsequent rebuild's freshly-compiled default would be silently shadowed. **Not yet confirmed** -- check `printenv bootcmd` at the U-Boot prompt vs the actual `CONFIG_BOOTCOMMAND` string; `env default -a; saveenv` resets to compiled defaults if they differ.
- **Found and fixed, same day**: `ark_carback_camera_check()` (the new first command in `CONFIG_BOOTCOMMAND`, §80) calls `ark_mcu_notify_backcar()` -> `ark_uart2_putc()`, which had an **unbounded busy-wait** on the UART2 TX-ready bit, no timeout. This runs automatically on every boot, gated only by a GPIO5 reverse-gear read -- on a bench/test rig a floating or unconnected GPIO5 pin can easily read as asserted (active-low), triggering this never-hardware-tested UART2 init/write path even when not genuinely in reverse. If any of the untested register assumptions were wrong and TXFF never cleared, this would hang the *entire* automatic boot chain silently and permanently with zero console output -- matching the observed symptom exactly (nothing prints, nothing boots, but the system isn't truly locked up since a manually-typed command afterward still works). **Fixed** (`linux-arkmicro` commit `5de2ed1d9`): bounded with the same timeout-loop pattern already used elsewhere in this file (`jpeghw`) -- times out and prints a warning instead of hanging. Not yet hardware-tested. This doesn't rule out the stale-`bootcmd` theory too (both could be real, independent issues) -- `printenv bootcmd` is still the right next check regardless.

**3. `/dev/mtd3` doesn't exist under the booted (bootmmc) kernel -- breaks the new bootcount-clear rcS line.** The very end of `v23`'s log shows, from inside the booted shell: `Cannot access MTD device /dev/mtd3: No such file or directory` / `Error: environment not initialized` -- this is §79's new `(sleep 15 && fw_setenv bootcount 0) &` line in `rcS` failing every single boot. `/etc/fw_env.config` expects the U-Boot environment on `/dev/mtd3` (offset `0x120000`), matching stock's own config exactly -- but that assumed MTD numbering isn't actually showing up as a real device node on this specific (SD-card/`bootmmc`) boot, even though the NAND controller itself does still probe successfully in the same log (`nand: device found...NAND: 128 MiB`). Not yet root-caused -- candidates: no `mdev` rule creates `/dev/mtdN` nodes for the environment-hosting partitions specifically (only `mtd8`-`mtd11` are visibly redirected to `/nanddata/` per the existing `redirect_mtd_data` rcS logic, nothing lower), or the MTD partition table genuinely differs/isn't fully registered on a `bootmmc` boot the way it would be on a real `nandboot`/NAND-rootfs boot. **Practical impact so far**: harmless failure (the subshell just errors and exits, nothing else depends on it), but it means `bootcount` is never actually being cleared from userspace on this boot path yet, undermining §79's whole "confirm-good clears the counter" design until fixed. Needs its own investigation -- check `ls /dev/mtd*` and `cat /proc/mtd` live on the device to see what's actually there before guessing further.

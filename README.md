...
**Default (non-interrupted) autoboot order:** `bootusb` → `boothybrid` → `bootstock` → `run nandboot` (last resort). Import of `uEnv.txt` from the SD card happens first and can override this (or any env var) without recompiling. All four paths (`bootmmc`/`bootusb`/`bootnand` directly, plus `bootstock`/`boothybrid` via chainload) are hardware-confirmed working; watchdog-arm-before-jump + bootcount fallback (see [`docs/5.1_UBOOT_REVERSE_ENGINEERING.md`](docs/5.1_UBOOT_REVERSE_ENGINEERING.md)) protect the automatic chain against a hung/failed jump on any of them.

...
| `HFPTIME` / `HFPTIME=` | query/set | STRING-ONLY | **Worth investigating for the clock-sync lead** — a `TIME` command distinct from `CCLK`, never tried. |
...
| `CCLK` / `CCLK?` | query | TESTED | Phone clock query, standard 3GPP `AT+CCLK`. **Fails with `ERR004` on every real hardware test this project has run** — most likely because this test phone's own HFP stack doesn't implement it (optional feature), not a blueware/wiring problem. See `project_aa_missing_auth_complete` memory follow-up #23/#24 for the full investigation and the still-open `HFPTIME`/CTS leads. |

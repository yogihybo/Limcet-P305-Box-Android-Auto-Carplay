...
## DECISIVE RESULT: local `aplay` playback is completely clean -- the kernel-side audio path is very likely NOT the cause (2026-08-02)

The mute-sequencing rework (`a2350d8a7`, 2026-07-30 -- real trigger-synchronized mute matching stock's architecture) was hardware-tested first: **no noticeable change** to the stutter. That fix stays in (correct on its own terms, matches stock's real mechanism), but it rules out ASoC's `.digital_mute` dispatch timing as *the* audible cause -- the mute-flapping was a real symptom of the XRUN storm, not what makes it audible on its own.

This pushed the investigation back to the long-pending, explicitly-flagged "single most decisive test not yet run": local playback via `aplay`, isolating the entire network/`sink`/GalReaderThread/demux/decrypt pipeline out of the picture entirely, while exercising the *exact* same production audio path (`dmix`->`softvol`->`hw:0,0`, same DMA/mute/codec/kernel code AA itself uses).

**Getting there needed solving a real side-blocker first**: `usb0` (the board's only external USB port) uses `dr_mode="otg"` under `bootmmc` to preserve wired CarPlay capability, and a plain USB flash drive gave zero enumeration under that mode -- confirmed via `/sys/kernel/debug/gpio` that `gpio-76` (`usb_id`) is a **software-driven output**, not a sensed input (this board's USB-A port has no real ID pin) -- `musb_ark.c`'s `ark_musb_set_mode()` sets it low as a strap value under `MUSB_OTG`, not by reading external cable state. A runtime sysfs mode switch (`echo host > .../mode`) was attempted as a live workaround and left the port in a broken state (neither the USB stick nor wired AA worked afterward) -- this runtime code path is genuinely undertested (the driver's own comments document prior hard-to-diagnose VBUS-timing bugs in this exact function). Recovered via a full reboot. Wired AA still not connecting after the reboot turned out to be a **pre-existing, already-tracked issue** (see the wired-AA/AOA and `usb0`-boot-mode investigations), not a new regression from this experiment. Ultimately solved by booting via `bootusb` instead (forces `dr_mode="host"` cleanly from cold boot, the same path already used for the kernel/DTB, no runtime state transition needed).

**Test**: `while true; do aplay /path/to/test.wav; done`, run continuously for 30-60+ seconds (not just a short clip -- the real symptom is "continuous, never recovers, for as long as playback runs," so a brief test wouldn't have been decisive either way).

**Result: completely clean, no stuttering, for the full duration.**

**Implication**: the entire kernel-side audio-path thread this investigation spent the most time on -- DMA/PCM architecture, mute-sequencing, IRQ contention (audio DMA vs WiFi/USB), scheduling/preemption, ALSA XRUN mechanism itself -- can sustain long, clean playback with zero glitches when fed clean data. The XRUN storm and audible stutter only manifest during real AA sessions, which is now strong evidence the root cause is **upstream of ALSA**, in `sink`'s own data-supply pipeline: either the network/WiFi link itself, or how `sink`'s `GalReaderThread`/demux/decrypt/write-to-ALSA logic paces data under real streaming conditions -- not a kernel-side audio driver bug.

**Redirects the investigation fully toward the network/data-supply side**, several already-identified but not-yet-executed angles from earlier in this file become the priority again: correlating `sink`'s data-supply timing against WiFi-layer signal/retransmission data (`iw dev wlan0 station dump`) during a reproduced stutter. The already-staged kernel-side logging (period-jitter, mute, mixer-control-write) is no longer the most useful lens here -- it already showed a clean local result, so during a real AA stutter it'll likely show the same XRUN/mute pattern as before, which is now understood to be a downstream *symptom* of late/gapped data arriving, not the point of origin.

### `strace` attempt: real, then RETRACTED -- ~0.84s "stalls" were a ptrace artifact, not a genuine finding (2026-08-02)

First attempt (`-f`, broad syscall set including `poll`/`ioctl`/`read`/`recv`) crashed the AA session outright: `read ret=0` / "Encountered fatal error on read" / full teardown shortly after attaching to several threads -- direct evidence `ptrace`'s overhead on this single-core SoC is severe enough to disrupt the live network connection by itself.

Narrowed to `-e trace=write,writev,semop,semtimedop` and retried -- survived this time, produced `docs/logs/pcm_strace.log` (5878 lines). Initial read looked like a major finding: a `ARKFB_SET_FB_ADDR` (video frame-address) ioctl blocked for 843ms on one thread. Investigated further before concluding anything -- found a *second*, completely unrelated syscall (`read()` on a different thread, a network socket) blocked for the **same ~847ms, at the same wall-clock time**. Two unrelated syscalls in different threads don't coincidentally block for an identical duration at an identical moment; that's the signature of the whole process not being scheduled at all for that window, not either syscall genuinely taking that long. A full scan of the log found **29 such stalls**, all clustering tightly around 0.84-0.86s, across many different syscall types (`ioctl`/`read`/`write`/`writev`/`poll`/`semop`) and threads (227/228/230/236/237/263/312), several with multiple threads freezing and resuming within milliseconds of each other.

**RETRACTED**: user directly confirmed the app "works smoothly" with `strace` *not* attached -- the ~0.84s stalls were an artifact of `ptrace` overhead (likely interacting with the frequent `SIGIO` signal delivery visible nearby in the same log), not evidence of a real, periodic system-wide freeze. Given the earlier crash already proved `strace`'s overhead is severe on this hardware, this is the expected outcome in hindsight, not a surprise -- `ptrace`-based tracing is confirmed **too invasive a tool for this investigation on this hardware** and should not be used again for this purpose without a much narrower, single-syscall/single-thread scope, if at all.

**Next step, lower-overhead approach needed**: the already-staged kernel-side logging (`pcm_dmaengine` period-jitter, `ark1668-sddac`/`ark1668-i2s` mute/trigger, `snd_ctl` elem_write -- all plain `printk`/`trace_printk`, no `ptrace` stop-the-world overhead) remains the safe tool for correlating kernel-side timing during a reproduced stutter. For sink's own userspace timing specifically, an `LD_PRELOAD` shim intercepting `snd_pcm_writei`/`writen` (logging timestamps only, no full syscall trap) would be far lower-overhead than `strace` if userspace-side timing is still needed -- not yet built.
without stuttering.

## Pivoted from diagnosis to mitigation, plus a real (unrelated) mic bug found along the way (2026-08-02)

Given how invasive `strace` proved on this hardware and how long this investigation has run, user asked to shift from further root-cause diagnosis toward directly resolving the symptom.

### Microphone investigated as a possible related bug -- confirmed unrelated, but found a real, separate bug

User reported the microphone doesn't currently work and asked to check all audio streams against stock. Dispatched a subagent to compare `sound/soc/arkmicro/ark1668-sdadc-codec.c` (the compiled-in ADC/capture codec, `CONFIG_SND_SOC_ARK1668_ADC=y`) against stock's disassembled `vmlinux`, the same methodology that found the real DAC-side bugs earlier in this investigation. **Result: our stubs are a byte-accurate match for stock.** Stock's real equivalent functions (`cs4334_startup`/`hw_params`/etc. -- stock's internal symbol name, confirmed NOT a real external chip, consistent with this file's own already-documented "CS4334 pivot reverted"/"Voice Processor chip absent" findings) disassemble to nothing but `mov r0,#0; bx lr` too -- stock has no `read`/`write` register-access helper pair for this codec at all. Not a bug on our side.

Traced further into `ark1668_i2s.c`'s capture path (`ark_i2s_startup()`'s `SNDRV_PCM_STREAM_CAPTURE` branch) directly -- found this substantially reconstructed already: real SARADC power-up, `MIC_LINE_SEL`, RX DMA enable (a prior real fix, same class as the DAC side's `TDMAENA` fix), record-enable via `SACR1`. `ark_i2s_txctrl`/`rxctrl`/`init_cfg` being empty was already checked against stock in an earlier session and confirmed to match (stock's own trigger-control functions are also no-ops -- DMA start/stop is handled entirely by the generic ASoC `dmaengine_pcm` framework on both builds, bypassing these hooks). DTS sound-card topology (`simple-audio-card,dai-link@1`, `i2s_adc` <-> `sdadc`) is correctly wired.

**Net result**: every layer of the capture chain checked (codec, CPU DAI register config, DMA trigger, card topology) either matches stock exactly or was already correctly fixed in an earlier session. No further static lead found. One thing flagged as worth double-checking but not yet verified: whether `MIC_LINE_SEL`'s bit polarity actually means what's assumed (mic vs. line input) -- this project has hit inverted-bit-meaning bugs elsewhere before, but confirming this needs either a live capture test or much more targeted disassembly, not a productive next static step on its own. **Genuinely unresolved why the mic doesn't work** -- likely unrelated to the audio-stutter investigation (capture and playback are different signal paths, and the stutter mechanism traced so far is entirely playback-side), needs a live test (`arecord` -> listen to the result: silence vs. garbage vs. hard failure) to even know what class of bug this is, whenever hardware time allows.

### Sink pipeline architecture, clarified (not yet resolved: sync vs. async audio dispatch)

Discussed the actual data flow, confirmed via already-completed disassembly earlier in this investigation: one shared `GalReaderThread` reads all bytes off the single TCP/TLS `Transport` (video+audio+control multiplexed together, capped 4096 bytes/read via `Accessory::readBytes`) -> decrypts inline on that same thread (`SslWrapper::decryptionPipelineEnqueue`/`Dequeue`, software AES, statically-linked OpenSSL, no hardware crypto) -> `MessageRouter::routeMessage()` is the actual split point, dispatching by channel ID to `AudioSource` (audio) vs `MediaSinkBase`/`VideoDecoder` (video).

Same logical constraint already used to rule out TCP-level theories applies to a "decryption is just slow" theory too: decryption happens *before* the audio/video split, on the shared reader thread -- if it were the direct bottleneck, video should be affected too, and it isn't. The narrower, not-yet-eliminated version: video's much larger buffers could simply *absorb* a small generic reader-thread hiccup that audio's tight buffer margin can't -- distinguishing "decrypt is slow" (ruled out) from "reader thread has some small unavoidable jitter, and video just tolerates it better" (not ruled out) requires knowing whether `routeMessage()`'s audio-channel handoff to `AlsaHandle::play()` is synchronous (still on `GalReaderThread`) or asynchronous (dispatched via the confirmed-present `WorkQueue::queueWork()` mechanism to a separate thread). This was flagged in this investigation once before as never fully confirmed either way. **Dispatched a subagent to resolve it directly via disassembly -- result: RESOLVED, asynchronous, see below.**

### RESOLVED: audio dispatch is genuinely asynchronous -- full call chain traced (2026-08-02)

Subagent traced the complete real call chain statically through `libAndroidAuto.so` and `sink` (`nm -D -C` / `objdump -d -C`, addresses cited below are real, from the actual binaries):

1. `GalReaderThread::run()` (0x121934) reads bytes, calls `GalReceiver::queueIncoming()` (0x12ff18) directly -- same thread, no queueing despite the name.
2. -> `ChannelManager::queueIncoming` (0x12b5b8) -> decode/route -> **`MessageRouter::routeMessage()`** (0x13affc) -- still synchronous, same thread. Confirms the question's premise: `routeMessage` genuinely executes on the reader thread.
3. For the audio channel (endpoint class `AudioSink`, inherits `MediaSinkBase::routeMessage` at 0x138990) -> for raw PCM frames, direct virtual call -> **`AudioSink::handleDataAvailable()`** (0x1240d0) -> through `IAudioSinkCallbacks` -> **`AudioSinkCallbacks::dataAvailableCallback()`** (0x1c7ac, in `sink` itself). Still one continuous synchronous call chain, same thread, from the socket read all the way to here.
4. **This is the actual hand-off point.** `dataAvailableCallback` constructs an `AudioPlaybackWorkItem` (0x1cf5c), wraps it in a `shared_ptr`, calls **`WorkQueue::queueWork()`** (0x1c82c) on a `WorkQueue` member of `LinuxAudioSink`, then immediately calls `MediaSinkBase::ackFrames(1)` and **returns**. `AudioPlaybackWorkItem::run()` (0x1cae0, calls `ArkMediaPlayer::play()` at 0x30e64 -- the actual audio-write path) is never invoked inline here.
5. Confirmed as a genuine separate thread: `WorkQueue::start()` (0x1222ec) constructs `WorkQueueThread` objects and calls `Thread::start()` on each -- the same `Thread` base class `GalReaderThread`/`pingThread` use for real OS threads. `WorkQueueThread::run()` (0x122100) loops dequeuing work items and invoking their `run()` -- this is where `ArkMediaPlayer::play()` actually executes, on the `WorkQueue`'s own thread pool, not `GalReaderThread`.

**Settled, not inferred**: `GalReaderThread` cannot be stalled by a slow/blocked ALSA write -- the moment audio data reaches `dataAvailableCallback`, it's handed to a genuinely separate worker thread and the reader thread returns immediately to read the next socket message. This rules out "decryption/reader-thread jitter" as a contributor to the audio-specific symptom by direct evidence, not just the video-doesn't-glitch inference that motivated checking this in the first place.

**Redirects the remaining investigation**: since network arrival is now confirmed structurally decoupled from audio processing, the real bottleneck -- whatever it is -- has to live *inside* the `WorkQueue`/`AudioPlaybackWorkItem`/`ArkMediaPlayer::play`/`AlsaHandle::play` chain itself. Either the `WorkQueueThread`(s) draining this specific queue are themselves getting starved of CPU time (which would make the `chrt -f 50` mitigation already in place a much more targeted fix than a blind guess, rather than an unrelated shot in the dark), or there's a pacing/backlog issue once items are already queued, independent of network health. `WorkQueue`'s thread count/sizing and whether `ArkMediaPlayer::play()` itself has any internal blocking/pacing logic are the natural next things to check if the current mitigations don't resolve it.

### Two practical mitigations implemented, neither a restoration of stock behavior (main repo commit `47aa357`)

1. **`chrt -f 50`** prefixed onto `sink`'s launch (`firmware_overlay/usr/share/dbus-1/services/com.arkmicro.auto.service`) -- real-time `SCHED_FIFO` priority 50 for `sink` and every thread it spawns (threads inherit parent scheduling policy via `pthread_create()`, so this covers all of them without needing to enumerate/attach to individual thread IDs). Checked directly against the real stock rootfs dump's own service file first: bare `Exec=/usr/bin/sink`, no priority wrapping at all -- and `sink`'s own binary (same Holden-sourced binary on both builds) never calls `sched_setscheduler`/`pthread_setschedparam` itself either. **Not a stock behavior match** -- a genuine deviation, a scheduling-priority experiment covering several remaining open theories (CPU contention, IRQ-disable windows, generic starvation) at once regardless of which is real.
2. **`net.core.busy_poll=50` / `net.core.busy_read=50`** sysctls added early in `firmware_overlay/etc/rc.d/rcS`. Makes the kernel busy-poll for incoming socket data system-wide instead of waiting purely on interrupt-driven wakeup, with zero changes needed to `sink` (already confirmed it never calls `setsockopt(SO_BUSY_POLL)`). **Also not a stock behavior match, architecturally impossible on stock** -- `CONFIG_NET_RX_BUSY_POLL` didn't exist until Linux 3.11; stock runs 3.4.

Both are explicitly framed as mitigation attempts, not confirmed fixes -- if either resolves the audible symptom, it tells us "this compensates for *something*," not "this is what stock secretly does." Builds/deploys via the normal rootfs pipeline, not yet hardware-tested.

### `ArkMediaPlayer::setup()` fully traced -- `AlsaHandle`-variant selection is a dead end, closes out the remaining static-analysis avenue (2026-08-02)

Following on from tracing `ArkMediaPlayer::play()`'s `flag@+40` dongle-vs-local-playback switch (see above, ruled out as taken on this system), traced its sibling `ArkMediaPlayer::setup(int,int,int,int)` (0x3079c in `sink`) to completion, resolving the two open threads flagged at the time: what the `mode` parameter (setup's 2nd arg, compared against `3`/`1`/`2`) actually selects, and what the `getRegValue()`-based `flag@+41` tail does.

**`mode` selects one of three literal ALSA PCM device strings** (resolved directly from the binary's `.rodata`, PC-relative literal-pool loads at 0x30a80/0x30ad0/0x30b20): `mode==3` -> `"plug:softvol2"`, `mode==1` -> `"plug:softvol1"`, `mode==2` -> `"plug:softvol4"`. Any other value (including the uninitialized-heap default of effectively-anything, e.g. `0`) constructs **no** `AlsaHandle` at all -- `play()`'s own null-check (`ldr r3,[r3]; cmp r3,#0; beq ...` at 0x30e98) then makes every subsequent call a silent no-op, no crash, no output, no XRUN line. `mode` itself is not a per-call/per-frame value -- it's read from the outer `LinuxAudioSink` object's own offset+24 field, set once during `LinuxAudioSink::init()` (0x1c3f8, the `bl 3079c` call site), so it can't be an intermittent-per-frame cause even in principle.

**Checked what `softvol1`/`softvol2`/`softvol4` actually differ by in `/etc/asound.conf` -- dead end, closes this thread.** All of `softvol`/`softvol1`..`softvol5` are thin `type softvol` wrappers, and `softvol1`/`softvol2`/`softvol4` all share the *identical* slave: `pcm "dmix"` (`period_size 1024`, `buffer_size 16384`, `slave.pcm "hw:0,0"`). They differ **only** in `max_dB` gain ceiling (`10.0` / `-0.0` / `5.0` respectively) and their own separate ALSA control name (`softmaster1`/`2`/`4` -- per-audio-source-type volume slider isolation, not a buffering/timing difference). **No difference whatsoever in period size, buffer size, sample rate, or underlying DMA path between the three variants** -- whichever `mode` this system actually uses is irrelevant to the stutter mechanism.

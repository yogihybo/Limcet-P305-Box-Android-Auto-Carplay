...
**Real, hardware-confirmed correction (2026-08-31)**: on this specific unit, the stock
app's own on-screen MCU Monitor showed **nothing** — but a direct UART sniff of
`/dev/ttyHS0` (`tools/mcu-handshake`, external to the stock app) during the exact same
button presses captured real, checksum-verified `CMD 0x02` key-event frames (real
button-press-to-frame correlation for Vol+/Vol-/Answer/Hangup, see
`docs/MCU_COMMAND_REFERENCE.md`). **The MCU genuinely is decoding CAN and sending real
key events** — the stock app's own MCU Monitor screen is the broken/misleading part here,
not the CAN→MCU→UART pipeline. Treat "MCU Monitor shows nothing" as inconclusive on this
hardware, not as "no change at all → CAN frames not reaching the MCU decoder" per the table
above — verify with an external UART capture before assuming a wiring/CAN-ID problem.

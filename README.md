...
| `run sdboot` | Boots kernel/rootfs from the SD card p1/p2 (`uboot_final.bin` only) — see [SD boot](#sd-boot) |
| `run usbboot` | Boots kernel/rootfs from a USB drive (`uboot_final.bin` only) — see [USB boot](#usb-boot) |

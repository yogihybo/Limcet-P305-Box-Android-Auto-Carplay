...
**Current state, top to bottom:** basic audio playback works (root-caused and fixed the initial
`SoundAdapter` crash and the missing kernel driver chain). The Android Auto audio-stutter symptom
that dominates the second half of this doc is **resolved** — the real fix was at the kernel/driver
level (DMA priority, tasklet, MUSB revert — see "RESOLVED" near the end), not any of the userspace
scheduling mitigations tried and reverted along the way. BD37033's I2C control path remains a
confirmed, benign, present-on-stock-too quirk — see `docs/1.6_BD37033.md`.
...
**Yes — a BD37033FV audio-processor IC is physically present on the board, confirmed by
chip marking on close inspection.** Arkmicro's own ARK1668E reference block diagram
(`http://www.arkmicro.com.cn/uploadfiles/2021/07/20210714150417532.jpg`) had suggested
otherwise ("SDDAC direct to amp," `AUDIO` running straight from the SoC to a `Power AMP.`
block with nothing in between) — wrong for this variant, the same class of mistake as the
earlier GT911 bus assignment.

An initial `CONFIG_SND_SOC_BD37033_NOP` stub build (every I2C register read/write silently
returns 0, satisfying the app's software requirement without any real hardware behind it)
got `SoundAdapter Create` to succeed with no crash, but the boot-time speaker "pop" (a
normal DAC power-on transient present in earlier builds) disappeared under it, suggesting
something downstream was sitting muted — consistent with a real chip existing that the stub
wasn't actually driving. `CONFIG_SND_SOC_BD37033_NOP` is disabled so the driver talks to the
real chip.
...
Edited in `firmware_source/mtd6_rootfs/rootfs/msnprofile/MsnProductInfo.ini`.
...
**Bus placement and the I2C failures — final state:** `i2c-gpio-0`'s SCL/SDA (GPIO2/GPIO3)
and `i2c-gpio-1`'s SDA (GPIO9) are pin-mux-conflicted with LCD RGB888 data lines — a real,
now-fixed hardware/driver issue documented in full in `docs/1.7_DISPLAY_SUBSYSTEM.md` (`i2c-gpio`
permanently steals those pads to GPIO at boot; fixed by a kernel-level pinmux reclaim,
hardware-confirmed). BD37033's real, correctly-wired bus is `i2c-gpio2` / this project's own
`i2c-gpio-1` (`sda_pin=9, scl_pin=121`, address `0x40` — not `i2c-gpio-0`, where an earlier
session had mistakenly moved it based on misreading `i2c-scan`'s `XX` marker as a live bus ACK
rather than what it actually means: `ioctl(fd, I2C_SLAVE, addr) < 0`, i.e. only that a kernel
driver is bound to that address, not that the chip answers). `drv_bd37033` now sits on the
correct bus. **The `bd37033_write_byte timeout` message itself, however, is not fixed by any of
this** — it's a separate, confirmed-benign quirk present identically on real stock hardware's own
factory-correct wiring (see "Root cause found" and the live re-test sections below): the chip's
very first probe-time write appears to race its own power-up sequence, in the vendor's own
shipped firmware. rn6752 stays on `i2c-gpio-0`/GPIO2-3 — confirmed to be its real bus on stock
hardware too, LCD sharing and all (the recurring `rn6752_eq_work reset` workaround is a likely
symptom of that sharing, not a sign the bus is dead).
...
not just asserted.** `docs/1.7_DISPLAY_SUBSYSTEM.md` (~line 217)
...
software view.** Traced the exact register from `pinctrl-ark.c`: our board's
`compatible = "arkmicro,ark1668-pinctrl"` takes the hardcoded-map branch
(`ark1668_pin_map[]`), not the DT-configurable `"arkmicro,arke-pinctrl"` branch
that a generic `PIN_REG_OFFSET`/`PIN_BIT_OFFSET` formula would apply to — instead,
array index N is pin N directly, each entry an explicit `{reg, offset, mask}`.
Pin 9's real entry is **`{0x1c0, 28, 0xf}`** — a **4-bit** field at bits `[31:28]`
of register `0xe4900000 + 0x1c0 = 0xe49001c0`.

Mux values (`include/dt-bindings/pinctrl/ark-pinfunc.h`): `ARK_PVAL_1 = 1` is
`lcd-rgb-0` (what `r7` is set to). Also found a **third, unused pinctrl state** in
the vendor's reference DTS, `pinctrl_lcd_hi_impedance`/`lcd-hi-impedance`
(`ark1668e-pinctrl.dtsi`), which sets the same pins to `ARK_PVAL_5` — an explicit
tri-state/hi-Z mode for exactly this class of pin, though not wired up as a
selectable state anywhere in this board's own DTS.

Built and verified a small tool for this, `tools/pin-dump/pin-dump.sh` — reads
every one of the 131 pins' live mux value via `devmem`, using this exact table
extracted from `pinctrl-ark.c`. Sanity-tested locally against a stub `devmem`
(pin 9 correctly decodes to `PVAL=1`) before running on real hardware.
...
assumption was wrong for this board. `docs/1.9_KERNEL_REFERENCE.md`
...
was built on that log line plus a `1.9_KERNEL_REFERENCE.md` module-table
...
Checked `firmware_source/mtd6_rootfs/rootfs/
...
Patched `firmware_source/mtd6_rootfs/rootfs/usr/lib/libSetting.so`
...
input ever received**. See [`docs/MSN_APP_ARCHITECTURE.md`](MSN_APP_ARCHITECTURE.md) band 03
...
**Resolved as not-a-bug: the deployed `sink`/`libAndroidAuto.so`/`libAutoDongle.so` copies are
genuinely Holden-sourced, and that's correct — leave as-is.** An initial pass suspected these
files had drifted from the real Limcet P306 dump and swapped in Limcet P306-dump copies
(commit `74ec4c9`) — but a follow-up check (extracting `firmware_dumps/Holden firmware
update/rootfs.img` via `ubireader_extract_files` and md5-comparing) confirmed the originally-deployed
files are genuinely, byte-identically sourced from the Holden firmware, not corruption or an
unknown build. That's expected: Holden and Limcet P306 share close enough lineage that the Holden
binary runs without any obvious error on a Limcet P306 image, and there was never a documented
reason this path should differ from stock. The swap was reverted (revert commit `289c750`);
`firmware_source`'s copies are back to the Holden-sourced binaries and this is not part of the
active investigation. Any static analysis done against the intermediate Limcet P306-dump swap
(the pipeline trace, the read-chunk-size finding, both below) was performed against the *wrong*
binary for a portion of that session — worth keeping in mind if revisited, though the deployed
binary is now confirmed correct.
...
`docs/1.4_WIRELESS_AND_INIT.md`, for a concrete instance where this
...
`docs/1.4_WIRELESS_AND_INIT.md` for the full wired-AA context). Not yet
hardware-tested.
...
See `docs/1.11_VENDOR_BSP_RESEARCH.md` §5 for the full writeup. Two points worth recording
here directly:
...
`docs/1.11_VENDOR_BSP_RESEARCH.md` §5.
...
## RESOLVED (2026-08-05): real fix was at the kernel/driver level, not userspace scheduling

The `chrt -f 50` / `ksoftirqd` priority / RT-throttle-disable / TCP-reno mitigations above were
hardware-tested together and confirmed **not sufficient on their own** — all four were reverted
back to stock. The actual, hardware-confirmed fix landed at the kernel/driver level instead:
DMA channel priority over eMMC, a cyclic-DMA tasklet on the HI softirq, a genuine vendor MUSB
regression revert, and bypassing the generic `dmaengine_pcm` framework (`linux-arkmicro` commits
`7e6d93657`/`886e8b56c`/`c195cb9de`/`de1e2adec`) — isolation-tested to work without any userspace
scheduling changes. The XRUN debug activation (`/proc/asound/card0/pcm0[pc]/xrun_debug`) stays as
general-purpose diagnostic infrastructure; everything else from the mitigation era (RT throttle
disable, `ksoftirqd/0` priority elevation, TCP reno override, the `chrt -f 50` wrap on
`com.arkmicro.auto.service`, and the diagnostic log poller that watched for RT-throttle
recurrence) has been removed from `rcS` and the service file, both restored to stock.

## OPEN HYPOTHESIS (2026-08-17): factory/OEM mic vs. aftermarket mic may be two genuinely
## separate physical inputs, gated by the same `SoundType` setting that already breaks one
## or the other

User-reported symptom on **stock firmware**: the aftermarket mic (bundled with this
head unit) works fully. The vehicle's factory/OEM mic acknowledges the "OK Google"
wake phrase but never actually captures/streams the spoken command afterward.

**Not yet hardware-confirmed** — this section records reasoning from existing evidence
in this doc, `docs/1.1_HARDWARE_AND_SOC_REFERENCE.md`, and this project's own memory
(`project_mic_capture_investigation`), not a new live test. No continuity trace of the
actual OEM-mic harness wire has been done.

**Step 1 — "OK Google" acknowledging doesn't necessarily mean the OEM mic circuit is
doing anything.** Android Auto/Assistant hotword detection normally runs on the
*phone's* own always-on microphone, independent of the head unit — the car mic is only
engaged once the phone opens a capture session with the head unit for the actual
spoken command that follows. So the wake-phrase working is consistent with the OEM
mic input carrying zero real signal the whole time.

**Step 2 — "session opens but nothing is actually captured" already has a precedent in
this exact investigation.** The 2026-08-03 mic-capture work (`project_mic_capture_investigation`
memory, `SoundType=0` config -- the aftermarket/direct path) found the capture stream
opens and closes *cleanly* at the ALSA/kernel level (`ark1668-i2s: trigger cmd=1
stream=1 (capture)` through a clean stop, `Microphone reader thread started`/`exit`,
zero errors) -- but whether real audio data was actually flowing through that
clean-looking session was never conclusively confirmed, and was left as an open,
unclassified item. The OEM-mic symptom described here (listens, but the command is
never captured) is the same shape.

**Step 3 — a real, hardware-grounded reason the two mic inputs could depend on
mutually-exclusive driver config.** From this doc and the hardware reference doc:

- The SoC's own SAR-ADC (`ark1668-sdadc-codec`) exposes exactly one `MICIN` pin,
  driven directly by `ark1668_i2s.c`'s capture path (`MIC_LINE_SEL`, SARADC power-up,
  RX DMA) -- independent of `SoundType`. This is very plausibly what a simple bundled
  aftermarket mic plugs into.
- BD37033 (`Sound_BD37033`, `SoundType=3`) is a real, populated chip on this board
  (confirmed I2C-present, see `docs/1.7_DISPLAY_SUBSYSTEM.md`'s `i2c-gpio-1` pinmux
  work) with its own multi-input analog selector (`bd37033_select_input_channel()`,
  8 real input options: A/B/C/D/E1/E2 single, D_DIFF, E_FULL_DIFF). A vehicle-integrated
  OEM mic is a plausible candidate for one of BD37033's own input channels, distinct
  from the SoC's direct `MICIN` pin.
- This device currently runs `SoundType=0` ("no adapter constructed" -- confirmed the
  only value that lets the direct/aftermarket mic path work end-to-end,
  `project_mic_capture_investigation` memory). Under `SoundType=0`, BD37033's own
  driver/init path never runs at all -- `bd37033_select_input_channel()`/unmute never
  execute, so whatever is wired to BD37033's inputs stays in whatever state the chip
  powers up in (very plausibly muted/unrouted).
- `SoundType=3`/`5` are separately documented (`docs/1.10_SETTINGS_REFERENCE.md` §`SoundType`)
  as **breaking** the direct/aftermarket mic path when active -- mechanism not fully
  understood, but the practical effect already observed is that this driver stack
  appears to only support ONE of the two mic inputs actively routed at a time, not both
  simultaneously.
- The reference schematic's separate, dedicated hands-free "Voice Processor" chip
  (`ARK1668 -> Voice Processor (SPI+I2C+I2S) -> MIC/BT`, `docs/1.1_HARDWARE_AND_SOC_REFERENCE.md`
  "Audio chain") is confirmed NOT populated on this board (no SPI controller node, no
  boot-log SPI probe) -- ruled out as the OEM mic's real path, since that would predict
  *no* signal at all, not a signal that gets far enough to matter for a wake-word
  acknowledgment (which per Step 1 doesn't require any car-mic signal in the first
  place, so this doesn't actually rule anything in or out on its own).

### Confirmed Microphone Hardware Architecture & Analog Input Multiplexer

Through deep firmware disassembly of `libMcuCenter.so` (`MCUAdapter_ZhongHang::syncSettingDataToMcu` / `0x0009B220`) and the STM32 MCU firmware (`hardware/MCU/can_app.bin` at `0x080089E2` & `0x08005AA0`), the exact microphone hardware and switching mechanism has been resolved:

1. **Onboard 2:1 Analog Multiplexer IC**:
   * The board contains an analog 2:1 multiplexer switch (e.g. SGM3157 / TS5A3159) gated by the **STM32 MCU's `GPIOB Pin 6`** (`0x08005AA0`).
   * **`GPIOB Pin 6 = 0` (Default / Aftermarket)**: Connects the external 3.5mm female pigtail jack directly to the ArkMicro SoC's internal SAR-ADC (`ahb:sdadc@0` / `MICIN` pin with $3.3\text{V}$ bias).
   * **`GPIOB Pin 6 = 1` (OEM Factory Mode)**: Disconnects the 3.5mm jack and routes the vehicle's 28-pin radio harness microphone input (`MIC+` Pin 4 / `MIC-` Pin 5) into the SoC `MICIN` pin.

2. **The Wire Protocol Command**:
   * Changing microphone source sends a `CMD 0xA0` (Setting Sync) packet to the MCU over `/dev/ttyHS0`:
     $$\text{Packet: }\texttt{[0x2E] [0xA0] [0x02] [0x09] [value] [CHK]}$$
   * `Setting ID 0x09`: Value `0x00` = Aftermarket 3.5mm mic, `0x01` = Toyota OEM Factory Roof Mic.

3. **Active Power Requirement for Factory Toyota Roof Mic (`MACC` +5V)**:
   * The Toyota Prado 150 factory roof console microphone is an active electret microphone with an integrated preamplifier circuit.
   * It requires **$+5\text{V}$ (or $+8\text{V}$) DC power** on **Pin 6 (`MACC`)** of the 28-pin Toyota radio connector.
   * If the aftermarket piggyback harness does not provide $+5\text{V}$ to the `MACC` pin, the factory roof microphone remains unpowered and silent regardless of multiplexer state.

4. **Software UI Integration**:
   * An interactive toggle `OEM Factory Microphone` is integrated into `custom_ui` (`Settings -> AUDIO`), saved under `[Audio] OEMMicrophone=true/false` in `/data/custom_ui/settings.conf`, and synced to the MCU on every boot.

### 2026-08-28 Live Hardware Resolution: Audio Power Gating & BD37033 Architecture

Through live memory inspection of the running STM32 companion MCU (`Limcet-V1.0-1302`) and disassembly of the live firmware dumps (`live_app_1302.bin`), the complete hardware audio routing has been resolved:

```mermaid
graph TD
    subgraph "ArkMicro Linux SoC (/dev/ttyHS0 @ 38400)"
        SoC["ArkMicro SoC (custom_ui / Linux 4.19)"]
        DAC["On-SoC Stereo DAC (ahb:sdadc / plughw:0,1)"]
    end

    subgraph "STM32F105 Companion MCU (Limcet-V1.0-1302)"
        MCU["STM32F105 MCU"]
        I2C2["Hardware I2C2 (0x40005800)<br>PB10 (SCL) / PB11 (SDA)"]
        GATES["Discrete Power & Mux Switches<br>PA1 (PA_MUTE), PA7 (AUDIO_EN), PB6 (MIC_MUX)"]
    end

    subgraph "Vehicle Audio Subsystem"
        BD["ROHM BD37033 5.1ch DSP (if populated)"]
        AMP["TDA7388 4-Channel Power Amplifier"]
        SPK["Vehicle Speakers"]
        MIC["Analog Microphone Mux (OEM vs 3.5mm)"]
    end

    SoC <-->|Serial Protocol (CMD 0x84 / 0xA0)| MCU
    DAC -->|AC-Coupled Analog Stereo| AMP
    AMP --> SPK
    MCU -->|I2C2 Control Bus| BD
    MCU -->|PA1 / PA7 Discrete Gates| AMP
    MCU -->|PB6 Multiplexer Gate| MIC
```

#### **1. Dual Power Gating Sequence**
* **MCU Power Rail (`GPIOA Pin 7`)**: Controls the $+8.5\text{V}$ analog power rail for the audio section. Must be energized by sending `CMD 0x84 [0x00, 0x03]` over `/dev/ttyHS0`.
* **Power Amplifier Mute (`GPIOA Pin 1`)**: Held in mute ($0\text{V}$) on cold boot until `CMD 0x84` is received, preventing power-on pops and thumps.
* **SoC Standby Gate (`GPIO 34`)**: Must be driven **LOW (`0`)** by Linux to enable the on-board P-FET/transistor power switch.

#### **2. BD37033 I2C Bus Destination Resolved**
* **Bus Routing**: On this head unit PCB, the BD37033's I2C control pins (`SDA`/`SCL`) route directly to the **STM32 companion MCU (`I2C2` on `PB10`/`PB11` at base `0x40005800`)**, NOT to the ArkMicro SoC.
* **MCU Engine**: The live MCU firmware (`live_app_1302.bin` at `0x0800B416`..`0x0800B490`) contains the complete volume, 3-band EQ, and fader translation engine, writing directly to BD37033 at I2C address `0x40`.
* **Linux SoC Role**: The Linux host never needs to communicate with the BD37033 directly; it sends high-level setting frames (`CMD 0xA0 [id, value]`) over `/dev/ttyHS0`.

#### **3. Confirmed ALSA Device Mapping & Physical Audio Path**

Live hardware execution (`cat /proc/asound/pcm`, `aplay -l`, `arecord -l`) confirmed the active ALSA PCM interfaces:

| Audio Function | ALSA Device | Hardware Entity | Physical Role |
|---|---|---|---|
| **Audio Playback (Output)** | **`plughw:0,0`** | `card 0, device 0` (`e4000000.sddac`) | ArkMicro internal stereo DAC $\rightarrow$ Direct AC-coupled line-out $\rightarrow$ Toyota OEM Radio/Amplifier AUX inputs |
| **Microphone Capture (Input)**| **`plughw:0,1`** | `card 0, device 1` (`ahb:sdadc@0`) | ArkMicro internal SAR-ADC (`MICIN`) $\leftarrow$ Gated by 2:1 multiplexer (`GPIOB Pin 6`) |

#### **4. Direct DAC Output & Software Volume Control Architecture**

Live testing on the physical Prado 150 vehicle established the real-world audio pathway:

```mermaid
graph LR
    subgraph "ArkMicro Linux SoC (Kernel 4.19)"
        PCM["PCM Audio Stream (custom_ui / Android Auto)"]
        VOL["Software Volume & EQ Scaling"]
        DAC["Stereo DAC (plughw:0,0)"]
        PCM --> VOL --> DAC
    end

    subgraph "Vehicle Harness & Factory Audio"
        OEM["Toyota Prado OEM Radio / Factory Amp (AUX In)"]
        SPK["Vehicle Speakers"]
        DAC -->|"Direct Analog Stereo (AC-Coupled)"| OEM --> SPK
    end

    subgraph "STM32 Companion MCU Role"
        MCU["STM32 Companion MCU (Limcet-V1.0-1302)"]
        MCU -.->|"CAN Bus / SWC Buttons / Touch / Video Mux / Mic Switch"| OEM
        MCU -.-x|"No Inline Analog Attenuator"| DAC
    end
```

* **Physical Verification**: Audio playback through `aplay -D plughw:0,0 /usr/share/sounds/test_tone.wav` immediately outputs crystal-clear sound through the vehicle's speakers.
* **Why MCU Volume/Power Commands Do Not Attenuate Playback**: On this Toyota Prado piggyback unit (`SoundType=0`), the DAC outputs directly to the vehicle's OEM radio AUX input. The STM32 MCU does not insert an analog attenuator into the DAC output lines.
* **Software-Driven Audio Processing**: All volume adjustments, mute toggling, and equalizer processing are performed **in software on the Linux SoC** prior to writing samples to `plughw:0,0`.
* **MCU's Real Hardware Responsibilities**:
  1. **CAN Bus Integration**: Steering wheel controls (`0x3C4`), reverse gear detection, and vehicle illumination.
  2. **Touchscreen Control**: Resistive touchscreen digitizer pass-through switch (`GPIOB Pin 0`).
  3. **Video Multiplexing**: OEM camera bypass relay (`GPIOC Pin 13`) and CVBS video mux (`GPIOC Pin 2`).
  4. **Microphone Multiplexer**: Switching between the OEM roof console mic and the aftermarket 3.5mm jack (`GPIOB Pin 6` via `CMD 0xA0 [0x09, val]`).

#### **5. Dedicated Diagnostic Tools**
* **[`tools/mcu-audio-test`](file:///home/osboxes/Downloads/prado-firmware-reconstruction/tools/mcu-audio-test/)**: Statically linked diagnostic daemon for testing power gating (`--power 1`), microphone mux switching (`--mic oem`), volume stepping (`--volume 25`), and EQ sweeps (`--all`).
* **[`tools/bd37033-test`](file:///home/osboxes/Downloads/prado-firmware-reconstruction/tools/bd37033-test/)**: Updated with `--mcu-scan` mode to test all I2C buses under active MCU audio power gating.
* **Test WAV Files**: Staged in `/usr/share/sounds/test_tone.wav` ($440\text{ Hz}$ sine) and `/usr/share/sounds/stereo_test.wav` (Left/Right separation).

...
| `0x05` | not resolved at byte level. **Confirmed (2026-08-31) by direct trace**: handler (`0x0003D8D8`) is structurally near-identical to `CMD 0x04`'s confirmed handler — same `MsnEvent` construction pattern, same `QByteArray` alloc, same two-bucket range-check shape on the first payload byte — and, like `0x04`, it feeds a payload byte (`payload[1]` here, vs. `0x04`'s `payload[0]`) directly into the same real function, `transRadarLevel(unsigned char)` | ⚠️ **CONFLICT — two competing theories, unreconciled** | Real disassembly leaves the byte-level meaning deliberately open either way (Qt event type `0x5018`, distinct from `0x04`'s `0x5019`). **Theory A (HVAC/climate)**: a separate report proposed dual-zone temperature/fan-speed telemetry, citing the same `MsnEvent` dispatch and speculating it reaches `AirConditionDlg` — plausible given `AirConditionDlg` is a real, confirmed consumer elsewhere in this binary (see `CMD 0x03`), but no direct call from `0x05`'s own handler into any `AirConditionDlg` setter was found, unlike `0x03`'s handler where that link is real and traced. **Theory B (radar-adjacent)**: the shared `transRadarLevel()` call with `0x04` (a "high"-confidence, real-function-name radar finding) is real, concrete, structural evidence that `0x05` may be a second radar/level-style field (e.g. a paired sensor zone) rather than climate data — a temperature value being run through the same transform as a confirmed distance byte is a better structural fit for "another radar field" than for HVAC. **Neither theory has live-capture support**: `MCUAdapter_BoxP300` is independently confirmed to be the *active* adapter class for this exact hardware (`McuType=6`), so this isn't dead code for the wrong vehicle — but that only means the code path exists and could run, not that the real Prado MCU ever actually sends `CMD 0x05`. Zero captures of this command exist in this project's history; `custom_ui` doesn't handle it. Genuinely possible this command simply never fires on real Prado hardware regardless of which theory is right |
| `0x06` | `payload[0]` via `QBitArray`. **Confirmed by direct trace (2026-08-31)**: bits `4`, `5`, `6`, `7` are individually extracted (`ubfx`/`asrs`) and combined into a single packed state value — passed as one 64-bit `MsnEvent` param, not four independent booleans as a simple per-bit table would imply. **Bit *positions* confirmed real; bit *meanings* are not** — no named function call or string evidence ties bit 4/5/6/7 to any specific signal, unlike e.g. `CMD 0x04`'s `transRadarLevel()` citation. A separate report's labels (bit4=Parking Brake, bit5=Footbrake, bit6=Turn Signals/Hazard, bit7=Reverse/Transmission) are recorded here as the best current guess, **to be confirmed** — same evidentiary weight as this project's own earlier unverified `MCU_CMD_REVERSE_GEAR` guess for this same command, not independently stronger | 🟡 **Vehicle dynamics/safety bitfield — bit positions confirmed, meanings unconfirmed** | Real dispatch (`0x0003BB24`→`0x0003DAD8`) and real event construction independently re-verified (`MsnEvent(type=0x501A)` to app id `0x190`/400, matching `movw r1,#0x190` — real, not previously nailed down this precisely). `custom_ui`'s own `MCU_CMD_REVERSE_GEAR` clean-room guess for this byte remains unreconciled with any of the above |

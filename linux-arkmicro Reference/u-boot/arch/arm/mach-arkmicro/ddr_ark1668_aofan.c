...
void ddr3_sdramc_init(void)
{
	...
	rDDR_CCR = 0x1<<2|0x01<<17 ;    //0x20104;
#if 1
	rDDR_DLLCR0 |= (1<<5)|(1<<11)|(0x1<<15); //phase detect forward adjust middle back adjust middle phase 0x01 means 72 0x00 means 00
	rDDR_DLLCR1 |= (1<<5)|(1<<11)|(0x1<<15);//phase detect forward adjust middle back adjust middle phase 0x01 means 72 0x00 means 00
	rDDR_DLLCR9 |= (1<<11); //cmd line dll back adjust middle
	rDDR_DQTR0  =0x55555555; //add 1 step
	rDDR_DQTR1  =0x55555555; //add 1 step
	rDDR_DQSTR  =0x12;  //sub 1 step
	rDDR_DQSBTR =0x12;  //sub 1 step
#endif
	rDDR_MR = (0x5<<9)|(0x1<<8)|(0x3<<4);   //MR0   BURST=8 CL=7 WR=10
	rDDR_EMR = (0x1<<6)|(0x3<<1);           //MR1  AL=0 CWL=6
	rDDR_EMR2 = (0x1<<9)|(1<<3);          //MR2 CWL=6
	rDDR_ZQCR0 =0x1<<31;
	udelay(50000);
	rDDR_IOCR =(0x3<<30)|(0xf<<7)|(0x0<<3)| (3<<0);

	rDDR_TPR0 = 0;
//	rDDR_TPR0 |= (3<<0)|(5<<2)|(5<<5)|(8<<8)|(8<<12)|(20<<16)|(6<<21)|(26<<25);
	rDDR_TPR0 |= (2<<0)|(6<<2)|(6<<5)|(11<<8)|(7<<12)|(18<<16)|(4<<21)|(24<<25);
	rDDR_TPR1 = 0;
//	rDDR_TPR1 |= (0x1<<2)|(30<<3)|(1<<11);
	rDDR_TPR1 |= (0x0<<2)|(30<<3)|(2<<12)|(2<<14);
	rDDR_TPR2 = 0;
	rDDR_TPR2 |= (1000<<0)|(30<<10)|(5<<15);
	rDDR_TPR3 = 0;
	rDDR_TPR3 |= (2<<0)|(7<<3)|(6<<7)|(10<<11);  //BURST LENGTH 8

//1G bit
	rDDR_DCR = 0x1<<24|(0x0<<14)|0x1<<7|0x4<<4|0x2<<2|0x1;
//2G bit
//	rDDR_DCR = 0x1<<24|(0x0<<14)|0x1<<7|0x5<<4|0x2<<2|0x1;
	printf("ARKSDLOAD 1Gb 20170322_7-6-10\r\n");
#endif
}

...
| `0xA0` | `syncSettingDataToMcu` (`0x38f5c`) | 2 bytes: `[settingId, value]` | **UI Settings Sync Channel** (`TBB` jump table in MCU): Setting IDs `0x00` (Display Mode), `0x07` (Dynamic Guidelines), `0x08` (Buzzer), `0x09` (SWC Mode), `0x0A` (Rev Vol Cut), `0x0B` (Radar PDC), `0x0C` (Radar Timeout), `0x0D` (Front Camera), `0x0F` (Brightness), `0x10` (Contrast), `0x11` (Camera Type: 0=Aftermarket, 1=Factory Relay) | high (fully disassembled and verified) |
| `0xE1` | `onSendUpdateReadyTimer` (`0x36720`) | 2 bytes | **Reboot to Bootloader**: writes `0x5555AAAA` to SRAM and triggers watchdog reset | high (verified in `can_app.bin` `0x080088E0`) |

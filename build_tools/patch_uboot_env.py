#!/usr/bin/env python3
"""
build_tools/patch_uboot_env.py - RELOCATE the compiled-in default environment in an
ARK1680 U-Boot binary so a full-size boot command fits entirely in the binary.

Why this exists (vs. build_tools/patch_uboot.py)
------------------------------------
build_tools/patch_uboot.py can only *edit* the compiled-in env in place. On a raw NAND
...
So instead of growing the env in place we:
  ...
  4. (optional) apply the same NAND-env-offset corruption as build_tools/patch_uboot.py so
...
--------
  ...
  python build_tools/patch_uboot_env.py -i uboot.bin --analyze

  # Relocate a full SD-boot env + corrupt NAND offset (recommended)
  python build_tools/patch_uboot_env.py -i uboot.bin -o uboot_relocenv.bin \
  ...
  python build_tools/patch_uboot_env.py -i uboot.bin -o out.bin --keep-original \
  ...
  python build_tools/patch_uboot_env.py -i uboot.bin -o out.bin --preset sdboot \
      --region-offset 0x50fdd --patch-nand-offset

  # Dry run
  python build_tools/patch_uboot_env.py -i uboot.bin --preset sdboot --patch-nand-offset --dry-run
"""
...
# ===========================================================================
# NAND env offset corruption (same technique as build_tools/patch_uboot.py)
# ===========================================================================

...
| `0x82` | `showApp` (`0x36028`), only when `appId==0xCC` (204) | 4 bytes, a value read from a fixed struct offset (`[r3+20]`) | notifies MCU when app `0xCC` is shown; no app-ID table exists yet in this repo to say what app `0xCC` is (worth checking against [`MSN_APP_ARCHITECTURE.md`](MSN_APP_ARCHITECTURE.md) / a live `showApp` trace) | med |

#!/bin/bash
# restore_rootfs_symlinks.sh — recreate the rootfs symlinks that a Windows
...
#
# The manifest (`rootfs.symlinks`) is `linkpath<TAB>target`, one per line, paths
...
#
# Usage: restore_rootfs_symlinks.sh <rootfs-dir> [manifest]
set -e

ROOT="${1:?usage: restore_rootfs_symlinks.sh <rootfs-dir> [manifest]}"

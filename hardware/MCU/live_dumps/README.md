...
>
> **Also corrected (2026-09-12):** the real factory bootloader's own app-validation code (disassembled at `0x08001844`) checks for the application vector table at **`0x08003000`**, not `0x08004000` -- see `docs/MCU_FIRMWARE_VERIFIED_FINDINGS.md` §5.2. **Update**: the sweep this note originally flagged as pending is done — the "Properly Aligned Flash Partitions" table below already uses the corrected `0x08003000` boundary, and the clean-room bootloader/application source (`hardware/MCU/bootloader/`, `hardware/MCU/source/`) was corrected to match in a later commit. The "Original Raw Extractions" table below still shows `0x08004000` for `live_app_1302.bin` deliberately — that's an accurate label for where that specific raw file's own bytes actually start (confirmed empirically, see §14), not a stale claim about the real application base.
...
| [`factory_app_1302_52k.bin`](file:///home/osboxes/Downloads/prado-firmware-reconstruction/hardware/MCU/live_dumps/factory_app_1302_52k.bin) | `0x08003000` – `0x0800FFFF` | 52 KB (53,248 B) | `577f976fa96dd738ba65e597b06416cd4872ec38cbec3125ba736d0cfa3258b8` | Complete Native **Limcet-V1.0-1302** Application. **Correction (2026-09-12)**: the first two words (`0x08003000`/`0x08003004` — the vector table's own Initial-SP and Reset entries) read as `0xFFFFFFFF`, not real captured values — see `docs/MCU_FIRMWARE_VERIFIED_FINDINGS.md` §18 before describing this file as including a complete vector table. The real vector table's remaining entries (NMI onward, `0x08003008`+) are present and real. |
| [`factory_full_64k.bin`](file:///home/osboxes/Downloads/prado-firmware-reconstruction/hardware/MCU/live_dumps/factory_full_64k.bin) | `0x08000000` – `0x0800FFFF` | 64 KB (65,536 B) | `81aa106c7452498b82f86621e12bc3930c9017ed9cd84c989b00db6cafa8968c` | Continuous 64 KB dump of the first half of the chip's real 128 KB flash. **Correction (2026-09-12)**: not "the entire MCU" — the chip's own application-accessible region actually extends to `0x0801FFFF` (116 KB total past the bootloader, confirmed via `docs/MCU_FIRMWARE_VERIFIED_FINDINGS.md` §9's independent 58-page erase-loop trace); `0x08010000`–`0x0801FFFF` (the upper 64 KB) was never captured. Real content in this dump's own application half ends cleanly at `0x0800D437` with a long, consistent run of erased flash (`0xFF`) all the way to this dump's own `0x0800FFFF` boundary — giving real confidence nothing meaningful was cut off by stopping here, but the upper 64 KB remains genuinely unverified, not confirmed-empty. |

...
| `0x01` | `payload[0]` bit `0x02` = on/off — the ONE bit `custom_ui` actually reads; no other bits decoded | 🟡 **Headlights / illumination status** | Live-hardware confirmed via direct behavioral test: **updates in real time when headlights are toggled on the stalk** — a much stronger form of evidence than a byte-parse guess. Independently corroborated by a real captured frame from the *stock* MSN app's own log, `2E 01 06 13 00 00 00 00 00 E5` — `len=6`, `payload[0]=0x13` (`0b00010011`, bit `0x02` set) — matches "Lights ON" exactly (checksum-verified real capture). ~~That same photo was separately re-labeled elsewhere as "Vehicle Status/Version, sub-version 0x1300"~~ — no behavioral evidence supports that reading, it contradicts the direct stalk-toggle test from the same session, and `payload[0]=0x13` is a poor fit for a version number (real MCU version is a 28-byte ASCII string via `CMD 0x7F`, not a 1-byte code). Treated as an unsubstantiated mislabel, not a competing finding. ~~Static disassembly separately guessed "key/button event, type 0x1013"~~ — also superseded |
| `0x02` | `payload[0]`(`b3`)=key code, `payload[1]`(`b4`)=1 press/0 release. **Resolved**: a fuller checksum-verified capture shows real press+release pairs for every code (e.g. `2E 02 02 04 01 F6` then `2E 02 02 04 00 F7`) — the earlier-observed "some codes only show `b4=0`" was an artifact of an abbreviated example listing, not a real per-key difference; `b4` is uniformly press(`1`)/release(`0`). Live-captured codes: `3`,`4`=steering wheel Vol±/Seek *(see CONFLICT in Meaning — was previously labeled Next/Prev Track)*, `8`=Answer,`9`=Hangup,`12`=Home,`13`=Knob push,`64`=Knob CCW,`65`=Knob CW. **Real, likely non-coincidental sub-bit pattern in `b3`** (inferred from the value list, not itself disassembly-confirmed): `8`/`9`, `12`/`13`, and `64`/`65` each share every bit except bit `0` — consistent with upper bits = control group, bit `0` = state/direction. `3`/`4` don't fit that pattern | 🟡 **Knob/button event** | Live-capture confirmed, what `custom_ui` runs on. ~~Originally guessed "handshake/reply builder"~~ — superseded. ⚠️ **Real, unresolved conflict on `b3=3`/`b3=4`'s identity**: `custom_ui`'s existing constants (`kBtnNextTrack=3`, `kBtnPrevTrack=4`) label these as media track-skip buttons. A fresh live capture from the same steering-wheel-button testing instead labels them "Volume Up/Seek" (`4`) and "Volume Down/Seek" (`3`). Both readings come from live testing, not disassembly — **not resolved which is correct**; `custom_ui`'s actual dispatch behavior for these two codes has NOT been changed pending confirmation on real hardware, since relabeling would change real UI behavior |
...
| `0x12` | `byte[4]` read; **no individual bit meaning resolved**. `len=3`, real checksum-verified payload captured: `2E 12 03 01 04 00 E5` → `payload = [0x01, 0x04, 0x00]` | ⚠️ **CONFLICT — two real but unreconciled findings** | Real disassembly leaves this deliberately open (Qt event type `0x5026`). Two separate live-capture sessions give it two different real jobs: (1) this session's fresh init-handshake capture shows `CMD 0x12` firing **once, as part of the startup telemetry burst** immediately after the `0x81/0x82/0x84/0x85` init sequence, with a payload loosely read as "Vehicle Profile / DIP Switches" (`payload[1]=0x04` → "Toyota Prado 150 CAN Matrix Mode" — this specific byte-to-meaning mapping is itself a guess, not disassembly-verified); (2) an earlier live test in this same conversation found `CMD 0x12` correlating with reverse gear *disengaging*. Both are real captures, not reconciled — possibilities include this being a general status-broadcast command that fires on multiple real triggers (startup AND certain vehicle-state changes), or the reverse-gear correlation being coincidental timing in that one test. `custom_ui`'s "Reverse gear: DISENGAGED" reading has real empirical support but is not the only behavior this command has been observed to have. See the dedicated reverse-gear note below |
...
| `0x30` | `payload[0] + payload[1]/100.0f` — a real, decodable payload format | ⚠️ **CONFLICT, evidence now more balanced** | "High"-confidence disassembly names this "arkdata/display-config file I/O." But a fresh live capture during the startup telemetry burst shows this command reporting a plausible real value — `12.x V`, exactly what a car battery reads — under `custom_ui`'s existing battery-voltage decode. A live value that plausible landing on this exact byte is real supporting evidence for the voltage reading, though it doesn't disprove the disassembly's own named-function finding either (a function name match can itself be a red herring if the disassembly traced the wrong call site). Not fully resolved either way |

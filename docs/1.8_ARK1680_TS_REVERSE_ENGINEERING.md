...
(`0x00000f04`/`0x00000007`, matching `docs/1.8_ARK1680_TS_REVERSE_ENGINEERING.md`'s
...
"MCU role" retraction in `1.3_MCU_ADAPTERS.md`: the MCU may only enable the
...
3. **`/dev/ark_display` is created by the stock kernel's
   `ark_display_drv.c`** (stock ArkMicro kernel `drivers/ark/display/ark_display_drv.c`),
...
ASTRI ArkPro `userspace/display.h`, see [`vendor_source/README.md`](../vendor_source/README.md)) + nr 29, read|write, 20-byte
...
`docs/1.7_DISPLAY_SUBSYSTEM.md`'s `ScreenId=0` entry and the ~5.5" real panel size).
...
(2026-07-11):** `docs/logs/archived/new kernel bootlog new uboot v6_260715.txt` confirmed
...
(confirmed across every command in ASTRI `userspace/display.h`)
...
- Paired with `ark_tool_handle()` right next to it — checksummed
  ...
  `docs/1.9_KERNEL_REFERENCE.md`'s already-documented **"arktool" binary

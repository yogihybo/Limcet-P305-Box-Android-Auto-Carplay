# 7.1 Custom U-Boot Build Guide

Background doc for [README §7.0 Custom U-Boot and Kernel](../README.md#70-custom-u-boot-and-kernel).
...
**The plan below was executed.** The custom `ark1668_limcet_p305` U-Boot board port it describes was built, boots on real hardware, and is now the confirmed-working path documented in README §7.0. Some details changed between the plan and the final build as real hardware testing corrected assumptions (e.g. `CONFIG_ENV_IS_NOWHERE` instead of `CONFIG_ENV_IS_IN_NAND`, no SPL) — see the "uboot_build.md" section below for the as-built record, including the actual source diffs and the first confirmed-boot serial log (2026-07-09). The section immediately below is kept for its original reasoning and config-delta table, which is still accurate background for anyone extending the build further.
...
the vendor source (see [`vendor_source/README.md`](../vendor_source/README.md)).

## Why bother

Every U-Boot workaround so far (`patch_uboot.py`, the `sdscript` self-contained SD-boot patch in
`docs/4.1_UBOOT_REVERSE_ENGINEERING.md` §8) exists solely because the Limcet P306's actual, deployed `uboot.bin`
...
[`vendor_source/README.md`](../vendor_source/README.md) for the live URL,
...
- **Linux or WSL build environment.** This project already requires WSL for `u-boot-tools`
  (`mkimage`) elsewhere (`docs/4.1_UBOOT_REVERSE_ENGINEERING.md` §8) — same requirement applies here, more so
  (a full kernel-style Kbuild needs a real Linux userland, not Git Bash on Windows).
- **Toolchain:** Linaro `gcc-linaro-7.3.1-2018.05` or `7.4.1-2019.02`, `arm-linux-gnueabihf-` target (see
  [`vendor_source/README.md`](../vendor_source/README.md)). Any reasonably
...
- **Full source clone** (clone the upstream repository `https://github.com/yogihybo/linux-arkmicro` as documented in
  [`vendor_source/README.md`](../vendor_source/README.md)):
  ```bash
...
`ark1668_dongle_sim` — kept for comparison only).

## 3. Config deltas needed — defconfig default vs. Limcet P306's real values

All real values below are from `docs/9.1_PARTITION_LAYOUT.md` and `firmware_source/mtd3_env/uboot-env.txt` (the live device's
actual, captured environment) — not guesses.

| Setting | `ark1668_defconfig` default | Limcet P306's real value | Why it matters |
|---|---|---|---|
| `CONFIG_ENV_OFFSET` | `0x160000` | `0x120000` | Must point at the Limcet P306's actual `U-boot-Env` partition start (`docs/9.1_PARTITION_LAYOUT.md`), not the reference board's offset. Same fix `docs/historical/SD_BOOT_PLAN.md` already identified before this source was actually located. |
...
| `CONFIG_MTDIDS_DEFAULT` | `nand0=ark-nand` | `nand0=ark1680-nand` | Cosmetic (env var default, overridable post-boot) but should match the Limcet P306's real env (`firmware_source/mtd3_env/uboot-env.txt`) for consistency with existing tooling/scripts. |
| `CONFIG_MTDPARTS_DEFAULT` | reference board's own layout (`bootstrap`/`bootloader`/`fdt`/... — see [`vendor_source/README.md`](../vendor_source/README.md)) | `docs/9.1_PARTITION_LAYOUT.md`'s full 12-partition table | Needed if anything reads the compiled-in default rather than the env-stored `mtdparts` (the live device already carries its own `mtdparts` string in its env either way, so this is a fallback/rescue-mode safety net, not a hard boot blocker). |
...
3. **SPL size budget** — this defconfig produces an SPL (`CONFIG_SPL=y`) that must fit in the 128K
   `S-Loader` partition budget (`docs/9.1_PARTITION_LAYOUT.md`) if it's ever flashed there. Not a concern
...
Following this project's existing safety pattern (`docs/4.1_UBOOT_REVERSE_ENGINEERING.md` §4, §7 — NAND
...
1. **Serial console first.** Flash nothing — boot with the new binary as `UBOOT.BIN` on an SD card's
   ...
   README §6.0), monitor via the existing serial console setup (README §2.0). **Go/no-go:** does a
...
(`docs/4.1_UBOOT_REVERSE_ENGINEERING.md` §7) that NAND-side U-Boot changes need real testing headroom

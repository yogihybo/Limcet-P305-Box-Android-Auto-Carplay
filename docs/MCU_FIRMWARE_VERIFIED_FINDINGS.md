...
**Real, honest conclusion**: within everything traced so far (which is now the large majority of this 12KB image's actual code, per `docs/MCU_FIRMWARE_COMPLETE_FUNCTION_AUDIT.md`'s function table), **no host-driven bulk firmware-image reception mechanism has been found in the real factory bootloader at all** — no YMODEM, no XMODEM-checksum, no USB DFU, no interrupt-driven UART reception. The confirmed flash unlock/lock/erase/write-adjacent routines exist and work correctly (section 6, 9), but nothing found so far *calls* them with a caller-supplied, host-transmitted image buffer at the scale needed to flash a full 116K application. Two real possibilities, not yet distinguished: (1) a bulk-receive loop still exists in an unexamined portion of the image (the search here covered every USART2 reference, but not every function in the bootloader — the small number of literal-pool USART2 references makes a *hidden* loop unlikely, but not impossible if it manipulates `DR`/`SR` via a completely different, not-yet-found code path); (2) **the real vehicle's actual field-update mechanism for this MCU does not go through this bootloader's own UART loop at all** — e.g. a dealer service tool reflashing via SWD directly, or the application itself implementing its own self-update trigger. Either way: **this does not block the bring-up plan.** Clean-room's `ymodem_receive_and_flash()` was always this project's OWN chosen transport for OUR replacement bootloader on the spare board — it was never a claim that it matches real vendor UART behavior, and unlike the bootstrapping/flash-primitive work (which must match real hardware register behavior to be correct), the host-to-bootloader wire protocol is an interface entirely within this project's own control. **Recommendation: proceed with Phase D/E using clean-room's existing YMODEM-CRC implementation as-is** (already correctly built on the now-confirmed-correct flash primitives), rather than continuing to search for a real UART transfer protocol that may not exist in this firmware at all.

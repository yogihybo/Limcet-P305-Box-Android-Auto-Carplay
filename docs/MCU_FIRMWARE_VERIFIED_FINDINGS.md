...
### 10. `ymodem_receive_and_flash()` NOT yet located — two real negative findings (2026-09-12)

Continued the search for the real YMODEM receive loop, with two useful negative results rather than a confirmed function:

**The 386-byte function at `0x08001D78`-`0x08001EFA`, initially suspected as the ymodem receiver, is NOT it.** Its internal state-machine dispatch values (`0x2e`, `0xe3`, `0x82` — found via a `tbb` jump-table on a byte fetched from UART) don't match YMODEM's real protocol bytes (`SOH=0x01`, `STX=0x02`, `EOT=0x04`, `CAN=0x18`, `ACK=0x06`, `NAK=0x15`). `0x2e` matches this project's already-documented custom `0x2E`-prefixed UART framing byte (see `project_mcu_firmware_uart_isr_confirmed` — the app's own UART protocol, not YMODEM) — this function is more likely a low-level UART byte-classifier/framing state machine shared by bootloader and app-side code, not the file-transfer receiver. Ruled out; the search continues elsewhere.

**No CRC16-CCITT polynomial constant (`0x1021`) exists anywhere in `live_bootloader.bin`** — searched exhaustively as a raw little/big-endian halfword and as every possible Thumb-2 `movw` 16-bit-immediate encoding, zero matches. No CRC16 lookup table (searched for the standard table's first 8 entries as a byte sequence) either. **This means the real bootloader's YMODEM implementation likely does NOT use YMODEM-CRC mode (`'C'` start byte, 2-byte CRC16 per packet) the way clean-room's `ymodem_receive_and_flash()` does** — it may instead use plain XMODEM/YMODEM checksum mode (`NAK` start byte, 1-byte additive checksum per packet, same one's-complement-style checksum pattern already seen in this project's app-side UART protocol) or handle CRC via some other, not-yet-found mechanism. **This is a real, protocol-level discrepancy that needs resolving before Phase D's host-side sender tool is written** — sending YMODEM-CRC packets to a receiver that only understands checksum mode (or vice versa) will simply fail the handshake. Recommend confirming this by disassembling whatever function reads the very first byte the bootloader transmits to request a transfer (uart_putc of either `'C'` (0x43) or `NAK` (0x15)) once located, rather than assuming either mode.

**Real, honest state of this item**: `flash_write_page()` and `ymodem_receive_and_flash()` remain unlocated. `0x08001BB2`-`0x08001C2A` and `0x08001C72`-`0x08001CF4` (both flagged as promising, UART-touching, unexamined functions in `docs/MCU_FIRMWARE_COMPLETE_FUNCTION_AUDIT.md`'s table) are the next functions to trace. This is genuinely unresolved work, not a "probably fine" assumption — Phase D (host YMODEM tool) should not be finalized against clean-room's current CRC-mode assumption until this is settled.

**Follow-up disassembly of `0x08001C2A`-`0x08001CF4` reinforces the "not standard YMODEM" hypothesis, does not yet confirm what it actually is.** `0x08001C72` compares two 24-bit values packed from adjacent byte fields (`bytes[7:8:9]` vs `bytes[9:a]`-shifted, i.e. two overlapping little-endian length/size fields) and bails out early if they mismatch — the shape of a length-plus-length's-complement sanity check, not a CRC16 verification. If they match, it computes a running value via `eor r, byte, #0xff` then `add r, r, next_byte` per data element — an **XOR-then-add one's-complement-style accumulator**, structurally the same checksum shape already documented elsewhere in this project for the app-side UART protocol (`project_mcu_firmware_uart_isr_confirmed` memory), not CRC16-CCITT's shift-and-conditional-XOR shape. `0x08001BB2`-`0x08001C2A` (examined earlier, section 10 context) similarly showed one's-complement-style checksum construction (`adds`/`mvns`/`uxtb`), not CRC.

**Working hypothesis, not yet fully confirmed**: the real bootloader's IAP transfer protocol is very likely **not standard XMODEM/YMODEM at all** — it may be a variant using the same SOH/STX/EOT-style framing bytes but with a simple additive/XOR checksum instead of CRC16 (an "XMODEM-checksum" style, which real YMODEM implementations sometimes fall back to), or it could be an entirely custom framing protocol that only superficially resembles YMODEM (this project's naming of `ymodem_receive_and_flash()` in the clean-room source was itself an assumption made without a real dump to check against, per this whole plan's Context section). **This is a significant, plan-relevant finding**: Phase D's "write a YMODEM host sender" task should not proceed on the assumption that standard YMODEM-CRC framing is correct until the actual receive loop is located and its packet-start/ACK/checksum logic is traced byte-for-byte, the same rigor already applied to every other confirmed finding in this document. Recommend the next RE session's first priority be finding the actual UART receive loop that reads packet-start bytes (trace forward from `uart_getc`-equivalent call sites already seen at `0x0800169c`/`0x080016b6` in this function and neighbors) rather than assuming the protocol shape from clean-room's naming.

### 11. Exhaustive USART2 call-site sweep — no bulk-transfer receive loop found at all (2026-09-12)

Confirmed the real `uart_putc()`/`uart_getc()`-equivalent primitives by literal-pool cross-reference: `0x080015C0(usart_base, byte)` writes `USART2->DR` (offset `+4`, 9-bit masked write), `0x080015C8(usart_base)` reads `USART2->DR` similarly. The wait-for-flag helpers `0x0800169C`(test SR bit)/`0x080016B6`(clear SR bit) are used around these — real transmit waits for **TC (bit6, 0x40)**, not TXE (bit7) the way clean-room's `uart_putc()` does (functionally fine, just more conservative/slower — not a correctness bug).

**Exhaustively enumerated every reference to the `USART2_BASE` constant (`0x40004400`) in the entire 16KB `live_bootloader.bin`** — only 3 literal-pool copies exist, referenced from exactly 18 instruction sites total, all individually traced:
- 5 sites: a generic RCC-peripheral-clock-enable dispatcher (`0x08001348`) that matches against I2C1/I2C2/UART4/USART1/USART2/USART3 base addresses — not data transfer.
- 8 sites (`0x08001920`-`0x08001A20` region): USART2 init/config (not yet fully traced, but confirmed to be setup, not bulk receive).
- 5 sites (`0x08001BB2`): the confirmed one's-complement-checksummed 5-byte status-packet transmitter (section 10) — sends a fixed 5 bytes, not a variable-length file.
- 3 sites (`0x08001D78`, ruled out in section 10 as a custom-framed byte-classifier, not YMODEM) — reads exactly one byte via `uart_getc`, does not loop over a large payload.

**No call site anywhere in the bootloader loops calling these primitives over a 128- or 1024-byte payload.** Cross-checked against interrupts too: the bootloader's own vector table (read directly, not the application's) has `USART1`/`USART2`/`USART3`/`OTG_FS`/`OTG_FS_WKUP` **all pointing at the same `Default_Handler`** (`0x080004C7`, confirmed by direct vector-table read) — no interrupt-driven reception exists either, and despite `hardware/MCU/live_dumps/README.md`'s inventory table labeling this dump "Factory IAP Bootloader (USB OTG + USART update engine)", **no USB OTG interrupt is wired and no RCC USB/OTG clock-enable was seen in the traced `clock_init()`-equivalent** — that description appears to be a generic/unverified label, not confirmed by this dump's actual code, and should be corrected or caveated in that README.

**SUPERSEDED (2026-09-15) — this section's "no bulk-transfer mechanism found" conclusion was WRONG.** The real mechanism exists; it was missed here because the search was scoped to literal-pool references to the `USART2_BASE` constant near the byte-reception primitives themselves, while the actual bulk-receive loop's growing-index buffer store (`buffer[offset+9+index] = received_byte; index++`, a self-looping state within a larger protocol state machine) doesn't re-reference `USART2_BASE` on every iteration — it's fed by the RXNE-poll/byte-read call one level up, in the same function, which *was* in the traced set but was mischaracterized (see below). Full corrected trace, hardware-plan-relevant, in section 13.
...
```
80018e0: movs r0,#0x18; strb.w r0,[sp,#0x13]; add r1,sp,#0x10; ldr r0,[GPIOA_or_AFIO_lit]; bl 0x8000b68   ; configure PA-something (pin 8?) alt-function
80018ee: movs r0,#8;   strh.w r0,[sp,#0x10]; ...; bl 0x8000b68                                            ; configure a second pin (mode 0x48)
08001902: movs r1,#1; lsls r0,r1,#0x11; bl 0x8001298     ; RCC->APB1ENR |= (1<<17)  -- USART2EN, confirmed via section 8's already-decoded 0x8001298 dispatcher
0800190a: mov.w r0,#0x9600     ; r0 = 38400 (0x9600 hex == 38400 decimal)
0800190e: str  r0,[sp]         ; struct.USART_BaudRate = 38400
08001910: movs r0,#0
08001912: strh.w r0,[sp,#4]    ; struct.USART_WordLength = 0 (8 bits)
08001916: strh.w r0,[sp,#6]    ; struct.USART_StopBits = 0 (1 stop bit)
0800191a: strh.w r0,[sp,#8]    ; struct.USART_Parity = 0 (none)
0800191e: strh.w r0,[sp,#0xc]  ; struct.USART_HardwareFlowControl = 0 (none)
08001922: movs r0,#0xc; strh.w r0,[sp,#0xa]  ; struct.USART_Mode = 0xC (Rx|Tx, matches ST SPL's USART_Mode_Rx|USART_Mode_Tx = 0x4|0x8)
08001928: mov  r1, sp          ; r1 = &struct
0800192a: ldr  r0,[USART2_BASE literal]
0800192c: bl   0x80013ce       ; USART_Init(USART2_BASE, &struct)  <-- shared/generic across USART1/2/3
08001930: movs r1,#1; ldr r0,[literal]; bl 0x80014e6   ; USART_Cmd(USART2, ENABLE) -- likely
```

**This is a byte-exact match to ST's Standard Peripheral Library `USART_InitTypeDef`/`USART_Init()` pattern** (struct field offsets 0/4/6/8/0xA/0xC = BaudRate/WordLength/StopBits/Parity/Mode/HardwareFlowControl, exactly ST's real struct layout) — this real bootloader was built against the SPL, not bare-metal register writes the way clean-room's `uart_init()` is. `0x080013CE` (already partially disassembled in section 8's clock-init detour) is a **shared, peripheral-generic `USART_Init()`** that takes `(peripheral_base, config_struct_ptr)` and internally must compute the real `BRR` divisor from whichever APB bus clock actually feeds that peripheral base — this computation was not itself traced (it's a nontrivial fixed-point routine, out of scope for this pass), but its *inputs* are now fully confirmed: **baud = 38400 is passed as a genuine runtime parameter**, exactly as suspected, not a hardcoded `BRR` literal the way clean-room does it.

**Item 6 does not have an independent answer — it collapses into item 8's still-open question.** The real `BRR` value `USART_Init()` computes for 38400 baud depends entirely on the real APB1 clock frequency, which is exactly the value section 8 flagged as unresolved (clean-room assumes 36MHz APB1 from a 72MHz SYSCLK; the disassembled `CFGR2`/`PLL2` sequence arithmetically suggests something closer to ~23MHz SYSCLK / ~11.5MHz APB1, unconfirmed). **If clean-room's clock config is correct, its hardcoded `BRR=0x03A98` is almost certainly numerically identical to what the real `USART_Init()` would compute, and no change is needed.** If section 8's ~23MHz reading turns out to be correct instead, clean-room's UART would run at completely the wrong baud rate on real silicon — easy to diagnose (garbled/no data received) but wasted debug time if not anticipated. **Recommendation: when Phase E hardware bring-up happens, if UART communication with the spare board fails or produces garbage bytes at the expected 38400 baud, check the actual clock configuration (scope/logic-analyzer on a toggled GPIO, or measure the UART bit period directly) before assuming a wiring or protocol bug** — this specific, pre-identified failure mode (wrong PLL config → wrong APB1 clock → wrong effective baud rate) is the most likely root cause given everything traced in sections 8 and 12.

### 13. Item 3 decided — `is_app_valid()` now matches the real bootloader exactly (2026-09-12)
...
**Real, NOT-yet-resolved anomaly found while verifying the bit16=1 fix**: re-measuring with the same DWT technique after the fix gave a CPU clock reading consistent with **~144 MHz, not 72 MHz** — roughly double the intended value, and repeatable across two separate measurement sessions (~142.5 MHz and ~143.9 MHz). Working hypothesis: **the spare test board's actual HSE crystal may be 16 MHz, not the 8 MHz this project has assumed throughout** (16 MHz × 9 = 144 MHz would exactly explain it) — plausible, since the spare board is a third-party CAN-filter accessory device chosen only because it shares the same STM32F105RBT6 chip, not because it replicates the real Prado MCU board's own crystal choice. **This was NOT confirmed**: an attempt to test-fix it by setting `PREDIV1=/2` (which should halve the PLL input and land back on 72 MHz if the 16 MHz-crystal hypothesis is correct) produced a real, unexplained measurement anomaly of its own — `RCC->CFGR2` reads back `0` even a few instructions after a single-step-confirmed write of a nonzero value, and the measured frequency did not change between `PREDIV1=/1` and `/2` in two clean, freshly-flashed test builds. This may be a real STM32 hardware behavior (some F1-family PLL-config registers are documented to read as their pre-PLL-lock value being masked/inaccessible once `PLLON=1`, which would make register-readback an unreliable way to confirm what was actually latched — the DWT-based *frequency* measurement remains valid regardless, since it observes real executed cycles, not a register value), or a real bug in the test methodology not yet root-caused.
...
### 17. Bootloader-entry trigger and no-op-update recovery path — CONFIRMED end-to-end on real hardware (2026-09-12)

Continued hardware testing on the spare board (now running the 72MHz-corrected, `is_app_valid()`-corrected build from sections 15-16) to exercise the exact real-world trigger path documented in section 14 (`CMD 0xE1`'s magic-cookie mechanism), without a UART cable connected (no USB-UART adapter available in this environment — only the Pico's own CMSIS-DAP interface was present). Simulated the app-side trigger directly via SWD, since it's just a single SRAM write:

1. **Bootloader-entry trigger, confirmed real**: with the app running normally, wrote `0x5555AAAA` to `0x20004004` via SWD, then `reset run`. After 300ms, PC was found at `0x08000156`/`0x08000158` — inside the bootloader's `uart_getc_timeout()` busy-wait loop (the same address range identified in section 15's earlier hang investigation, now understood to be entirely expected here) — confirming `main()`'s `force_bootloader` check correctly detected the magic cookie and skipped the app-jump. The magic cookie itself read back as `0x00000000` immediately after, confirming `main()`'s one-shot `*BOOTLOADER_MAGIC_ADDR = 0;` clear runs correctly (not a sticky flag that would force bootloader mode forever).

2. **Timeout recovery path, confirmed real — no bricking on an aborted/never-started update**: left the board running freely with no YMODEM data ever sent (simulating a real-world "CMD 0xE1 triggered but the update never actually starts" scenario — a real, non-hypothetical risk this project's own `tools/mcu-probe --reboot-probe --confirm-erase-risk` gate already exists to warn about). After ~58 seconds total (consistent with `ymodem_receive_and_flash()`'s 30-retry × 1000ms `uart_getc_timeout()` loop, which returns `false` once exhausted), PC was found back inside the **application** region (`0x0800339A`, then `0x0800360A` on a follow-up check 500ms later — genuinely progressing, not stuck), with `CFSR=0` (zero HardFault/BusFault/UsageFault). This confirms the complete real recovery chain: YMODEM timeout → `ymodem_receive_and_flash()` returns `false` → `main()` falls through to `SCB->AIRCR` system reset → fresh boot → `is_app_valid()` finds the still-valid, never-erased application (erase in `ymodem_receive_and_flash()` only happens once a real, non-empty header packet is received — confirmed in section 15's bug-fix description — so an update that never starts never touches the application flash at all) → jumps back into the running application cleanly.
...
### 18. Cross-check: is the documented state consistent with the live dump files? — mostly yes, one real overclaim corrected (2026-09-12)

Prompted by a direct question about whether everything documented actually agrees with what's really in the dump files (`live_bootloader.bin`, `live_app_1302.bin`, and the newer re-sliced `factory_bootloader_12k.bin`/`factory_app_1302_52k.bin`/`factory_full_64k.bin`), rather than assuming consistency.

**Internal consistency: confirmed solid.** All five files' SHA-256 hashes match their documented values. Directly verified the three newer "aligned partition" files are byte-for-byte exact re-slices of the original raw extractions, not a separate/different capture that could disagree: `factory_bootloader_12k.bin == live_bootloader.bin[0:12288]`, `factory_full_64k.bin[0:12288] == factory_bootloader_12k.bin`, `factory_full_64k.bin[12288:] == factory_app_1302_52k.bin`, and `factory_app_1302_52k.bin[0x1000:] == live_app_1302.bin` (consistent with this project's own earlier empirically-derived finding, section 14, that `live_app_1302.bin`'s file offset 0 is flash `0x08004000`, exactly `0x1000` bytes into `factory_app_1302_52k.bin`'s own claimed `0x08003000` start).

**One real overclaim found and corrected**: `live_dumps/README.md` described `factory_app_1302_52k.bin` as including "the Complete Native Application (including Vector Table & Startup)." Directly checking its first 8 bytes (flash `0x08003000`/`0x08003004` — the vector table's Initial-SP and Reset entries) shows `0xFFFFFFFF 0xFFFFFFFF`, not real captured values. A real, working device cannot have an erased stack pointer at its own application's entry point — this can't be genuine silicon content. This matches (and gives an unexplained-until-now root cause hint for) `docs/MCU_FIRMWARE_COMPLETE_FUNCTION_AUDIT.md`'s own vector-table table, which had already labeled these same two entries "Unpopulated / Hardware Masked" without further explanation. **The rest of the vector table (NMI onward, `0x08003008`+) is real** — `0x08003008`/`0x0800300C` read `0x080031ED`/`0x080031EF`, matching that same audit's documented NMI/HardFault handler addresses exactly.

**Investigated the root cause, did not fully resolve it — recorded honestly rather than overclaiming a fix**: `tools/stm32f1_extractor_fixed.py`'s CVE-2020-8004 technique has a documented `INACCESSIBLE_EXC_NUMBERS = [0, 1, 7, 8, 9, 10, 13]` list (exception numbers that can't be captured this way). Since `0x08003000` is naturally 256-byte-aligned (this extractor's own `table_size=64` block size), it initially looks like a clean case of exception-number-0/1 inaccessibility. Two checks, both against the tool's actual behavior rather than assumption:
- **Tested whether this affects every 256-byte boundary in the dump** (which would make it a systematic, fully-explained gap): it does not — only ~23.1% of 256-byte boundaries across the 64K dump show both words as `0xFFFFFFFF`, statistically indistinguishable from the file's own ~23.5% general background rate of ordinary erased flash. This specific hypothesis is refuted by the data.
- **Re-read the extractor's own wrap-around logic** (`calculate_vtor_exc()`, lines 176-186): for address `0x08003000` specifically, the secondary-alignment-shift path computes an alternate exception number (64, a real external interrupt slot) that is *not* in `INACCESSIBLE_EXC_NUMBERS` — meaning the tool's own code should have been able to route around the limitation for this exact address, not hit it.

**Follow-up (2026-09-12, later same day): root cause now CONFIRMED, not just suspected — reproduced live on real hardware against known ground truth.** Since the spare test board is not RDP-locked, its own currently-flashed values are directly readable (no exploit needed) and serve as trustworthy ground truth. Replicated the CVE-2020-8004 technique's exact wrap-around mechanism by hand via direct OpenOCD/SWD commands (matching `generate_exception()`'s external-interrupt branch: relocate `VTOR`, enable+pend NVIC external interrupt 48 — the same `exception_number=64` the extractor's own `calculate_vtor_exc()` computes for this address — set PC/xPSR/SP per the script, single-step, read back PC):

- **Target: `0x08003000` (real, ground-truth value `0x20005000` — the app's genuine Initial-SP)**. `VTOR` set to `0x08002F00`, read back as written exactly (`0x08002F00`, no silent hardware masking/truncation). Recovered "PC" after the single-step: **`0xFFFFFFFE`** (→ `0xFFFFFFFF` once the extractor's own `recover_pc()` ORs in the T-bit) — **exactly, precisely reproducing the archived dump's own artifact**, on demand, against a value we independently know is really `0x20005000`. `CFSR`/`HFSR` both read `0x00000000` — no fault was recorded despite the clearly wrong result. Reproduced twice, byte-identical both times.
- **Control test, same mechanism, different known-even target**: `0x08003100` (real, ground-truth value `0x00000000`) recovered **correctly** as `0x00000000` via the identical procedure (just a different `VTOR`/exception-number pairing landing on the same offset). This rules out the simplest theory ("any even-valued/non-Thumb word fails") — it isn't that simple, since a genuinely different even value (zero) came back right.

**What this does and doesn't establish**: the extraction technique's failure for this specific address is now a **directly reproduced, hardware-confirmed fact**, not an inference — this is definitively an extraction-methodology artifact, not a property of the original vehicle's silicon (which obviously has a real, working, non-erased SP there, or the vehicle couldn't boot). What remains genuinely uncharacterized is the precise ARM-architectural reason the `0x20005000` case fails while the `0x00000000` case at a structurally identical redirect succeeds — a real, open question (candidate explanation: `0x20005000` is a plausible, executable SRAM address the CPU might actually attempt to fetch-and-execute from during the faulty vector-load rather than failing cleanly, cascading into a second, different fault state whose own recovered value happens to match the dump's artifact pattern; `0x00000000`'s behavior may be shaped by this chip's own boot-time memory remapping aliasing address 0 to flash — neither explanation confirmed at the instruction level). **Practical implication, stated carefully**: this class of extraction error is real and reproducible, but the control test shows it is not simply "every even word is wrong" — so this specific finding should not be generalized into a claim that other even-valued words elsewhere in the dump are also suspect without the same kind of direct reproduction/ground-truth check done here. It is confirmed, specifically, for vector-table-style redirected reads of code-adjacent, plausible-looking RAM addresses (matching the same underlying class already documented for the `0x08001234` artifact in section 5.1).

**Nothing else in this document's findings depends on this value** — `is_app_valid()`'s real behavior and the `0x08003000` app-base-address finding were both derived from disassembling the *bootloader's own code* (sections 5.2, 6), never from reading this address directly out of the dump, and this session's own live-hardware SWD reads of the currently-flashed clean-room build (sections 15-17) are a separate, independently-confirmed data source unaffected by this dump-specific gap.

### 19. Review of the dump realignment work (`cb900dc`) — sound, two documentation-only corrections made (2026-09-12)

Reviewed the commit that produced `factory_bootloader_12k.bin`/`factory_app_1302_52k.bin`/`factory_full_64k.bin` from the original raw extractions, checking the actual realignment operation itself (not just the resulting files, already cross-checked in section 18).

**The realignment/stitching operation is correct**: it's a straight concatenation of already-existing raw-extraction bytes (`live_bootloader.bin`'s first 12K + `live_bootloader.bin`'s trailing 4K + `live_app_1302.bin`, confirmed via direct byte comparison in section 18) at the correct, disassembly-confirmed boundary (`0x08003000`, matching the app-base-address finding from sections 5.2/6) — not a fresh, independent re-extraction that could disagree with prior data, and not introducing any new error. The one known artifact (the vector table's own Initial-SP/Reset words reading `0xFFFFFFFF`, root-caused in section 18's follow-up) was already present, unchanged, in the original raw extraction before this realignment — the stitching didn't cause or worsen it.

**Two real documentation overclaims found and corrected in `live_dumps/README.md`** (beyond the one already fixed in section 18):
- `factory_full_64k.bin` was described as "the entire MCU" — the chip's real total flash is 128KB, and the application's own accessible region extends to `0x0801FFFF` (116KB past the bootloader, per section 9's independently-confirmed 58-page erase-loop trace); this dump only covers the first 64KB (`0x08000000`–`0x0800FFFF`). The upper 64KB (`0x08010000`–`0x0801FFFF`) was never captured by any file in this directory.
- Checked whether that gap is likely to matter: real content in the application half of this dump ends cleanly at `0x0800D437`, followed by over 11KB of consistent, unbroken erased flash (`0xFF`) all the way to this dump's own `0x0800FFFF` boundary — a clean "real firmware genuinely ends here" pattern, not a suspicious mid-content truncation. This gives real confidence the captured 52KB holds the complete, real application image, but the never-captured upper 64KB remains genuinely unverified (not confirmed-empty, just never checked) — corrected the README to say this precisely rather than either overclaiming completeness or unnecessarily casting doubt on the part that was actually captured.

### 20. Review of the Touchscreen/SWC driver commit (`ae53323`) — sound, one real flashing-procedure gotcha found (2026-09-12)

Reviewed `hardware/MCU/source/`'s new `swc_driver.c`/`touch_driver.c` (analog steering-wheel-control resistor ladder via `ADC1`+`DMA1`, capacitive touchscreen via `I2C1` Goodix GT911 protocol, quadrature rotary-encoder decoding) plus the `SysTick`-based millisecond scheduler wired into `main.c`.

**Register-level correctness, checked against real STM32F1 semantics, not assumed**: the new `ADC_TypeDef`/`DMA_Channel_TypeDef`/`DMA_TypeDef`/`I2C_TypeDef`/`EXTI_TypeDef`/`SysTick_TypeDef` struct layouts in `stm32f105.h` all match the real STM32F1 register map exactly (field order and computed offsets cross-checked by hand against RM0008, including the DMA channel's real 0x14-byte inter-channel spacing, not the naively-assumed 0x10). The ADC calibration sequence (`ADON` → delay → `RSTCAL`+wait → `CAL`+wait) matches ST's own documented procedure verbatim. The `I2C1` 100kHz-standard-mode `CCR`/`TRISE` values are correctly derived from the real 36MHz APB1 clock (`CCR=180`, `TRISE=37`), matching the standard formulas.

**Cross-referenced against the real dump, not just plausible-looking code**: disassembled the claimed real ADC1/DMA init sequence (`0x080076D0`-`0x0800774C` in `live_app_1302.bin`) — genuinely real, structured code calling into what looks like a Standard-Peripheral-Library-shaped `ADC_RegularChannelConfig()`-equivalent, invoked twice with parameters matching exactly the claimed rank-1/channel-1 and rank-2/channel-9 sequence configuration, followed by two poll loops matching the claimed calibration-wait pattern. This is real corroborating evidence, not just a plausible-sounding commit message.

**Build-verified**: `hardware/MCU/source/` builds clean with zero warnings under `-Wall -Wextra` (5,676 bytes, well within the 116K app region). `SysTick_Handler`'s weak-alias override is correctly wired into the app's own vector table (confirmed in `startup_stm32f105.c`).

**Real, reproducible flashing-procedure gotcha found and resolved (not a defect in this commit's code)**: the first several attempts to reflash this build onto the spare board via `flash write_image` + `verify_image` (using a plain `halt` beforehand, following this session's own established pattern from earlier, simpler builds) produced **real, reproducible checksum mismatches at two fixed addresses** (`0x0800326E`/`0x0800422A`), with the specific wrong byte values differing between otherwise-identical attempts — impossible to explain by a source/build bug (the `.bin` is byte-for-byte deterministic), and ruled out as a flaky-read/degrading-flash-cell issue (repeated reads of the same already-written content were perfectly stable). **Root cause: a plain `halt` is not sufficient before reflashing this specific build**, because — unlike every earlier, simpler build this session flashed — this one has `SysTick` firing a real 1kHz interrupt continuously once running; halting a target with a live periodic interrupt mid-flash-programming can desync the SWD-driven flash write sequence. **Fix, verified working**: use `reset halt` (not a bare `halt`) before both the write and the verify — this produced a clean, zero-mismatch verify on the first attempt, twice. Confirmed the resulting firmware boots and runs correctly: `CFSR=0` (no faults) across two checks with PC genuinely progressing through app code, and every relevant peripheral register (`RCC` clock-enable bits, `ADC1_CR2`, `DMA1_Channel1_CCR`, `SysTick_CTRL`, `I2C1_CR1`) reads exactly the configuration the driver code intends — including `SysTick_CTRL`'s `COUNTFLAG` bit being set, confirming it's genuinely counting down and wrapping, not stuck.

**One expected, benign observation, not a bug**: `I2C1_CR1` was caught mid-transaction (`START`/`STOP`/`ACK` all appearing set) when sampled — expected and correct, since this spare test board has no real GT911 touch controller wired to its `I2C1` pins, so `touch_process_digitizer()`'s I2C reads legitimately never get a response. The driver's own timeout-based design (`I2C_TIMEOUT_CYCLES`, explicitly commented as existing to handle exactly this "hardware is absent" case) is working as intended — the main loop keeps running cleanly (`CFSR=0`, PC progressing) rather than hanging on the unanswered I2C bus.

**Practical takeaway for future flashing sessions**: any build that enables a free-running periodic interrupt (SysTick, a timer, etc.) should be reflashed with `reset halt` before both `flash write_image` and `verify_image`, not a bare `halt` — this project's own earlier, interrupt-free builds never exposed this, so it's easy to carry forward the simpler habit and hit this exact class of false "corruption."

### 21. Review of the GPIO/relay/power-management/CAN2 commit (`9c61012`) — mostly sound, one real bug found and fixed (2026-09-12)

Reviewed `gpio_driver.c`/`power_manager.c` (Task 5 discrete-input polling and relay control, Task 4 power-state machine with `WFI`-based low-power sleep) and the `can_driver.c`/`startup_stm32f105.c` CAN2 integration.

**Vector table**: the two new IRQ handler slots (`EXTI9_5_IRQHandler` at IRQ23, `CAN2_RX0_IRQHandler` at IRQ64) exactly match the real, already-audited factory vector table (`docs/MCU_FIRMWARE_COMPLETE_FUNCTION_AUDIT.md`'s own IRQ-index-to-real-handler mapping) — both declared correctly as weak aliases to `Default_Handler`, safely overridden by strong definitions elsewhere.

**Relay/pin claims independently, precisely confirmed against the real dump — not just plausible-looking code**: disassembled the claimed relay-control block (`0x08006C1A`-`0x08006CE0` in `live_app_1302.bin`). Every single claimed relay's bit-mask matched the real disassembly exactly (`0x4`=PB2, `0x2000`=PC13, `0x8`=PB3, `0x1000`=PC12, `0x10`=PB4, `0x4000`=PC14), and resolving the literal-pool port-base reference for the most novel/uncertain claim (`PC14`, described as a second "SoC Reset" pin distinct from the already-hardware-confirmed `PB14`) confirmed the real port base is exactly `0x40011000` = `GPIOC_BASE`. This is real, bit-exact, disassembly-verified RE work, not a fabricated commit message. `gpio_driver_init()`'s own `PB14` re-assert runs strictly *after* the existing, hardware-confirmed `ARK1668` reset-hold sequence in `gpio_hardware_init()` completes — a harmless redundant re-assert (pin already high), not a re-toggle that could shorten/corrupt the real reset pulse.

**Power manager's `WFI` sleep loop reviewed for hang risk — no real risk found**: feeds the watchdog every loop iteration before checking wake conditions; wakes on *any* enabled interrupt, and since `SysTick` fires every 1ms regardless of sleep state, the loop is guaranteed to re-check its wake condition roughly every millisecond rather than genuinely deep-sleeping — this means the implementation doesn't achieve real power savings (a legitimate design-quality note, not a safety bug) but also means there is no path to a true, un-recoverable hang: the watchdog keeps getting fed far more often than its ~2s timeout regardless of whether `ACC`/CAN activity ever resumes. `SCB->SCR` `SLEEPDEEP` is correctly left untouched, so this is genuine `Sleep` mode (safe, no post-wake clock-recovery sequence needed) rather than the much riskier `Stop` mode.

**One real, confirmed-on-hardware bug found and fixed**: `CAN2`'s clock, GPIO pins (`PB12`/`PB13`), and NVIC route (`IRQ64`) were all correctly configured, but the `CAN2` peripheral itself was never taken out of its post-reset Sleep mode — no exit-sleep/init-mode/`BTR`-configure/normal-mode-entry sequence, and `CAN2->IER`'s `FMPIE0` was never enabled. Confirmed directly on hardware before assuming from source alone: `CAN2->MCR` read back `0x00010002` (`SLEEP` bit still set, the real bxCAN post-reset default) and `CAN2->MSR` showed `SLAK=1` (sleep genuinely hardware-acknowledged, not just requested), with `CAN2->IER=0`. This meant `CAN2` could never actually receive bus traffic, and `CAN2_RX0_IRQHandler` could never fire even if it somehow did — despite the commit's "Integrated secondary bxCAN (CAN2)" claim. Not a crash risk (the peripheral just silently does nothing, consistent with the commit's own "86s+ glitch-free runtime" claim, which is about the absence of faults, not `CAN2` functionality specifically), but a real functional gap.

**Fix, itself grounded in a real disassembly check rather than assumption**: before writing the fix, disassembled `0x08008F1A`-`0x08008FE4` (the real `CAN2`-init code, right after the already-cited `CAN2` pin-config range) and confirmed the real firmware configures `CAN2` with the exact same bit timing as `CAN1` (`Prescaler=9`, `BS1`/`BS2` fields matching exactly) — so mirroring `CAN1->BTR`'s value for `CAN2` in the fix is evidence-based, not a convenient guess. Added the missing `CAN2` exit-sleep/init-mode/`BTR`/normal-mode sequence and `FMPIE0` enable to `can_driver.c`, plus a filter-bank-14 pass-all filter configured via `CAN1`'s own filter-control registers — a real STM32 dual-CAN quirk where filter master control and all 28 filter banks are physically implemented only inside `CAN1`'s register block, with `CAN1->FMR`'s `CAN2SB` field (left at its default value of 14, the standard even 14/14 split) marking where `CAN2`'s own banks begin.

**Fix verified on real hardware, not just build-checked**: rebuilt (clean, zero warnings), reflashed (using `reset halt`, per section 20's own lesson), and confirmed live: `CAN2_MCR`/`CAN2_MSR` now read identically to `CAN1`'s (`0x00010060`/`0x00000c08` — awake, `ABOM`/`AWUM` set, normal operating mode), `CAN2_BTR` reads `0x00140008` matching `CAN1` exactly, `CAN2_IER` reads `0x00000002` (`FMPIE0` enabled). The application continues running cleanly after the fix (`CFSR=0` across two checks, PC genuinely progressing through app code) — the fix didn't introduce any regression.

### 22. Review of the extended CAN matrix/telemetry commit (`e36d8e3`) — steering math confirmed precisely, one real bug fixed, one deeper issue flagged unresolved (2026-09-12)

Reviewed the new `handle_toyota_prado_*` CAN decoders and the `uart_send_composite_vehicle_status()`/radar-telemetry packet builders in `vehicle_profiles.c`/`uart_protocol.c`.

**Steering-angle math (CAN `0x025`) — confirmed precisely against the real dump**: disassembled `0x080091E0` in full. The claimed sign-bit test (bit3), `0x0FFF`-inversion when the sign bit is set, a correctly-rounded arithmetic `>>1` scale (the real code uses the standard ARM `add r,r,r>>31; asr r,r,#1` round-toward-zero idiom before the shift — a detail this level of care matches), and the final clamp to `0x7F` all match the real disassembly exactly, instruction for instruction. This is solid, precise RE work. One caveat stated plainly, not glossed over: the real handler reads its sign/magnitude bytes from a computed per-CAN-ID history-struct offset (`+0xB`, `+0xC` — the same struct-indirection pattern already known from the SWC handler), not directly from `f->data[0]`/`f->data[1]` — the exact history-struct-offset-to-wire-byte correspondence was not independently re-traced this pass, so the *arithmetic* is confirmed exact, but which physical CAN payload bytes feed it carries the same inherited, not-fully-wire-verified assumption already flagged for the SWC handler in earlier sessions.

**Relay-pin GPIO claims from the previous commit remain consistent** (already verified in section 21) — nothing in this commit touches those.

**One real, confirmed bug found and fixed**: `handle_toyota_prado_radar_speed()`'s rear-sensor array had `rear[1]` and `rear[2]` computing the *identical* expression (`f->data[2] & 0x0F` twice) — an internal inconsistency regardless of the real wire mapping, since two independently-named sensor channels (rear-middle-left, rear-middle-right) reading the exact same nibble isn't a plausible real sensor layout. Fixed by using `data[2]`'s high nibble for `rear[2]`, mirroring the already-correct `front[1]`/`front[2]` high/low split of `data[2]`.

**A deeper, genuinely unresolved issue found while fixing the above, flagged rather than silently patched over**: the whole front+rear radar decode only reads `f->data[1]`, `f->data[2]`, `f->data[3]` (3 bytes = 6 independent nibbles) to derive 8 independent sensor channel values (4 front + 4 rear) — meaning the front and rear channels are forced to *share* the same two source bytes (`data[1]`/`data[2]` for front, `data[2]`/`data[3]` for rear, with `data[2]` doing double duty for both). A real, physically-independent 8-sensor parking-radar system would need 8 independent nibbles (4 bytes) at minimum, not 6. This may mean the real frame's rear-channel data actually lives in payload bytes this handler doesn't yet read (the partial disassembly of `0x080093D8` this pass reached struct offsets `0xB`/`0xC`/`0xE` before running out of review time — a genuinely large, struct-indirected function not fully traced). **Not resolved this pass** — the fix applied above removes the one clear internal inconsistency (an exact duplicate) without claiming to have found the true, complete real byte mapping. A full disassembly of `0x080093D8` through its actual end address, tracing every struct-offset read back to a real CAN wire-byte position, is the right next step before trusting the rear-channel values as wire-accurate.
...
**Independently re-verified, not taken on faith**: re-disassembled `0x08009480`-`0x08009568` directly and confirmed every claim above byte-for-byte — the rear write sequence (`0x080094EC`-`0x08009504`) genuinely reads `[sp+0x12]` twice (offsets `0x18`/`0x19` = McuSettings 24/25) while `0x17`/`0x1A` (23/26) get distinct values; the front sequence (`0x0800953A`-`0x08009558`) shows the identical pattern (`[sp+6]` duplicated into offsets `0x21`/`0x22` = 33/34); and both threshold-comparison loops (`0x080094C8`-`0x080094EA`, `0x08009510`-`0x08009538`) skip exactly the duplicated offset (25, 34) in their change-detection checks. Also confirmed the underlying `Data[N]` byte-offset claims (`+0xB` through `+0x12` for `Data[0..7]`) against the five actual payload-load instructions cited — all match exactly. This closes out section 22's open item with real confirmation: the code's `rear[]`/`front[]` array assignments (unchanged by this commit — only the explanatory comment was updated) were already correct as fixed in section 22, now backed by full evidence rather than a plausible guess. Build re-verified: identical binary size to the already hardware-tested build in section 22 (a comment-only change doesn't affect compiled output), so no re-flash was needed to confirm this.

### 24. Review of `tools/test_mcu_rigorous.py` (`d369506`) — a genuine, working 23-point HIL suite, independently executed and confirmed (2026-09-13)

Reviewed the new hardware-in-the-loop test tool, which drives the spare board purely over SWD (no UART cable needed) by writing synthetic `CanFrame` entries directly into the ring buffer's SRAM and reading back decoded state.

**The two things most likely to be wrong in a tool like this were checked directly, not assumed**:
- **`inject_can_frame()`'s assumed 20-byte struct layout** (`id`@0, `ext_id`@4, `ide`@8, `rtr`@9, `dlc`@10, `data[8]`@11-18, `fmi`@19) matches the clean-room's actual `CanFrame` struct in `can_driver.h` exactly, field for field — not a mismatched assumption borrowed from the real factory firmware's own (different) `CanRxMsg` convention.
- **Every hardcoded `SYM_*` SRAM address** (14 checked directly: `g_active_mode`, `g_rear_radar`, `g_front_radar`, `s_power_state`, `g_system_ticks_ms`, `g_can_rx_ring`, `g_door_open`, `g_gear_field`, `g_steering_mag`, `g_steering_sign`, `g_last_door_state`, `g_last_lights_state`, `g_last_reverse_state`, `s_can_activity_flag`, `s_timer_counter`) was cross-checked against a freshly rebuilt `can_app.map` and matches exactly — these aren't stale/guessed addresses from an old build.

**Actually run, not just read**: executed `tools/test_mcu_rigorous.py` against the spare board after flashing the current build — **23/23 tests passed live**, independently, not by trusting the commit's own report. This end-to-end exercises CAN frame injection, all four `handle_toyota_prado_*` decoders, `CAN1`/`CAN2` register state, the power-manager state machine, and a 5-second sustained-execution watchdog check. Also hand-verified the radar test's own expected values (`data=[0x14,0x23,0x51]` → `front=[1,5,5,8]`, `rear=[8,5,5,1]`) against `map_radar_distance()`'s real logic — they check out exactly, which is a genuine additional end-to-end confirmation of the section 22/23 radar-decode fix, from a completely different angle (live CAN injection rather than static disassembly).
...
### 27. RESOLVED (2026-09-15): the real bootloader DOES have a bulk-transfer/firmware-update mechanism — sections 10-11's "not found" conclusion was wrong

Directly prompted by a pre-flight safety question (is there an application-only update path that avoids a full RDP-unlock mass-erase?), re-opened the search sections 10-11 had closed as "no host-driven bulk firmware-image reception mechanism... found... at all." That conclusion does not hold — the real mechanism exists, traced end-to-end against `live_factory_bootloader_12k_reconstructed.bin`, and independently reconfirmed present byte-for-byte on the now-100%-reconstructed binary (sections 10-11 worked from an earlier, less-complete extraction pass).

**Why the earlier sweep missed it**: it searched for literal-pool references to `USART2_BASE` at every call site and characterized each cluster from a narrow instruction window. Two real functions that ARE part of this mechanism (`0x08001D78`/`0x08001BB2`/`0x08001C72`, flagged in section 10 itself as "next functions to trace" but not fully traced there) were sampled too narrowly and mischaracterized — one as "a custom-framed byte-classifier... reads exactly one byte," another as "a fixed 5-byte status-packet transmitter." Both are real, correct observations about parts of these functions, but neither is the whole picture: the actual growing-index buffer fill (the real receive loop) is a *self-looping state* inside the larger protocol state machine, one level removed from the `USART2_BASE`-referencing call site the earlier sweep was anchored to.

**The real, complete mechanism, traced instruction-by-instruction:**

1. **App-side trigger** (already confirmed elsewhere in this project): app writes the magic cookie (`0x5555AAAA` → SRAM `0x20004004`) and resets; bootloader's own `main()` (`0x080017E2`) checks this on every boot and, if set, skips the app-jump and falls into the update-wait flow.
2. **UART handshake**: a real byte-by-byte receive state machine (RXNE-poll → `DR` read → table-branch on protocol state, confirmed via two separate `tbb` jump tables, 7-state and 22-state) recognizes a magic ASCII sync string sent by the host. On match, it builds and transmits a real 5-byte reply `[0x2E][0xE2][0x01][0x01][0x1B]` — `0x2E` is this project's already-documented UART sync byte, `0xE2` is exactly `MCU_CMD_UPGRADE_ACK` ("firmware update ACK handshake"), confirming this uses the *same* framing convention as the application's own SoC↔MCU protocol, not textbook XMODEM/YMODEM framing (`SOH`/`STX`/`EOT`) — which is exactly why grep-for-YMODEM-markers searches never found it.
3. **Application-region-only erase**: `0x08001B82` loops 58 times calling the already-confirmed `flash_erase_page()` (`0x080005B4`) — 58 × 2048 bytes = 116K, precisely the application region (`128K total − 12K bootloader`). The bootloader's own 12K region is never in this loop's range.
4. **Status + ACK**: `0x08001BB2` sends a 6-byte status/size report (`CMD 0xE4`, a real, previously-undocumented outbound command — payload is a 16-bit count) followed by another 5-byte `CMD 0xE2` ACK.
5. **Bulk data reception**: the 7-state machine's state 6 (`0x08001E24`) is a genuine self-looping byte-accumulator — `buffer[base_offset + running_index] = received_byte; running_index++` — filling a 128-byte payload buffer one UART byte at a time via the same RXNE-poll mechanism confirmed in step 2.
6. **Per-block checksum + flash write**: `0x08001C72` processes one completed 128-byte block at a time: bails immediately if a received-block counter (struct offset `+9`/`+0xA`) already equals a target-block counter (offset `+7`/`+8`); otherwise computes a running one's-complement-style checksum (`eor`/`add`, not CRC16 — confirming section 10's own "not standard YMODEM" hypothesis) over the 128 bytes, then writes each halfword (2 bytes) via the confirmed real flash-program primitive `0x080007BA` (`FLASH_CR |= PG` then `strh` to the target address, advancing by 2 each write), and finally increments the received-block counter and loops back for the next block.

**Real, concrete conclusion**: this is a genuine, complete, application-only firmware update mechanism, using a real (non-standard, checksum-not-CRC, `0x2E`-framed) protocol structurally related to but distinct from XMODEM/YMODEM — not the clean-room's own YMODEM-CRC assumption, and not "no mechanism exists" either. Critically for the live-vehicle safety question this was prompted by: **the erase step only ever touches the 116K application region — the bootloader itself is never erased or rewritten by this path**, meaning a successful update via this real mechanism would not require RDP unlock, would not require a full-chip mass-erase, and would leave the working bootloader in place as a fallback throughout.

**What is NOT yet confirmed, and must be before this is relied on for anything real**: the exact packet header field layout (which bytes carry total-block-count vs. per-block-length vs. the one's-complement checksum, and in what order), the retry/NAK behavior on a checksum mismatch (does it silently continue, request a retry, or abort?), the exact magic sync string's full byte sequence, and — most importantly, given this whole project's repeated experience this session with disassembly-only conclusions turning out wrong or incomplete (see this same section's own correction of sections 10-11) — **hardware verification on the spare board that this mechanism actually works end-to-end**, not just that its disassembly looks correct. Static tracing got the flash-primitive and erase-scope claims right before (sections 6, 9); it has also been wrong or incomplete twice now on this exact bootloader (sections 10-11's original conclusion, and this section's own correction of it) — a live test on the spare board, sending a real small test image through this exact protocol and reading back flash content, is the necessary next step before this can be trusted as a real answer to the live-vehicle safety-plan question that prompted this re-investigation.

### 28. HARDWARE-VERIFIED (2026-09-15): the block-write mechanism works end-to-end on real silicon, exact obfuscation transform confirmed

Direct follow-up, same day: tested section 27's finding on the spare board rather than trusting the disassembly alone, per that section's own stated next step. Since USART2 isn't physically wired to anything on this bench setup, the front-end UART reception/handshake was bypassed the same way this project has consistently done elsewhere (CAN ring buffer injection, UART command ring injection) — directly populate the SRAM state the *processing* logic consumes, then force execution into it via SWD, rather than trying to fake real electrical UART signaling.

**Method**: flashed only the real reconstructed bootloader (`live_factory_bootloader_12k_reconstructed.bin`), halted at its own reset vector before any of its code ran (guaranteeing no unrelated execution could corrupt register/stack state before the forced test — a real mistake caught and fixed mid-session: an earlier attempt called `resume` between setup steps across separate script invocations, letting real bootloader code run for real in between and corrupting state, producing a genuine `HardFault`/`CFSR=1` before the fix). Directly unlocked flash (`FLASH_KEYR` = `0x45670123`/`0xCDEF89AB`), then:

1. **Erase primitive test**: forced `r0` = a disposable, page-aligned, in-range test address (`0x0801F800`, the flash's own last page — verified in-range and page-aligned in code, after an earlier hand-typed address had an extra hex digit and silently pointed outside the 128KB flash entirely, causing a read failure that was correctly investigated rather than assumed to be a hardware/tooling fault), forced `PC` to `flash_erase_page()`'s entry (`0x080005B5`), breakpointed its real, disassembly-confirmed exit (`0x080005FA`), resumed. Result: clean halt, `r0=4` (success status), page read back all `0xFF`, `CFSR=0`. The real erase primitive works exactly as traced.
2. **Block-processing test**: with the target page still erased and the CPU still halted continuously (no intervening `resume`), populated the real struct at `0x20004008`: flash-target address (4 bytes) = the test page, target/current block-count fields set **equal** (`0x0000`/`0x0000` — the branch this section had to re-derive correctly: `cmp r7,r4; beq <process>` means the processing path is taken when the counts *match*, not when they differ, the opposite of the first attempt's assumption, which instead hit the harmless early-exit path and correctly proved nothing), and a 128-byte payload buffer (`0x20004025`) filled with an incrementing test pattern (`0x00, 0x01, 0x02, ...`). Forced `SP` to a safe scratch area, `PC` to the block-processor's entry (`0x08001C73`), breakpointed its real exit (`0x08001CAA`), resumed.

**Result: complete success, zero faults.** `CFSR=0`, `HFSR=0`, the received-block counter (`r4`, and the struct's own offset `+9`/`+0xA` fields) correctly advanced from `0` to `1`, and — the real proof — flash content at the test address changed from all-`0xFF` to real, deterministic data.

**The exact obfuscation transform, derived from disassembly, independently confirmed against this real hardware output**: `output_byte = (~payload_byte + table_byte) & 0xFF`, using the fixed lookup table baked into the bootloader's own flash at `0x08001F72`. Checked all 8 observed output bytes against this formula using the real table bytes read directly from the binary — **every single byte matches exactly**:

| index | payload | table | computed | observed (real flash) |
|---|---|---|---|---|
| 0 | `0x00` | `0x78` | `0x77` | `0x77` |
| 1 | `0x01` | `0xF8` | `0xF6` | `0xF6` |
| 2 | `0x02` | `0x79` | `0x76` | `0x76` |
| 3 | `0x03` | `0xF9` | `0xF5` | `0xF5` |
| 4 | `0x04` | `0x7A` | `0x75` | `0x75` |
| 5 | `0x05` | `0xFA` | `0xF4` | `0xF4` |
| 6 | `0x06` | `0x7B` | `0x74` | `0x74` |
| 7 | `0x07` | `0xFB` | `0xF3` | `0xF3` |

**Conclusion, hardware-confirmed not just plausible**: section 27's finding is real. The real factory bootloader has a genuine, working, application-only firmware-update write path — trigger, erase (application region only, bootloader untouched), receive, per-byte de-obfuscate via a fixed table, and flash-program — and this session independently reproduced and verified the exact transform algorithm against real silicon output, not just a disassembly guess. **Still open, and still required before this could inform an actual live-vehicle update**: the real UART-side framing this test deliberately bypassed (magic sync string's exact bytes, packet header layout, retry/NAK behavior) — this test proves the *backend* (buffer-in, flash-out) is real and correct; the *frontend* (how a real host actually gets 128 bytes into that buffer over the wire) still needs its own dedicated trace and, ideally, a from-scratch host-side tool test once traced. Board restored to the known-good clean-room combo afterward, as always.

### 29. Disassembly trace of the real UART frontend framing — sections 27-28's "still open" item, partially resolved (2026-09-16, static only, NOT hardware-tested — UART is not yet physically wired on the bench)

Direct continuation of section 27's stated gap. Traced the 22-state `tbb` byte-classifier state machine (`0x08001A28`+, already located in section 27 but not fully decoded state-by-state there) instruction-by-instruction for all 22 states, to answer exactly what bytes a host must send and what the bootloader replies.

**Two independent accepted trigger sequences were found, both feeding the same final accept action:**

1. **States 0-4 (a 5-byte header)**: state 0 requires the received byte `== 0x2E` (this project's already-documented framing/sync byte), state 1 requires `== 0xE1`, state 2 requires `== 0x02`. States 3 and 4 do **not** compare literal bytes — they each shift a running 16-bit value and OR/store the newly received byte into it (`strh`/`ldrh` on a peripheral-adjacent struct field at offset `+0xc`, classic two-byte accumulator pattern: state 3 places the byte in the high half, state 4 ORs the next byte into the low half). This is most likely a 16-bit length or checksum-seed field riding in bytes 4-5 of the header, not two more literal sync bytes — exact semantics (length vs. checksum vs. block index) not yet determined from this trace alone. So the confirmed literal prefix is **`2E E1 02 <b3> <b4>`**, where `b3`/`b4` are host-supplied and combined into one 16-bit field.
2. **States 15-20 (a 6-byte literal ASCII string)**: `cmp r4,#0x58` ('X'), `#0x49` ('I'), `#0x4E` ('N'), `#0x42` ('B'), `#0x41` ('A'), `#0x53` ('S') in strict sequence — i.e. the literal ASCII bytes **`"XINBAS"`** (`58 49 4E 42 41 53`), a completely separate 6-byte magic string with no accumulator step, terminating at state 21.

State 5 (the terminus of path 1) and state 21 (the terminus of path 2) execute **byte-identical bodies** (state 21 literally branches into state 5's own success tail at `0x8001abc`): both read one hardware/status register (`ldrh r0,[r_literal,#0xc]`), and if it's zero, call `0x80019a2` and return `r0=1` (accept); if nonzero, call `0x8001956` instead (a distinct, differently-coded error path) before falling through to the shared reset target. This means **either the 5-byte `2E E1 02 b3 b4` header or the 6-byte ASCII string `"XINBAS"` independently satisfies the front-end and reaches the same accept/dispatch step** — two valid ways in for a host, not one.

**The ACK/NAK reply frames, fully confirmed byte-for-byte (both functions disassembled in full)**:
- `0x80019a2` (accept path): builds and sends **`2E E2 01 01 1B`** — matches the already-documented `0x2E`-framed, `0xE2`=`MCU_CMD_UPGRADE_ACK` reply from section 27, now confirmed as the literal byte sequence, not just "a 5-byte ACK."
- `0x8001956` (reject path): builds and sends **`2E E2 01 00 1C`** — same framing/command byte, but byte 4 (the status/success flag) is `0x00` instead of `0x01`, and the final byte (a checksum or terminator, not yet determined which) is `0x1C` instead of `0x1B`. **This is the first time this project has confirmed the actual NAK byte sequence** — previously only the ACK had been traced.

**Retry/timeout behavior, partially confirmed**: any state mismatch (e.g. wrong byte at states 0-2 or 15-20) falls through to a shared reset target (`0x08001B34`) that zeroes the local state variable (`r5=0`) and enters a bounded retry-count loop (`adds r6,r6,#1; cmp r6,<limit>; blo <continue-waiting>`) — reading more bytes and re-trying framing detection from scratch, up to a literal-pool-defined retry limit (not yet extracted — literal pool value not read in this pass), after which it resets the retry counter and explicitly calls the NAK sender (`0x8001956`, i.e. sends `2E E2 01 00 1C`) rather than silently hanging. So: **a malformed/unrecognized byte is not fatal** — the machine keeps trying to resync on incoming bytes, and only sends an explicit NAK after exhausting a bounded retry budget, never before.

**What remains open**: the exact meaning of the states-3/4 16-bit accumulator field (length vs. checksum-seed vs. something else) and its expected/validated value (state 5's own "reads a register, branches on zero/nonzero" check reads from a *peripheral-adjacent* address, not obviously the accumulator value itself — these may be two separate checks, not yet disentangled); the literal retry-limit constant; and — as with sections 27-28 — **none of this has been hardware-tested with real UART signaling**, since USART2 still is not physically wired on the bench. This is a disassembly-only trace, following the same "trust but verify" standard as the rest of this document: treat it as a strong, specific hypothesis to test against real wiring once the UART-USB adapter is connected, not as confirmed-on-silicon fact the way sections 27-28's backend now is.

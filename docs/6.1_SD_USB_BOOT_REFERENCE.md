...
**Status:** Reference — full command sequences, build-tool flags, and file layouts behind §6.0's
summary table. All three methods below are hardware-confirmed for kernel loading; see the table
in README §6.0 for the current per-method status.
...
> **U-Boot patching here is the same technique as [Self-contained SD auto-boot](#self-contained-sd-auto-boot-env-relocation--hybrid-boot) above** — the "Patch U-Boot for SD boot" toggle below uses the env relocation method (safe on a raw/Holden-derived `uboot.bin`, avoiding the 52-byte compiled-in space limitation). If it doesn't work on your unit, toggle U-Boot patching off (or pass `--uboot PATH`) and rely on [Manual SD Card Boot](#manual-sd-card-boot) instead.
...
"Redirect bootlogo/bootanimation/etc to SD" controls the `/nanddata/` symlink patch described below: on (default) symlinks mtd8–11 to files on the SD card, off leaves `/dev/mtd8`–`/dev/mtd11` reading whatever's already in NAND — useful when testing a change without needing real dumps of those partitions.
...
Requirements — `parted`, `mkfs.fat` (dosfstools), `mkfs.ext4` (e2fsprogs), `losetup` (util-linux), `rsync`, `python3` — are checked at startup, same as `build_update.sh`'s requirements check (plus `mtd-utils` if also building rootfs/userdata images via the [Build & Flash Tool](../README.md#100-build--flash-tool)).
...
| 10 | reversingtrack | `mtd10_reversingtrack/reversingtrack` | Dumped (1.2 MB, RSTK format — see [`docs/8.1_PARTITION_LAYOUT.md`](8.1_PARTITION_LAYOUT.md)) |

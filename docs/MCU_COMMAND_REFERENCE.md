...
| ↳ | — | — | **A separate report proposed** `GPIOB Pin 2` is literally the vehicle's reverse-gear wire / camera power line, framing the whole mechanism as "the firmware handles reverse-camera switching entirely autonomously once armed, no `custom_ui` involvement needed." **Plausible, not confirmed** — same evidentiary gap as above, no schematic/probe evidence, only a coherent story layered onto the already-confirmed edge mechanism | ❌ hypothesis only |
| ↳ | — | — | **Real tension found (2026-08-31) between this mechanism and `custom_ui`'s own code, not yet resolved either way**: `main.cpp`'s reverse-gear handler currently sends `id=0x11=(reverseEngaged && factoryCamera) ? 1 : 0` on *every* transition — including forcing `id=0x11=0` unconditionally on disengage. That disengage send is **not theoretical** — it's an already hardware-confirmed fix (2026-08-31, real user-observed regression: "stuck in the video mux" after exiting reverse with OEM camera on) for a real bug. Whether that software resend is (a) genuinely redundant with the autonomous `flag_5e`-poller path above, or (b) a necessary backup because the autonomous path doesn't always fire reliably in practice, **cannot be determined from static disassembly alone** — it needs a real hardware test (does removing the disengage resend regress the mux-stuck bug?). **Decision: not changed.** A narrower candidate fix was identified — stop tying the *engage*-time resend to `reverseEngaged` (arming should reflect the static preference, not the transient state) while leaving the disengage `id=0x11=0` safety-net send in place — but this needs real hardware validation before implementing, given the risk of regressing a confirmed-working fix | ⚠️ open, needs hardware test before any code change |

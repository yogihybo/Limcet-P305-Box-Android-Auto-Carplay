...
| `CMD 0x12` presence | any frame with this cmd byte, no payload check | Live capture (2026-08-31) confirms it fires reliably on reverse-gear **disengage** | **Three different unreconciled guesses** across this project's own history (unresolved status event / "DIP switch profile" / "reverse disengaged"), zero disassembly support for any. Also confirmed firing **once at app startup**, unrelated to reverse gear, as part of the same telemetry burst the init handshake triggers — the same byte now directly observed doing two different real things. **New real hardware evidence (2026-09-01)**: user reports a consistent ~5s delay between physically leaving reverse and `custom_ui` actually switching back — and a real boot-log capture the same day shows `CMD 0x04` (engage) firing at t=19.467s, `CMD 0x12` (disengage) not firing until t=25.231s (**5.76s later**), then `CMD 0x04` firing *again* just 4ms after that. A 4ms engage→disengage→re-engage flip is not plausible real gear-shifting behavior — this pattern is much more consistent with `CMD 0x12` being a periodic status/telemetry broadcast we happen to catch on its next cycle, not an edge-triggered "left reverse" push. **Correction, same-day cross-check against a separately-captured MCU Live Log photo**: the two real `0x12` payloads captured that session (`[01 04 00]` and `[02 01 00]`) were independently timestamp-matched against this exact boot log's own `DISENGAGED` print lines (`25.230979`↔`25.231`, `42.358503`↔`42.358603`) — confirming these ARE the exact frames `custom_ui` reacted to, essentially instantly (sub-100ms of log-flush jitter, no software-side debounce). So the ~5s lag is not a `custom_ui` detection bug — it's on the MCU side, either a genuine multi-second delay before the MCU reports leaving reverse, or (still the more likely read, given the confirmed startup-burst misfire) a periodic broadcast we catch on its next cycle rather than push-on-edge. The payload also isn't constant between the two confirmed disengage events (`01 04 00` vs `02 01 00`), so there's no simple fixed signature in what's been captured so far to gate on | ✅ used as "DISENGAGED" trigger — weaker case than `0x04`'s, now with concrete hardware evidence the ~5s lag is MCU-side, not a `custom_ui` bug |
| `CMD 0x01` bit `2` + `CMD 0x12`'s `payload[1]==0x11` gate | both post the exact same real Qt event, `MsnEvent(0x5026)`, to app id `0x191` | Real, disassembly-confirmed convergence between two independently-traced commands — a named, specific shared event, not just "this byte showed up." Confirmed via a full-binary search: within the confirmed-active `MCUAdapter_BoxP300` class, only these two commands post it | What actually *consumes* event `0x5026` was traced as far as `MsnCoreApp`'s own main dispatcher — which does **not** test for it at all, so no confirmed downstream behavior either way. Never behaviorally tested against real reverse-gear toggling (unlike `0x04`/`0x12`'s live-capture correlation). **RETRACTED (2026-09-01)**: this row's `payload[1]==0x11` gate was floated as a possible discriminator for "the real disengage frame" vs. other `0x12` traffic, but the two real, confirmed-disengage-correlated `0x12` payloads captured this session (`[01 04 00]`, `[02 01 00]`) both have `payload[1]` of `0x04`/`0x01` — neither is `0x11`. This gate is about the unrelated `0x5026` Qt-event convergence, not a usable signal for isolating the disengage-relevant `0x12` occurrence | ❌ not wired in — a real, promising lead for the `0x5026` question specifically, not applicable to the disengage-timing question |
| `/dev/carback` | dedicated SoC-level GPIO IRQ, real kernel driver (`linux-arkmicro/linux/drivers/soc/arkmicro/ark-carback.c`) | Completely independent of the MCU UART protocol and all of the guessing above — a real, purpose-built reverse-gear hardware signal | None found — the strongest-evidenced signal of the four, but **currently unavailable at runtime** on this exact build/boot (`[HAL:REVCAM] WARN: /dev/carback unavailable (No such file or directory)`, 2026-09-01 boot log) — not a live fallback option right now, see `project_carback_probe_order_bug.md` memory (IRQ deliberately disabled 2026-08-03) | ✅ present in code, but only checked **second**, as a fallback if the MCU-UART signal doesn't fire first — and not actually usable on this boot regardless |
